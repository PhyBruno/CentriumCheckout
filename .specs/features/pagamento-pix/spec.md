...
- WHEN `StatusPIX` retorna `StatusTransacao` THEN o sistema SHALL interpretar o valor por nome semântico — `PagamentoRecebido` é o único estado tratado como aprovado; `Aguardando` (e o estado inicial da transação) mantém o polling; `Expirada`/`Recusada`/`Erro` são tratados como falha terminal, reaproveitando o **mesmo** fluxo já decidido para o fechamento manual acima (aviso de desassociação manual, remoção do pagamento local, nenhuma chamada de cancelamento) — não é um segundo mecanismo de estado, é o mesmo caminho acionado por um gatilho diferente. **Achado (2026-08-27), fase Design da feature 009:** os cinco nomes de estado foram confirmados lendo o código-fonte real do ERP via KB GeneXus; os literais exatos armazenados por cada nome permanecem uma pendência não-bloqueante — ver item 33 de `.specs/project/PENDENCIES.md` e `specs/009-pagamento-pix/research.md` (D8).
