...
- **Novo (2026-09-21, AD-250):** WHEN a forma de meio `Pix` traz `FormaIntegracaoCartao = '1'` (`FPGNFTEFPO`) AND a empresa tem `ConfiguracoesTEF.TEFAtivo` THEN esta feature **não é acionada**: a cobrança pertence ao endpoint do TEF (feature 010), e o sistema SHALL NOT chamar `GerarPIX` — nenhum QR Code, nenhum "copia e cola", mesmo com `UtilizaCentriumPAG = true`. A condição é avaliada pelo roteamento da feature 008 (`resolverIntegracao`), que devolve o veredito `TEF` e nunca `PIX_DINAMICO` nesse caso; esta feature continua reagindo só ao veredito, sem reavaliar o cadastro da forma.
