...
**Coverage:** 12 total, 1 edge case resolvido em 2026-08-21 (impressão pós-autorização, AD-024), 1 edge case que só a equipe do ERP resolve diretamente (semântica dos códigos de `GetStatusSistema`, confirmado como lacuna de documentação do próprio ERP) e 2 pendências novas de 2026-08-25 (contrato técnico do serviço de impressão local + indicativo no `GetSessao`, AD-037; timing de chamada de `GetStatusSistema`, AD-051) — ver `.specs/project/PENDENCIES.md`. **Correção (2026-08-25, AD-062):** `FIN-02` deixa de depender do campo `produtoCancelado` (removido do escopo) — trilha de auditoria de cancelamento passa a viver no log geral de `FIN-12`.
