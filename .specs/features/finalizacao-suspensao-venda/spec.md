...
3. WHEN a venda foi carregada de um documento existente no ERP (rascunho via `CarregarNFCe` ou DAV via `GetDav`) THEN o sistema SHALL enviar `NumeroRascunho` preenchido com o número devolvido pelo ERP; WHEN a venda foi criada do zero no Checkout THEN o sistema SHALL enviar `NumeroRascunho = 0`; WHEN um envio (`SUSPENDER` ou `FATURAR`) é recusado com o rascunho já gravado — o ERP devolve `NumeroRascunho > 0` junto das `messages` — THEN o sistema SHALL adotar esse número e enviá-lo nos envios seguintes da mesma venda, mantendo a origem dela. O mesmo número vai em `SUSPENDER`, `FATURAR` e `VALIDAR`. **Atualizado (2026-09-16, AD-235):** o contrato de `20260914191012` renomeou o campo de primeiro nível `NumeroNota` para `NumeroRascunho`; o `NumeroNota` de dentro de `NotaFiscal` (a nota fiscal emitida) não mudou.
...
**Independent Test**: Finalizar uma venda criada do zero (`NumeroRascunho=0`) e uma carregada de rascunho (`NumeroRascunho` preenchido); suspender uma venda nova com o ERP recusando e devolvendo um número, e confirmar que o reenvio leva esse número; confirmar payload correto em cada caso.
...
- WHEN a NFCe é autorizada THEN o diálogo do documento fiscal SHALL identificar a nota como `NFCe <NumeroNota> · série <SerieNota>`, omitindo o número `0` e a série vazia. **Novo (2026-09-16, AD-238).**
- WHEN a NFCe é gravada e não autorizada (`Autorizada ≠ 'S'`) THEN o diálogo de rejeição SHALL mostrar o bloco "Retorno da SEFAZ" (código em campo próprio e mensagem, com o HTML de `ErroMensagem` reduzido a texto), a "Sugestão de correção" de `RetornoMensagemIA` **como texto puro** com as quebras de linha preservadas, e o link "Abrir no ERP" de `UrlChamadas` **só** quando a URL for `http(s)` absoluta, aberto em nova aba com `rel="noopener noreferrer"`. Sugestão e link ausentes (desfecho `N`, ERP anterior ao contrato) não geram bloco vazio. **Novo (2026-09-16, AD-238).**
...
- WHEN é necessário obter o `NumeroRascunho` antes de faturar uma venda criada do zero THEN o sistema SHALL enviar `NumeroRascunho = 0`, sem controlar nenhum contador local. **Resolvido (2026-08-21, AD-023; campo renomeado em 2026-09-16, AD-235):** confirmado na KB do GenExus — `PCheckout_FaturarNFCe` trata `NumeroRascunho = 0` gravando um rascunho novo via `PNFeSerializaRascunhoNota.Call(..., 'SERIALIZA', ...)` (`NewCapa`), sempre numerado pelo ERP; o Checkout nunca precisa de um `GetProximoNumeroNFCe` ou equivalente. `NumeroRascunho <> 0` usa `AtualizarCapa` sobre o rascunho existente e ocorre em dois casos: a venda veio de um rascunho/DAV (ver `FIN-03`), ou um envio anterior da mesma venda foi recusado depois de gravar o rascunho — a procedure preenche `NumeroRascunho` na resposta **antes** de rodar `PNFCe_ValidaSaldoProdutos`/`PNfeValidaRascunho`, e reenviar com `0` criaria um segundo rascunho da mesma compra (decisão do usuário: adotar o número devolvido).
...
| FIN-03 | `NumeroRascunho` correto (0, do documento, ou adotado após recusa — AD-235) | - | Verified |
...
| FIN-08 | `CadSerieNFCe` enviado em `FaturarNFCe` (FATURAR e SUSPENDER): a série **do documento** quando a venda veio de DAV/NFCe ou adotou um rascunho, e a de `SessaoUsuario.CadSerieNFCe` como recuo (AD-239 — a da sessão vem vazia no tenant de preview) | - | Verified (2026-08-25, AD-034 — clarificação de processo do usuário) |
| FIN-09 | Falha de rede em `FaturarNFCe` exige confirmação manual antes de reenvio | - | Verified (2026-08-25, AD-038) |
| FIN-16 | Os desfechos ruins do envio são **quatro**, e cada um tem cópia e efeito próprios (AD-239): **falha técnica** (HTTP/sessão/resposta sem desfecho) devolve a venda ao caixa; **recusa de validação** (`messages` com `Type: 1`) também devolve, com tom de aviso e o verbo da operação, e adota o rascunho gravado; **NFCe rejeitada** (bloco `NotaFiscal` com `Autorizada ≠ 'S'`) libera o caixa ao fechar e identifica o documento pelo **rascunho** (`NumeroRascunho`/`CadSerieNFCe` da raiz — o ERP devolve `NotaFiscal.NumeroNota: "0"` na rejeição, medido em 2026-09-16), com sugestão da IA e link "Consultar solução detalhada"; **cenário tributário não encontrado** libera o caixa e manda o operador ao ERP, porque a correção é de cadastro fiscal | - | Verified (2026-09-16, AD-239) |
