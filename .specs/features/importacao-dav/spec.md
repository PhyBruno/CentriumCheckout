...
2. WHEN o DAV é carregado THEN o sistema SHALL reusar exatamente o mesmo mecanismo de import/mapeamento já usado para retomar um rascunho de NFCe (`.specs/features/recuperacao-nfce/spec.md`, story "Retomar rascunho de NFCe para o carrinho") — incluindo preservar `NumeroRascunho` (reenviado em `FaturarNFCe`; era `NumeroNota` até o contrato de 2026-09-14, AD-235 — o `GetDav` converte o DAV num rascunho de NFCe e devolve o número dele) e manter o preço de cada item preservado/congelado exatamente como veio no rascunho, sem disparar o motor de precificação (`.specs/features/carrinho-produto-precificacao/spec.md`) automaticamente — e não uma lógica de mapeamento própria a partir de `DavItemStruct`/`DavForPagamento`. **Resolvido (2026-08-25, AD-057).**
3. WHEN um DAV é importado THEN o sistema SHALL sempre sobrescrever o cliente e o vendedor default da venda (pré-selecionados via `GetSessao`, AD-032) pelos dados de cliente/vendedor trazidos no próprio DAV — nunca preservar o default anterior; WHEN o `GetDav` devolve `vendedorCodigo = 0` THEN o sistema SHALL usar o `VendedorCodigo` (e o `VendedorNome`, quando vier) da linha de `ListaDAVs`. **Resolvido (2026-08-25, AD-055):** decisão direta do usuário. **Atualizado (2026-09-16, AD-235):** o fallback pela linha cobre a divergência do ERP de preview que devolve `0` no documento (`.specs/project/PENDENCIES.md`, item 56).
4. WHEN o documento importado (`GetDav` ou `CarregarNFCe`) traz `ClienteNome` e o `GetCliente` do cliente falha por rede, resposta inválida ou cadastro em branco THEN o sistema SHALL importar mesmo assim, com o cliente identificado por código e `ClienteNome` do documento (lista de preço e convênio ausentes, nunca inventados); WHEN o `GetCliente` responde que o cliente não existe, a sessão caiu ou o documento não traz nome THEN a importação SHALL ser abortada sem mutação. O `GetCliente` SHALL continuar sendo chamado sempre, porque traz lista de preço, convênio, crediário e celular. **Novo (2026-09-16, AD-237).**
...
| DAV-06 | O item importado exibe o **nome** do produto: o código do documento vai sem os espaços do `char` do ERP e a descrição é consultada por código interno (`Tipocodproduto='R'`), não pelo tipo da sessão. A série do documento (`CadSerieNFCe`) acompanha o rascunho na identidade da venda e volta no `FaturarNFCe`; um documento **sem itens** é importável (o ERP omite a chave `produtos`), e quem separa documento de recusa é `NumeroRascunho > 0` | - | Verified (2026-09-16, AD-239 — vale igual para a retomada de NFCe, que usa o mesmo mecanismo) |
| DAV-04 | Importação sempre sobrescreve cliente/vendedor default | - | Verified (2026-08-25, AD-055) |

**Coverage:** 4 total. Filtro de data em `ListaDAVs` resolvido por completo (2026-08-26, AD-077/AD-087 — parâmetros `Datainicial`/`Datafinal` já no contrato); a janela fixa "hoje" deixa de existir, mas filtros de status/vendedor/tipo/origem seguem sem suporte server-side, sem decisão de produto pendente sobre eles (nunca fizeram parte do escopo prometido pelo ERP). `GetDavOutput` já corrigido no contrato desde 2026-08-25 (AD-057) — não é mais pendência (item 26 de `.specs/project/PENDENCIES.md` já removido). Mecanismo de marcar DAV como importada/faturada resolvido (2026-08-25, AD-058) — o próprio ERP fecha a DAV automaticamente a partir do vínculo interno criado por `GetDAV` (AD-057), sem depender de campo novo em `CheckoutFaturarNFCe`. Ação de reimpressão por linha resolvida (2026-08-25, AD-035) — não será implementada, fora de escopo por decisão direta do usuário. Concorrência entre operadores resolvida (2026-08-25, AD-052) — sem lock no Checkout, ERP resolve.
