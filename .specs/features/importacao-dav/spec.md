...
**Independent Test**: Importar um DAV mockado com 2 itens e 1 forma de pagamento; verificar que o carrinho reflete exatamente esses dados antes de qualquer edição manual. Importar um DAV com cliente/vendedor diferentes do default da sessão e confirmar que o DAV sobrescreve ambos.
...
**Coverage:** 4 total, 1 edge case pendente de decisão de produto (janela fixa "hoje + aberto" em `ListaDAVs`, filtros de vendedor/tipo/origem sem suporte, 2026-08-21 AD-024), 1 pendência de KB do ERP (campo/lógica de marcação de DAV importado em `CheckoutFaturarNFCe` — "PENDÊNCIA DEV", reforçada em AD-024 como mudança de KB, não só de nomenclatura, ver `.specs/project/STATE.md`). Ação de reimpressão por linha resolvida (2026-08-25, AD-035) — não será implementada, fora de escopo por decisão direta do usuário. Concorrência entre operadores resolvida (2026-08-25, AD-052) — sem lock no Checkout, ERP resolve.
