...
- WHEN o cliente tem `DescontoConvenio` aplicável THEN o sistema SHALL tratar o valor como percentual. **Precisão (2026-08-31, AD-108):** o **cliente default** nunca tem convênio aplicável — `descontoConvenio = 0` para `origem = 'DEFAULT'`, sem nenhuma consulta ao cadastro do cliente. **Resolvido (2026-08-21, AD-023):** confirmado na KB do GenExus — `PGeraPedidoVenda` calcula `&ConvDsc = (1 - CliConvDsc / 100)`, fator de desconto percentual (ver também `.specs/features/identificacao-cadastro-cliente/spec.md`).
