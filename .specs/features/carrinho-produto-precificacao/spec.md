...
**Coverage:** 10 total, 10 requisitos confirmados e prontos para Design/Tasks (`CART-09`/`CART-10` resolvidos em 2026-08-24, AD-030, por decisão direta do usuário), 4 edge cases pendentes de confirmação com equipe do ERP (marcação de autoria de cancelamento resolvida em 2026-08-24, AD-026 — campo `produtoCancelado`, ainda não implementado no ERP; formato de código de barras pesável resolvido em 2026-08-24, AD-028 — detecção pelo comprimento/prefixo confirmada, parse fino dos dígitos internos segue em aberto como detalhe de Design). **Atualização (2026-08-25):** padrão de arredondamento monetário generalizado (AD-039), interação com split de pagamento (AD-036), preço congelado ao retomar rascunho via `recuperacao-nfce` (AD-041) e recálculo de `TipoPreco=9` ao trocar cliente com carrinho populado (AD-043) documentados nos Edge Cases — nenhum deles introduz pendência nova.
