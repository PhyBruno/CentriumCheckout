...
- WHEN uma NFCe nasce com o cliente default pré-selecionado (`ClienteDefaultCodigo`/`ClienteDefaultNome`, AD-032) e o operador nunca abre o modal de busca THEN o sistema tem apenas código e nome desse cliente — **não** tem `ListaPreco` nem `DescontoConvenio`, campos exigidos por `TipoPreco = 9` (`.specs/features/carrinho-produto-precificacao/spec.md`, AD-092). **Pendência bloqueante (2026-08-26, AD-094, item 31 de `.specs/project/PENDENCIES.md`):** `GetCliente` só aceita `CPFCNPJ` como filtro — não existe parâmetro por código — então não há como o Checkout buscar os dados completos do cliente default sem o CPF dele, que também não vem em `GetSessao`. Aguardando a equipe do ERP (parâmetro `CodCliente` em `GetCliente`, ou os campos já embutidos em `GetSessao`).
...
**Coverage:** 8 total, 0 mapeados a tasks, 0 requisitos pendentes, 1 edge case bloqueante de contrato (dados completos do cliente default sem CPF disponível — AD-094, item 31 de `PENDENCIES.md`), 0 edge cases pendentes de decisão de produto (campos "Limite de crédito"/"Permite venda a crédito" — decisão de remover do design confirmada em 2026-08-24, AD-026; remoção visual no Pencil ainda não aplicada; filtro "Ativo" removido do modal por falta de campo de status no contrato, AD-093). Adaptação de layout mobile do cadastro simplificado pendente na fase Design de `layout-responsivo-mobile` (AD-046).
