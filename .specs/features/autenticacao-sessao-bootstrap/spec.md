...
4. WHEN a troca de credenciais por token, a chamada a `/api/bootstrap` e o parse/validação do payload ainda não terminaram THEN a interface SHALL exibir uma tela de carregamento bloqueante ("montando a sessão") — podendo ser em formato skeleton via Boneyard (AD-005/AD-007 em `.specs/project/STATE.md`) — a tela principal só aparece depois que tudo termina com sucesso, nunca parcialmente carregada.
