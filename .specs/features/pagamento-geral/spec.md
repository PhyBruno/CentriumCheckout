...
1. WHEN o operador aplica um ticket devolução THEN o sistema SHALL chamar `POST /ApiCentriumOAuth/ValidaTicketDevolucao`, que retorna `ValorTicket: number`, `Valido: boolean` e `Mensagem: string`. **Regra vigente (2026-08-26, AD-099):** a validade é decidida por `Valido` quando o campo vem presente na resposta, e pela comparação `Mensagem === 'Ticket Válido'` quando `Valido` vem ausente/`undefined`. O frontend **nunca** assume "HTTP 200 = válido", e o fallback de `Mensagem` **não pode ser removido** enquanto o item 32 de `.specs/project/PENDENCIES.md` estiver aberto. **Correção de AD-023:** a redação anterior deste AC afirmava que "não existe campo booleano de validade" e mandava usar apenas a comparação de `Mensagem` — isso está **superado**: o campo `Valido` existe no contrato (`ApiCentriumOAuth.yaml`, linhas 668-676). O que segue em aberto é apenas se `PCheckout_ValidaTicketDevolucao` efetivamente o preenche, já que a inspeção da KB de 2026-08-21 (`PValidaTicketNfCe.Call`) só encontrou a comparação de string.
...
| PAY-05 | Ticket devolução — valor via `ValidaTicketDevolucao`; validade por `Valido` com fallback para `Mensagem` | Design | Verified com pendência (2026-08-26, AD-099 — corrige AD-023: o campo `Valido` existe no contrato; item 32 de `PENDENCIES.md` aberto para o ERP confirmar se é preenchido) |
...
**Coverage:** 10 total, 0 edge cases bloqueantes, **1 pendência aberta** (item 32 de `.specs/project/PENDENCIES.md`, não-bloqueante — campo `Valido` de `ValidaTicketDevolucao`, AD-099; o item 30 foi resolvido em 2026-08-26 por AD-078). `PAY-04` (status PIX) fica em `.specs/features/pagamento-pix/spec.md`.

**Fase Design concluída (2026-08-26):** `/speckit-plan` gerou `specs/008-pagamento-geral/` (`plan.md`, `research.md` com as decisões D1-D14, `data-model.md`, `contracts/erp-pagamento-api.md`, `contracts/pagamento-domain-api.md`, `quickstart.md`). Três achados de contrato viraram AD-097, AD-098 e AD-099. Próximo passo: `/speckit-tasks`.
