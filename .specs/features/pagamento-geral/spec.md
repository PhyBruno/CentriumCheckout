...
1. WHEN `FormaMeioPagtoNFe` for `CartaoCredito` ou `CartaoDebito` AND `ConfiguracoesTEF.TEFAtivo` for `true` AND `FormaIntegracaoCartao` for `'1'` THEN o sistema SHALL chamar a integração TEF local e somente adicionar o pagamento após a aprovação do TEF. **Corrigido (2026-09-08, AD-180):** a redação anterior exigia só as duas primeiras condições. Faltava a terceira — `FormaIntegracaoCartao` (`FPGNFTEFPO`) igual a `'1'` marca a forma como TEF; `'2'` ou vazio marca **POS/avulso**, e uma empresa que usa TEF pode deliberadamente cadastrar formas de cartão para serem cobradas em maquininha fora do Checkout.
1-A. WHEN `FormaMeioPagtoNFe` for `CartaoCredito` ou `CartaoDebito` AND `FormaIntegracaoCartao` for `'2'` ou vazio THEN o sistema SHALL NOT chamar a integração TEF, SHALL manter a forma disponível na tela e SHALL tratá-la como pagamento manual (cobrado fora do Checkout, confirmado pelo operador) — o mesmo tratamento já dado a cartão em empresa sem `TEFAtivo`. **Novo (2026-09-08, AD-180).**
2. WHEN `FormaMeioPagtoNFe` for `Pix` AND `FormaIntegracaoCartao` **não** for `'1'` (ou a empresa não tiver `ConfiguracoesTEF.TEFAtivo`) AND `ConfiguracoesPIX.UtilizaCentriumPAG` for `true` THEN o sistema SHALL chamar `POST /ApiCentriumOAuth/GerarPIX`, consultar `GET /ApiCentriumOAuth/StatusPIX` a cada 10 segundos e somente adicionar o pagamento após a aprovação do PIX. **Corrigido (2026-09-21, AD-250):** a redação anterior mandava todo `Pix` ao CentriumPAG sob `UtilizaCentriumPAG`, sem olhar o cadastro da forma. `FPGNFTEFPO` não é campo só de cartão — ver 2-A.
2-A. WHEN `FormaMeioPagtoNFe` for `Pix` AND `ConfiguracoesTEF.TEFAtivo` for `true` AND `FormaIntegracaoCartao` for `'1'` THEN o sistema SHALL encaminhar a cobrança ao **endpoint do TEF** e SHALL NOT chamar `GerarPIX` — nenhum QR Code, nenhum código "copia e cola", mesmo com `UtilizaCentriumPAG = true`. **Novo (2026-09-21, AD-250):** regra de negócio informada pelo usuário; gerar a cobrança no CentriumPAG criaria uma segunda cobrança para o dinheiro que o terminal já está cobrando. O endpoint do TEF **ainda não existe** (feature 010): até ele existir, essa forma fica `PENDENTE_INTEGRACAO` sem quem a confirme — efeito conhecido e aceito pelo usuário na mesma decisão.
...
**Independent Test**: Mockar formas com `FormaMeioPagtoNFe` igual a `CartaoCredito`, `CartaoDebito`, `Pix`, `PixEstatico` e `Dinheiro`, cada uma nas três variantes de `FormaIntegracaoCartao` (`'1'`, `'2'`, vazio); confirmar que cartão de crédito/débito **e** `Pix` chamam TEF exatamente quando `FormaIntegracaoCartao = '1'` e a empresa tem `TEFAtivo` (AD-180 para cartão, AD-250 para PIX — nas demais variantes o cartão vira pagamento avulso e o `Pix` vai ao CentriumPAG), que somente `Pix` **fora** dessa condição chama o fluxo PIX dinâmico, e que as demais formas não chamam integração externa. Repetir com layout mobile e confirmar que o resultado é **idêntico** ao do desktop para todas as formas — inclusive cartão com `TEFAtivo=true`, que chama TEF (AD-144).
...
- WHEN uma forma de pagamento é selecionada THEN o Checkout SHALL rotear a operação pelo valor de `FormaMeioPagtoNFe` **combinado com `FormaIntegracaoCartao`**: `CartaoCredito`, `CartaoDebito` **e `Pix`** usam TEF quando `ConfiguracoesTEF.TEFAtivo = true` E `FormaIntegracaoCartao = '1'` (ver `PAY-08` acima); `Pix` fora dessa condição usa PIX dinâmico; `PixEstatico` não usa automaticamente o fluxo PIX dinâmico; os demais valores não usam essas integrações. **Regra vigente desde 2026-09-08 (AD-180) e estendida ao PIX em 2026-09-21 (AD-250):** quem paga pelo terminal é distinguido pelo campo `SDTCheckout_GetSessao.CondicoesDePagamento[].CondicaoFormasDePagamento[].FormaIntegracaoCartao` (origem `FpgNfTefPos`/`FPGNFTEFPO`, domínio `NFCe_tpIntegra`) — `'1'` = TEF (`PagtoIntegrado`), `'2'` **ou vazio** = POS/avulso. O nome do campo engana: ele **não** é exclusivo do cartão. Uma empresa com TEF ligado pode cadastrar formas de cartão como POS (e o Checkout não as manda ao terminal) e formas de PIX como TEF (e o Checkout não gera QR Code para elas). **Histórico:** AD-073 (2026-08-26) registrou a impossibilidade de fazer essa distinção como pendência não-bloqueante (item 30 de `.specs/project/PENDENCIES.md`), aceitando rotear todo cartão para TEF sob `TEFAtivo`; AD-078 (2026-08-26) confirmou o campo na KB mas o deixou apenas disponível para uso futuro. As duas foram superadas por AD-180, que o torna condição obrigatória do roteamento; AD-250 corrigiu a leitura, também de AD-180, de que o campo seria irrelevante fora do cartão.
...
| PAY-08 | Roteamento por `FormaMeioPagtoNFe` **+ `FormaIntegracaoCartao`** (no cartão **e no PIX**) para TEF/PIX | - | Verified (regra confirmada pelo usuário em 2026-08-24; a exclusão de TEF no mobile de AD-074 foi **revogada em 2026-09-03, AD-144** — o roteamento não depende de plataforma; **refinado em 2026-09-08, AD-180**: cartão só chama TEF com `FormaIntegracaoCartao = '1'`, superando o trade-off aceito em AD-073 e ativando o campo que AD-078 só havia confirmado na KB; **estendido em 2026-09-21, AD-250**: o mesmo par de condições vale para `Pix` — com `'1'` + `TEFAtivo` a cobrança é do terminal e o QR Code **não** é gerado) |
