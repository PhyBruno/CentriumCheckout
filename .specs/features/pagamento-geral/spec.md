...
**Independent Test**: Mockar formas com `FormaMeioPagtoNFe` igual a `CartaoCredito`, `CartaoDebito`, `Pix`, `PixEstatico` e `Dinheiro`; confirmar que somente cartão de crédito/débito **com `FormaIntegracaoCartao = '1'`** chama TEF (AD-180 — o mesmo cartão com `'2'` ou vazio não chama, e continua disponível na tela), somente `Pix` chama o fluxo PIX dinâmico e as demais formas não chamam integração externa. Repetir com layout mobile e confirmar que o resultado é **idêntico** ao do desktop para todas as formas — inclusive cartão com `TEFAtivo=true`, que chama TEF (AD-144).
...
- WHEN uma forma de pagamento é selecionada THEN o Checkout SHALL rotear a operação pelo valor de `FormaMeioPagtoNFe`: `CartaoCredito` e `CartaoDebito` usam TEF **quando `ConfiguracoesTEF.TEFAtivo = true` E `FormaIntegracaoCartao = '1'`** (ver `PAY-08` acima); `Pix` usa PIX dinâmico; `PixEstatico` não usa automaticamente o fluxo PIX dinâmico; os demais valores não usam essas integrações. **Regra vigente desde 2026-09-08 (AD-180):** o "cartão avulso" (maquininha standalone fora do TEF) é distinguido pelo campo `SDTCheckout_GetSessao.CondicoesDePagamento[].CondicaoFormasDePagamento[].FormaIntegracaoCartao` (origem `FpgNfTefPos`/`FPGNFTEFPO`, domínio `NFCe_tpIntegra`) — `'1'` = TEF (`PagtoIntegrado`), `'2'` **ou vazio** = POS/avulso. Uma empresa com TEF ligado pode cadastrar formas de cartão como POS, e o Checkout não as manda ao terminal. **Histórico:** AD-073 (2026-08-26) registrou a impossibilidade de fazer essa distinção como pendência não-bloqueante (item 30 de `.specs/project/PENDENCIES.md`), aceitando rotear todo cartão para TEF sob `TEFAtivo`; AD-078 (2026-08-26) confirmou o campo na KB mas o deixou apenas disponível para uso futuro. As duas foram superadas por AD-180, que o torna condição obrigatória do roteamento.
