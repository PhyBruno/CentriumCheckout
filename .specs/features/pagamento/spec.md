...
3. WHEN uma forma de pagamento específica não aceita ticket devolução THEN ⚠️ pendente: `CondicaoFormasDePagamento[]` (`ApiCentriumOAuth.yaml`) não tem campo de elegibilidade a ticket devolução nem o campo `FpgUtiCar` citado no Edge Case abaixo — mecanismo real de restrição por forma de pagamento não confirmado (fora do escopo da verificação de AD-023).
...
| PAY-07 | Ticket devolução — elegibilidade por forma de pagamento (`FpgUtiCar`) | - | ⚠️ Pendente — contrato não tem campo de elegibilidade correspondente (não coberto pela verificação de AD-023) |

**Coverage:** 7 total, 1 requisito (`PAY-07`) e 2 edge cases pendentes de confirmação com equipe do ERP (intervalo de polling, `FpgUtiCar`).
