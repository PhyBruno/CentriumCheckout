...
**Current Work:** **2026-09-03 (AD-143) — bloqueio explicativo vira padrão da base:** `src/client/lib/bloqueio.ts` (novo) define `MotivoBloqueio` (o texto que o operador lê, nunca um booleano) e os helpers `atributosDeBloqueio`/`acaoBloqueavel`; `Button` ganhou os estados de `aria-disabled`. Aplicado a "Cancelar venda" (que trocou `enviando`/`bloqueado: boolean` pelo motivo textual), ao botão de inserir item e a "Identificar" cliente, com o texto espelhando termo a termo a condição que já bloqueava. Vale de agora em diante para **todo** botão que desabilite. 457 unitários/integração e 110 E2E verdes. Anteriormente: **2026-09-03 (AD-142) — o atalho de importação bloqueado explica o motivo ao ser clicado:** o `disabled` nativo da AD-140 impedia qualquer evento, então a notificação com o motivo só aparecia quando a importação era possível — ou seja, quando não era necessária. Trocado por `aria-disabled`: mesma aparência apagada e mesma leitura de "desabilitado" para leitores de tela e para os testes, mas o clique chega ao `abrir()`, que reaplica a recusa e emite a notificação. Anteriormente: **2026-09-03 (AD-141) — a recusa de importação passa a contar linha cancelada:** pedido do usuário logo após a AD-140. `itensAtivos` vira `linhasNaVenda` em `EstadoVendaParaImportacao`, e o "Menu Importação" fica desabilitado com qualquer linha no carrinho, cancelada inclusive — a linha cancelada não sai do array (`CART-08`), aparece na grid, entra no `Log` e vai no retrato de `FaturarNFCe`, então importar por cima dela misturaria dois conteúdos numa nota só. "Nada a faturar" não é "nada foi digitado". A mensagem passou a dizer "mesmo que cancelados", senão o motivo não bateria com o carrinho que o operador está vendo. Junto com a AD-140, a venda com todos os itens cancelados fica com o atalho de importação fechado **e** o "Cancelar venda" aberto — que é a saída. 457 unitários/integração verdes. Anteriormente: **2026-09-03 (AD-140) — cinco correções de uso na importação de DAV e na saída da venda:** pedidos diretos do usuário sobre a 006 já entregue, aplicados sem reabrir o ciclo de Spec Kit. ESC fecha a janela de importação (ouvinte de `window`, porque o `onKeyDown` do backdrop só valia com o foco dentro do modal); o filtro de emissão vira dois campos com calendário próprio (`src/client/components/ui/campo-data.tsx`, sem dependência nova) que abre a qualquer clique, já preenchidos com hoje − 7 dias até hoje e recalculados a cada abertura; Enter importa o documento selecionado, com a linha resolvendo a tecla por conta própria para nunca importar a linha anterior; "Menu Importação" fica `disabled` com a venda iniciada (`useImportacaoDav` passa a expor `recusa` reativa por seletor do `vendaStore`, com o motivo no `title`); e **linha cancelada volta a permitir cancelar a venda** — a regra anterior exigia linha ativa e deixava sem saída justamente a venda com todos os itens cancelados. As emissões dos DAVs sintéticos do mock E2E passaram a ser relativas a hoje, senão o filtro padrão de 7 dias quebraria a suíte com a passagem do tempo. 456 unitários/integração e 106 E2E verdes. Anteriormente: **2026-09-03 (AD-139) — a guarda pós-pagamento da identidade da venda desce para o slice, e o início de sessão vira ação separada:** a revisão da própria etapa da AD-138 achou três defeitos. (1) A lacuna **alta** que a AD-138 declarou ficou fechada só na borda da 006 — `identidadeVendaSlice` continuava sem `deps`, então qualquer outro call site gravaria a identidade com pagamento aprovado. Fechada no slice, mas dividindo a ação em duas: `iniciarIdentidadeVenda` (não guardada, exclusiva de `abrirSessaoDeVenda`) e `definirIdentidadeVenda` (guardada, no-op com aviso, usada pela 006/011) — porque `abrirSessaoDeVenda` roda **depois** de `FaturarNFCe` retornar sucesso, com o pagamento ainda aprovado, e uma guarda única teria transformado a abertura da venda seguinte num no-op silencioso no dia em que a 008 ligasse o predicado real. É o mesmo padrão que `limparCarrinho`/`descartarAuditoria`/`inicializarClientePadrao` já seguem. (2) `clienteIdentificado` usava só `houveEscolhaExplicita`, e `limparCliente()` (recusa de PJ, AD-133) zera o cliente sem zerar a flag: depois de recusar uma pessoa jurídica a venda ficava **sem cliente** e a importação era recusada por "cliente identificado", sem saída além de cancelar a venda. (3) A pré-condição era lida uma vez, antes de dois `await`; agora é reverificada colada nas mutações, senão um estado que virasse no meio registraria `DAV_IMPORTADO` com o carrinho vazio. Três suspeitas não se confirmaram (ordem dos motivos, duas instâncias do hook, guarda dupla do botão) e ficam registradas na AD. 456 unitários/integração e 102 E2E verdes. Anteriormente: **2026-09-03 (AD-138) — revisão da 006 e a regra de que um documento nunca entra numa venda em digitação:** a revisão de código (`ecc:code-reviewer`, Sonnet) sobre a implementação da 006 achou um defeito **crítico** — importar um segundo DAV sobre o primeiro sobrescrevia `identidadeVenda.numeroNota` enquanto as linhas apenas se somavam, então o ERP fechava só o último documento e o primeiro ficava aberto para sempre com os itens já faturados sob outro número, sem erro nem aviso — e um **alto** latente (`definirIdentidadeVenda` não é barrada por `podeMutarCarrinho()`, o que morde quando a 008 ligar o lock real). O usuário fixou a regra geral: um DAV/NFCe não entra numa venda que já tem cliente identificado ou item lançado, e a tentativa é recusada com notificação de erro (`FR-011`). Resolvido com uma regra pura (`recusaDeImportacao`, quatro motivos) aplicada em dois pontos — no clique do atalho, para o operador saber antes de escolher um documento, e dentro de `importarVendaExistente`, antes até da rede. O cliente **default** não conta como identificado (AD-032), senão nenhuma importação seria possível. 445 unitários/integração e 101 E2E verdes. Anteriormente: **2026-09-03 (AD-137) — feature 006 (importação de DAV) implementada:** as 25 tarefas de `specs/006-importacao-dav/tasks.md` concluídas; 436 testes unitários/integração e 98 E2E verdes, `tsc`/`eslint` limpos. Três achados que nenhum artefato de design previa: (1) a orquestração precisava gravar `identidadeVenda` — sem isso a venda importada seria faturada com `NumeroNota: 0` e o DAV nunca fecharia no ERP, já que desde AD-107 esse número é o único elo com o documento; (2) **bug pré-existente da feature 005** — `useFinalizarOuSuspenderVenda` mandava `clienteCodigo: sessao.ClienteDefaultCodigo` literal, então **toda** NFCe saía para o consumidor padrão mesmo com outro cliente identificado; corrigido e travado com dois testes de regressão; (3) cinco desvios deliberados do Pencil na janela de importação (filtros e colunas sem campo no contrato, rótulo "Importar DAV" em vez de "Faturar DAV", e o seletor `yg9zq` não construído por ter uma opção só até a feature 011 existir). `trocarVendedor` (012) e `importarFormasDePagamento` (008) seguem como stubs injetados — item 39 de `PENDENCIES.md`. Próximo passo sugerido: `/speckit-implement` na feature 008 ou 012, que fecham os dois stubs. Anteriormente: **2026-08-31 — `/speckit-analyze` aplicado na feature 007 (layout responsivo):** 6 achados, 1 CRÍTICO. O crítico (I1): a primeira geração de `tasks.md` nunca agendava a tarefa que cria `src/client/layout/desktop/DesktopLayout.tsx` — `plan.md` já listava o arquivo na árvore da feature, mas nenhuma tarefa (nem desta feature, nem de 002-006) o criava; `AppShell` (T009) ficaria importando um arquivo inexistente, travando a própria US1 (MVP). Corrigido acrescentando T008, que extrai a composição hoje inline em `src/client/App.tsx` (002, T027) para esse componente. Três achados de cobertura (C1/C2/C3, HIGH/MEDIUM/MEDIUM): `FR-009` (sem TEF no mobile), `FR-008`/`FR-010` (sem importação de DAV/recuperação de NFCe/telas de gestão no mobile) e `SC-001` (nenhuma regra de negócio duplicada) tinham implementação sem teste automatizado correspondente — corrigido com 3 novos testes de integração (T019, T027, T028). Dois achados de documentação (I2/I3, LOW): o contrato `layout-domain-api.md` rotulava `obterPlataforma()` (não é hook) dentro da seção "Hook" — corrigido; e `tasks.md` citava o código de domínio `MOB-05` (de `.specs/features/`) de forma isolada e inconsistente com o resto do arquivo, que só usa `FR-*`/`SC-*` do `spec.md` do Spec Kit — removida a citação solta. `tasks.md` da feature 007 passa de 27 para 31 tarefas. Anteriormente: **2026-08-31 (AD-116) — achado de `/speckit-tasks` da feature 007:** `specs/008-pagamento-geral/contracts/pagamento-domain-api.md` já declarava que a feature 007 fornece `plataforma` dentro de `capacidades()` (dependência injetada no `pagamentoSlice`, chamada como função plana fora de React), mas o contrato da 007 só expunha `useIsMobile()` (hook React, inutilizável nesse ponto). Resolvido acrescentando `obterPlataforma()` — função sem estado que reaproveita `classificarLayout` lendo `window.innerWidth` — ao design da 007, sem introduzir store nova. `tasks.md` da feature 007 gerado (27 tarefas, 6 fases, 3 user stories P2/P2/P3); duas dependências (012 `CampoVendedorVenda.tsx`, 008 painel de pagamento) ainda sem `tasks.md` entram por composição de ponto reservado, não bloqueiam o fechamento. Próximo passo sugerido: `/speckit-tasks` em 008 ou 012. Anteriormente: **2026-08-31 (AD-109 a AD-113) — nasce a feature 014, validação prévia da venda no ERP:** decisão direta do usuário cria um **gate de inserção de pagamento**. Toda inserção de forma/condição — em qualquer caminho (tela de pagamento, atalho F6–F9 da feature 013, ou qualquer fluxo futuro) e em **toda e cada** inserção da mesma venda, inclusive a segunda e as seguintes de um pagamento dividido — passa antes por `ValidarNFCe`, que devolve `Valido` + `messages`. `false` bloqueia a inserção e o motivo aparece em notificação de erro; `true` efetiva a inserção e eventuais mensagens viram avisos que não bloqueiam. A pergunta do usuário "fase própria ou emenda à 008?" foi resolvida como **fase própria** (AD-109): o insumo é o `CheckoutFaturarNFCe` completo (escopo da 004), o gatilho não é exclusivo da tela de pagamento (013) e o consumidor do veredito é a finalização (004) — mesmo padrão transversal da feature 001. Fases **Specify** e **Design** concluídas no mesmo dia, artefatos em `specs/014-validacao-previa-nfce/`. Quatro achados vieram da leitura direta do código-fonte de `PCheckout_ValidarNFCe` na KB `CentriumDEVU6`: só `Valido` decide bloqueio, porque há `Warning` que bloqueia e `Warning` que não (AD-110); o retrato precisa conter a forma **candidata** e o campo `FormaEntrada`, sem os quais o ERP calcula crediário zero e aprova o que deveria barrar (AD-111, que também unifica o montador do payload entre validação e emissão); falha de comunicação é recusa (*fail-closed*, AD-112); e não há revalidação na finalização, valendo o veredito vigente, em troca de congelar carrinho, cliente, vendedor e desconto de capa enquanto houver pagamento aplicado (AD-113). O endpoint **não** valida nada tributário — isso segue exclusivo da emissão. **Item 36 de `PENDENCIES.md` fechado por completo** (AD-109 dá destino a `ValidarNFCe`, AD-111 resolve `FormaEntrada`, `FormaFpgUtiCar` já era sem ação). Emendas aplicadas nas features 004, 008, 009, 013 e 001. Próximo passo sugerido: `/speckit-tasks` na feature 014, ou nas features 008/004 já emendadas. Anteriormente: **2026-08-31 (AD-108) — o cliente default deixa de ser um snapshot parcial:** decisão direta do usuário fecha o **item 31** de `PENDENCIES.md` e a sub-pendência `ListaPrecoDefault` do **item 36**. Quando a venda corre com o cliente default (`origem = 'DEFAULT'`, AD-032), o Checkout **não** chama `GetCliente`, usa `SessaoUsuario.ListaPrecoDefault` como lista de preço (parâmetro `Listapreco` de `GetProduto` quando `TipoPreco = 9`) e trata esse cliente como **sem desconto de convênio** (`descontoConvenio = 0`). `ClienteVenda` de origem `DEFAULT` deixa de ter `listaPreco`/`descontoConvenio` em `null` — só `documento` continua `null` (AD-100 segue válida). AD-094 fica **superada no ponto** e a feature 005 perde sua única limitação conhecida; artefatos de 003, 005 e 013 atualizados. Próximo passo sugerido: `/speckit-tasks` na feature 005 ou 003. Anteriormente: Fases **Specify** (`/speckit-specify`) e **Design** (`/speckit-plan`) da feature nova `013-venda-rapida-cenario-pagamento` concluídas — artefatos em `specs/013-venda-rapida-cenario-pagamento/` (`spec.md`, `plan.md`, `research.md`, `data-model.md`, `contracts/erp-cenario-pagamento-api.md`, `contracts/venda-rapida-domain-api.md`, `quickstart.md`, `checklists/requirements.md`), worktree/branch `worktree-013-venda-rapida-cenario-pagamento`. Venda rápida por teclas F6–F9: o ERP publica cenários de pagamento (condição + forma + tecla + indicador de encerramento) no campo `SessaoUsuario.CenarioPagamento` do `GetSessao`, e o Checkout os transforma em até 4 atalhos que lançam o saldo em aberto integral e, quando o cenário indicar, finalizam a venda sem confirmação. Origem e formato confirmados por inspeção direta da KB GeneXus (`PCheckout_GetSessao`, `TCenarioPagamento`, `PCenarioPagamento_RevisaTeclasAtalho`) — três achados viraram AD-104 (campo embutido na sessão, sem endpoint dedicado), AD-105 (formato delimitado definitivo — serialização estruturada inviável no ERP, mas com garantia de que os textos do cadastro não conterão `;`; item com nº de campos ≠ 7 segue descartado como defesa) e AD-106 (booleano de encerramento por conjunto fechado de literais, `false` como fail-safe, solução definitiva). Ambos fechados por decisão direta do usuário em 2026-08-31 — os itens 34 e 35 de `PENDENCIES.md` foram removidos, sem pendência remanescente com o ERP. Decisões diretas do usuário nesta sessão: valor lançado = saldo em aberto integral; finalização automática sem diálogo; cenários com TEF/PIX seguem elegíveis; atalhos ativos em qualquer momento da venda; feature restrita ao desktop. Ajustes correlatos aplicados em 002 (`contracts/session-bff-api.md`, `.specs/features/autenticacao-sessao-bootstrap/spec.md`) e 008 (`.specs/features/pagamento-geral/spec.md`). Cinco mudanças colaterais do contrato `20260827192357` ficaram registradas no item 36 de `PENDENCIES.md` para as features às quais pertencem; a remoção de `DavNum` de `CheckoutFaturarNFCe` **já foi resolvida em 2026-08-31 (AD-107) e não bloqueia a feature 006** — o ERP identifica sozinho que a NFCe faturada veio de um DAV, restando quatro mudanças abertas no item 36. Próximo passo sugerido: `/speckit-tasks` na feature 013. Anteriormente: fase **Design** (`/speckit-plan`) da feature `012-selecao-vendedor` concluída — artefatos gerados em `specs/012-selecao-vendedor/` (`plan.md`, `research.md`, `data-model.md`, `contracts/erp-vendedor-api.md`, `contracts/vendedor-domain-api.md`, `quickstart.md`), branch `docs/plan-selecao-vendedor`. Um achado de contrato levantado nesta fase, fechado por decisão de design (mesma lacuna já corrigida para cliente em AD-093): `GetListaVendedores` não tem parâmetro de status nem campo `Ativo`/`Status` na resposta, nem campo de função/cargo — filtro "Ativo" e coluna de subtítulo removidos do design (AD-103), corrigindo `.specs/features/selecao-vendedor/spec.md` e `specs/012-selecao-vendedor/spec.md` no ponto (FR-002/FR-003, VEND-03/VEND-08). Diferente do cliente (AD-094), o vendedor default não tem nenhum campo indisponível — `SessaoUsuario.VendedorCodigo`/`VendedorNome` já vêm completos do bootstrap. A action pública `vendedorSlice.trocarVendedor({ codigo, nome })`, já reservada por `specs/006-importacao-dav/contracts/importacao-domain-api.md`, é formalizada neste plano e cobre também a retomada de rascunho via `CarregarNFCe` (feature 004/011, ainda não desenhada), que só devolve `vendedorCodigo` sem nome — mesmo fallback "Vendedor #<código>" já decidido para DAV em AD-095. Próximo passo sugerido: `/speckit-tasks` na feature 012, ou `/speckit-plan` em 004/008 antes disso, para fechar a superfície de integração que este plano hoje só reserva (`podeMutarCarrinho()`, call site de `trocarVendedor` a partir de `CarregarNFCe`). Anteriormente: fase **Design** (`/speckit-plan`) da feature `007-layout-responsivo-mobile` concluída — artefatos gerados em `specs/007-layout-responsivo-mobile/` (`plan.md`, `research.md`, `data-model.md`, `contracts/layout-domain-api.md`, `quickstart.md`), branch `docs/plan-layout-responsivo-mobile`. Nenhum achado de contrato novo — a feature é presentation-only (`AppShell`/`useIsMobile` decidem entre `DesktopLayout` e `MobileWizard` de 3 etapas, ambos lendo o mesmo `vendaStore`), sem chamada de rede própria. Único ponto técnico decidido nesta fase: detecção do botão "Scanner" via `BarcodeDetector in window` **+** UA Chrome/Android (não só capacidade), para respeitar a restrição explícita de escopo já fixada em AD-086/AD-090. Dependência registrada como constraint (não pendência bloqueante): as features 004 (finalização/suspensão) e 012 (seleção de vendedor) ainda não passaram por `/speckit-plan` — este plano referencia os requisitos `FIN-*`/`VEND-*` já aprovados nos specs delas, sem redesenhar essas features. Próximo passo sugerido: `/speckit-tasks` na feature 007, ou `/speckit-plan` em 004/008/012 antes disso, para fechar a superfície de integração que 007 hoje só reserva. Anteriormente: fase **Design** (`/speckit-plan`) da feature `006-importacao-dav` concluída — artefatos em `specs/006-importacao-dav/` (`plan.md`, `research.md`, `data-model.md`, `contracts/`, `quickstart.md`). Dois achados de contrato levantados durante esta fase, ambos resolvidos por decisão de design (não pendência bloqueante): `ListaDAVs`/`GetDav` nunca retornam `VendedorNome` (só `VendedorCodigo`), corrigindo uma nota desatualizada em `.specs/features/selecao-vendedor/spec.md` (AD-095); `CheckoutFaturarNFCe.produtos` (retorno de `GetDav`, mesmo shape de `CarregarNFCe`) não traz descrição do produto, resolvido por busca best-effort via `GetProduto` só para exibição, sem tocar no preço congelado (AD-096). Anteriormente: fase **Design** da feature `005-identificacao-cadastro-cliente` concluída — artefatos em `specs/005-identificacao-cadastro-cliente/` (`plan.md`, `research.md`, `data-model.md`, `contracts/erp-cliente-api.md`, `contracts/cliente-domain-api.md`, `quickstart.md`). Dois achados de contrato levantados durante a fase: filtro "Ativo" removido do modal de cliente por falta de campo de status (AD-093, fechado por decisão direta do usuário); `GetCliente` sem forma de buscar por código, bloqueando dados completos (`ListaPreco`/`DescontoConvenio`) do cliente default para `TipoPreco=9` (AD-094, registrado como pendência bloqueante, item 31 de `PENDENCIES.md`, não impede a implementação). Próximo passo sugerido: `/speckit-tasks` na feature 005. Anteriormente: fase **Design** (`/speckit-plan`) da feature `003-carrinho-produto-precificacao` concluída — artefatos em `specs/003-carrinho-produto-precificacao/` (`plan.md`, `research.md`, `data-model.md`, `contracts/erp-produto-api.md`, `contracts/precificacao-domain-api.md`, `quickstart.md`), branch `docs/plan-carrinho-produto-precificacao`. A inspeção do `ApiCentriumOAuth.yaml` durante o Design levantou dois achados de contrato, ambos fechados por decisão direta do usuário no mesmo dia: **AD-091** (`GetProduto` é o único endpoint que resolve a linha do carrinho; `GetListaProdutos` só capta/seleciona) e **AD-092** (não existe lista de preço padrão da empresa; `TipoPreco = 9` usa sempre a lista do cliente, e `SessaoUsuario.listaPrecoPadrao` nunca existiu). Próximo passo sugerido: `/speckit-tasks` na feature 003. Antes disso, esta sessão continuava o trabalho descrito a seguir. Sessão de grilling (`mattpocock-skills:grilling`) auditando os 20 documentos de `.specs/` em busca de contradições e ambiguidades residuais. Sete decisões novas (AD-067 a AD-073) fecham: escopo de `repriceSku` sobre linha congelada de rascunho/DAV (AD-067); reclassificação do parse fino do código de barras pesável para pendência bloqueante, corrigindo AD-028 (AD-068); fechamento sem pendência nova da trilha de tier em `FaturarNFCe.produtos` e de `DavMatProdPes`, corrigindo AD-024/AD-063 (AD-069); confirmação estrutural da exclusividade pesável/editável, corrigindo AD-063 (AD-070); aritmética monetária em centavos inteiros e BFF em Fastify, fechando `STACK.md` (AD-071); método do maior resto para resto de arredondamento, corrigindo AD-039 (AD-072); e manutenção da regra de roteamento TEF com nova pendência não-bloqueante sobre "cartão avulso" (AD-073). Em seguida, mais sete decisões (AD-075 a AD-081), várias confirmadas por inspeção direta da KB real do GeneXus (`CentriumDEVU6`) via MCP: semântica de retorno + polling de 60s de `GetStatusSistema` (AD-075, itens 7/23, **corrigida por AD-080** — é `numeric` com limiar `0`/`>=1`, não `boolean`); parse completo do código de barras de balança e fórmula de quantidade, **resolve a pendência bloqueante item 29/AD-068** (AD-076); filtro de data real no modal de DAVs (AD-077, item 10); `FormaIntegracaoCartao` confirmado em `GetSessao` (AD-078, item 30); achado sobre o QR Code do PIX — a imagem já é gerada/persistida pelo ERP, só falta expor no contrato de `GerarPIX` (AD-079, item 24, corrige a suposição de baixa confiança de AD-047); e confirmação de que a equipe do ERP vai atualizar os endpoints pendentes dos itens 10 e 24 (AD-081). Em seguida, AD-082: `GetSessao` retorna `TipoImpressao` (`'E'`=direta, `'P'`=PDF), resolvendo a metade de AD-037 sobre o indicativo de mecanismo de impressão. E AD-083: usuário forneceu o `Impressao.js` real do PDV atual, resolvendo por completo a outra metade — contrato técnico do serviço de impressão local (host/porta via `CadMaqHost`, default `127.0.0.1:4545`, `POST` para a raiz, `text/plain`, corpo = `XMLImpressao` cru, sem validação de resposta) — **item 22 de `PENDENCIES.md` fechado, sem depender do ERP**. Próximo passo sugerido: fase **Design** da feature `carrinho-produto-precificacao` (ver `.specs/project/ROADMAP.md`)
...
**Decision:** ⚠️ **Superado em 2026-08-31 por AD-108 — leia lá antes de implementar.** A lacuna descrita aqui **não existe mais** e o item 31 de `.specs/project/PENDENCIES.md` está **fechado**: com o cliente default, a lista de preço vem de `SessaoUsuario.ListaPrecoDefault` (`GetSessao`) e o desconto de convênio é inexistente por regra de negócio, de modo que o Checkout nunca precisa resolver esse cliente por código nem chamar `GetCliente`. Redação original (2026-08-26): registrado como **pendência bloqueante** (item 31 de `.specs/project/PENDENCIES.md`), não resolvido naquela rodada — decisão direta do usuário foi sobre **como tratar** o achado, não sobre a lacuna em si. `GetCliente` aceitava somente `Empresa` e `CPFCNPJ` como parâmetros (confirmado em `ApiCentriumOAuth.yaml`) — não existia `CodCliente`/`CodigoCliente`. **Isso deixou de ser verdade em 2026-08-31 (AD-115): `GetCliente`/`PCheckout_GetCliente` passaram a aceitar também `CodCliente`**, para atender a um caso de uso diferente (importação de DAV, feature 006) — não reabre esta pendência, que continua fechada pelo motivo de AD-108 (cliente default nunca precisa de busca por código). `GetSessao.SessaoUsuario` devolve `ClienteDefaultCodigo`/`ClienteDefaultNome` (código + nome), sem CPF.
...
### AD-109: `ValidarNFCe` é gate de **inserção de pagamento**, não de finalização — e vira a feature própria 014 (2026-08-31)

**Decision:** Toda inserção de forma/condição de pagamento — **em qualquer caminho de acionamento e em toda e cada inserção da mesma venda**, inclusive a segunda e as seguintes de um pagamento dividido — passa antes por uma consulta a `ValidarNFCe`. A validação vive numa feature própria (`014-validacao-previa-nfce`), transversal e sem UI, não como bloco de requisitos dentro da feature 008.

**Reason:** Decisão direta do usuário (2026-08-31), que também levantou a pergunta "fase própria ou emenda à 008?". Três razões independentes sustentam a separação: (1) o insumo do endpoint é o `CheckoutFaturarNFCe` completo, cuja montagem é escopo declarado da feature 004 e explicitamente **fora** do escopo da 008; (2) o gatilho não é exclusivo da tela de pagamento — a feature 013 lança pagamento por tecla de função sem passar por ela, e pode encadear a finalização; (3) o consumidor do veredito é a finalização (feature 004). Mesmo padrão da feature 001 (auditoria, AD-061): mecanismo transversal, sem tela, consumido por várias features.

**Trade-off:** Uma feature a mais no roadmap, e a feature 008 passa a receber duas dependências injetadas. Em troca, nenhuma feature importa a 014, o gate não é contornável por um caminho novo de inserção, e o custo de integração fica concentrado em uma única action (`aplicarPagamento`).

**Impact:** Corrige o destino provisório "feature 004" que o achado colateral de `ValidarNFCe` tinha no item 36 de `.specs/project/PENDENCIES.md` — o momento correto é a inserção do pagamento, não a finalização. Cria `specs/014-validacao-previa-nfce/` (`spec.md`, `plan.md`, `research.md`, `data-model.md`, `contracts/erp-validacao-api.md`, `contracts/validacao-domain-api.md`, `quickstart.md`, `checklists/requirements.md`) e emenda as features 004, 008, 009, 013 e 001.

---

### AD-110: Só `Valido` decide bloqueio em `ValidarNFCe` — `messages[].Type` é apresentação (2026-08-31)

**Decision:** O Checkout ramifica **exclusivamente** no campo `Valido` da resposta. A severidade GeneXus das mensagens (`Type`) é usada só para escolher o estilo da notificação, nunca para decidir se a inserção é bloqueada. `Valido = false` sem nenhuma mensagem produz uma recusa genérica, nunca um bloqueio silencioso.

**Reason:** Leitura direta do código-fonte de `PCheckout_ValidarNFCe` na KB `CentriumDEVU6` (2026-08-31). O procedure produz `Warning` que **bloqueia** (limite de crédito com `EmpLimCre = 'B'`, data limite vencida, `CliAutCre = 'N'` — todos com `&Valido = false`) e `Warning` que **não bloqueia** (limite de crédito com `EmpLimCre = 'A'`, que mantém `&Valido = true`). Mapear severidade para bloqueio — padrão comum em respostas GeneXus — seria um bug garantido neste endpoint.

**Trade-off:** O Checkout perde a possibilidade de classificar mensagens por conta própria e passa a depender inteiramente do booleano. É o comportamento correto sob a Constitution III, e o custo é um teste negativo obrigatório (`Valido = false` + `Type = Warning` ⇒ recusa).

**Impact:** `specs/014-validacao-previa-nfce/` (`research.md` D4, `contracts/erp-validacao-api.md`, `data-model.md`, `quickstart.md` cenário 3), `specs/008-pagamento-geral/spec.md` (Edge Cases).

---

### AD-111: O retrato enviado inclui a forma candidata, vem de um montador compartilhado, e `FormaEntrada` é insumo obrigatório (2026-08-31)

**Decision:** Três pontos ligados:

1. A consulta descreve a venda **como ela ficaria** — formas já aplicadas **mais** a candidata —, não o estado anterior ao gesto.
2. O corpo é produzido por `montarRetratoVenda` (`src/client/domain/venda/`), generalização do `montarPayloadFaturarNFCe` da feature 004, parametrizado por operação (`'FATURAR' | 'SUSPENDER' | 'VALIDAR'`) e pela lista de pagamentos. O nome antigo deixa de existir.
3. Cada forma enviada precisa carregar `FormaEntrada` (`FpgEnt`), além de `FormaFpgUtiCar` — o que obriga a feature 008 a carregar esse campo do catálogo de `SessaoUsuario.CondicoesDePagamento[]`.

**Reason:** O código do ERP soma o crediário percorrendo `&CheckoutFaturarNFCe.FormasDePagamento` e acumulando as formas com `FormaFpgUtiCar = 'CRD'` **e** `FormaEntrada = 'N'`. Com o estado anterior ao gesto, `&TotalCrediario` seria `0` na primeira inserção e todo o bloco de limite de crédito seria pulado; sem `FormaEntrada`, o mesmo acontece em qualquer inserção. Nos dois casos o gate aprovaria exatamente o que existe para barrar. O montador único existe para impedir o pior modo de falha da feature: o ERP aprovar um retrato e a emissão enviar outro.

**Trade-off:** A feature 004 perde a exclusividade sobre a montagem do payload, e um módulo passa a ter dois consumidores. Como ele já era função pura recebendo snapshots prontos, a generalização é mecânica.

**Impact:** **Fecha a sub-pendência `FormaEntrada` do item 36** de `.specs/project/PENDENCIES.md` — e, com ela, o item inteiro (as outras duas já estavam resolvidas: `ValidarNFCe` por AD-109 e `FormaFpgUtiCar` sem ação, confirmando AD-048). Atualiza `specs/004-finalizacao-suspensao-venda/` (`plan.md`, `spec.md` `FR-015`, `contracts/faturamento-api.md`), `specs/008-pagamento-geral/` (`spec.md` `FR-022`, `plan.md`) e `specs/014-validacao-previa-nfce/`.

---

### AD-112: Falha de comunicação com `ValidarNFCe` é recusa (*fail-closed*), sem retry automático (2026-08-31)

**Decision:** Timeout, erro de rede, erro HTTP ou resposta que não passe na validação de fronteira produzem o veredito `INDISPONIVEL`, que **não** efetiva a inserção. Não há retry automático — a nova tentativa é gesto explícito do operador. A mensagem ao operador distingue "não consegui perguntar ao ERP" de "o ERP recusou". O schema Zod não dá default a `Valido`: resposta sem o campo é inválida, nunca aceite presumido.

**Reason:** Decisão direta do usuário (2026-08-31), escolhendo *fail-closed* contra a alternativa de seguir com aviso. Coerente com a Constitution III: sem resposta do ERP, o Checkout não tem autoridade para aceitar um pagamento a prazo. Um retry automático, além disso, multiplicaria consultas justamente quando o ERP está degradado.

**Trade-off:** O caixa para de operar quando o ERP não responde, em vez de acumular vendas para conciliar depois. É o comportamento desejado num documento fiscal.

**Impact:** `specs/014-validacao-previa-nfce/` (`spec.md` `FR-009`, `research.md` D5, `contracts/erp-validacao-api.md`, `quickstart.md` cenário 4).

---

### AD-113: Sem revalidação na finalização — vale o veredito vigente, e a venda fica congelada com pagamento aplicado (2026-08-31)

**Decision:** O veredito favorável obtido na última inserção aceita é a licença de finalização; `FaturarNFCe` **não** dispara nova consulta. O veredito é invalidado ao remover uma forma de pagamento (e ao trocar a condição ou limpar os pagamentos). Em contrapartida, enquanto houver **qualquer** forma de pagamento aplicada, ficam congelados o carrinho, o cliente, o vendedor e o desconto sobre o total — para alterar qualquer um deles é preciso antes remover a forma aplicada.

**Reason:** Decisão direta do usuário (2026-08-31), com a justificativa que a torna segura: não existe janela em que o veredito descreva uma venda diferente da que será emitida, porque toda alteração relevante exige remover o pagamento, e remover derruba o veredito. Evita uma ida ao ERP no momento de maior pressa do caixa.

**Trade-off:** **Amplia** o congelamento que a feature 008 já tinha. A invariante I7 daquela feature cobria apenas o carrinho (AD-030/`CART-09`); agora cliente, vendedor e desconto de capa entram junto (I12). É uma restrição real de fluxo — trocar o cliente depois de lançar um pagamento passa a exigir dois gestos — aceita deliberadamente em troca de dispensar a revalidação.

**Impact:** `specs/014-validacao-previa-nfce/` (`spec.md` `FR-013`/`FR-014`/`FR-016`, `research.md` D6, `data-model.md` I6/I7/I10), `specs/008-pagamento-geral/` (`spec.md` `FR-021`/`FR-023`, `data-model.md` I11/I12, `contracts/pagamento-domain-api.md`), `specs/004-finalizacao-suspensao-venda/` (`spec.md` `FR-014`/`FR-016`, `plan.md`, `contracts/faturamento-api.md`) e `specs/001-auditoria-acoes-operador/data-model.md` (evento 18, `VALIDACAO_VENDA_RECUSADA`; avisos não entram na trilha). Impacta também as features 003, 005 e 012, que consomem o predicado de congelamento por injeção.

---

### AD-114: `DAV_IMPORTADO` entra como tipo #20 no catálogo de auditoria da feature 001 — achado de `/speckit-tasks` da feature 006 (2026-08-31)

**Decision:** O catálogo de eventos de auditoria (`specs/001-auditoria-acoes-operador/data-model.md`) ganha o tipo #20 `DAV_IMPORTADO`, com `detalhes: { numeroDav: string, numeroNota: number, quantidadeLinhas: number, quantidadeFormasDePagamento: number }`, origem declarada como `006-importacao-dav`. É extensão puramente aditiva — nenhum dos 19 tipos existentes muda de shape ou de semântica.

**Reason:** Ao gerar `tasks.md` da feature 006, `specs/006-importacao-dav/data-model.md` (§6) e `contracts/importacao-domain-api.md` (§3, passo 7) já assumiam a existência de `registrarEventoAuditoria('DAV_IMPORTADO', ...)`, mas o catálogo fechado da feature 001 (implementada, `tasks.md` já existente) nunca listou esse tipo — lacuna introduzida quando o Design da 006 (2026-08-26) referenciou um evento que a 001 (concluída antes) não previa. `/speckit-analyze` não detecta esse tipo de gap: exige `tasks.md` já existente e só compara artefatos dentro de uma mesma feature, nunca o catálogo fechado de uma feature diferente. Resolvido por decisão direta do usuário (2026-08-31), escolhendo estender o catálogo agora em vez de gerar `tasks.md` da 006 assumindo silenciosamente um tipo inexistente.

**Trade-off:** Nenhum identificado — mesmo padrão aditivo já usado para os tipos 18–19 (`VALIDACAO_VENDA_RECUSADA`/`VENDA_RAPIDA_ACIONADA`, AD-109/AD-113), que também chegaram depois do fechamento inicial do catálogo.

**Impact:** `specs/001-auditoria-acoes-operador/data-model.md` (catálogo, tipo #20) e `contracts/auditoria-events.md` (linha de features consumidoras). `specs/006-importacao-dav/tasks.md` ganha a tarefa de wiring do call site (`auditoriaSlice.registrar('DAV_IMPORTADO', ...)` na orquestração `importarVendaExistente`). Nenhum artefato já implementado de 001 muda de comportamento — a extensão é só de catálogo/tipagem.

---

### AD-115: `GetCliente` passa a aceitar busca por `CodCliente` — desbloqueia a importação de DAV (feature 006) sem regredir a feature 005 (2026-08-31)

**Decision:** O procedure `PCheckout_GetCliente` (KB GeneXus `CentriumDEVU6`) e a rota `GetCliente` da API `APICentriumOAuth` ganham um terceiro parâmetro de entrada, `CodCliente` (`NUMERIC(6)`, mesmo tipo de `CliCod`), opcional e aditivo: `parm(IN:&Empresa, IN:&CliCgc2, IN:&CodCliente, OUT:&ClienteCheckout, Out:&Messages)`. A validação de entrada só recusa quando **ambos** `CliCgc2` (CPF/CNPJ) e `CodCliente` vêm vazios — antes bastava `CliCgc2` vazio. Quando `CodCliente` vem vazio, o procedure segue **exatamente** o mesmo `For Each Where CliCgc2 = &CliCgc2` de sempre (texto idêntico ao original, sem reescrita); quando preenchido, busca por `Where CliCod = &CodCliente`. Os dois caminhos convergem numa subrotina compartilhada (`Sub 'AssignCliente'`) para montar `&ClienteCheckout`, evitando duplicar a lógica de endereço/convênio/lista de preço. No lado Checkout, a camada de serviço da feature 005 ganha `fetchClientePorCodigo(codigo)` (`clienteQueries.ts`), e `OrigemCliente` (`specs/005-.../data-model.md`) ganha o valor aditivo `'DAV'`, consumido por `selecionarCliente(cliente, 'DAV')` na orquestração de importação da feature 006.
**Reason:** Achado bloqueante da fase Tasks da feature 006 (`/speckit-tasks`, 2026-08-31): a importação de DAV só recebe `clienteCodigo`/`clienteNome` do documento (`ListaDAVs`/`GetDav`, sem CPF/CNPJ), mas a única action pública já implementada para trocar o cliente da venda (`clienteSlice.selecionarCliente`, feature 005) exige um `ClienteCheckout` completo — que só existia via `GetCliente(CPFCNPJ=...)`, busca impossível sem documento (limitação registrada em AD-094, "não existe caminho de contrato para resolver um cliente a partir só do código"). Sem lista de preço/desconto de convênio resolvidos, um produto novo inserido depois da importação (Cenário 3 de `specs/006-.../quickstart.md`) seria precificado com um fallback arbitrário — risco direto à Constitution V (precisão monetária). Decisão direta do usuário (2026-08-31): alterar o contrato real do ERP via KB GeneXus (não contornar do lado Checkout), escolhida sobre a alternativa mais barata de aceitar preço provisório pós-importação.
**Trade-off:** A mudança é do lado do ERP, não só de spec — exige a mesma disciplina de qualquer alteração de contrato (build/compilação verificados antes de depender dela em produção; ver nota abaixo). Em troca, a importação de DAV resolve o cliente com o mesmo rigor de precificação de uma seleção manual, sem inventar um caminho alternativo de baixa fidelidade dentro do Checkout.
**Impact:** Corrige AD-094 no ponto (a frase "não existe `CodCliente`... isso continua verdadeiro" deixa de valer; a pendência do item 31 continua fechada por AD-108, motivo diferente). Atualiza `specs/005-identificacao-cadastro-cliente/` (`contracts/erp-cliente-api.md`, `data-model.md` — `OrigemCliente`/`selecionarCliente`, `tasks.md` — novas tarefas `fetchClientePorCodigo`/extensão de `OrigemCliente`) e `specs/006-importacao-dav/` (`contracts/importacao-domain-api.md`, `data-model.md`, `research.md` — corrige a suposição de `trocarCliente({codigo,nome})`/`trocarVendedor({codigo,nome})` para o caminho real via `GetCliente(CodCliente)` + `selecionarCliente`). **Pendência não fechada por esta AD:** o build/compilação completa do procedure alterado ainda não foi confirmado (só *Specify*/spec-check rodou, `0 errors/0 warnings`, sem build real) — `Fluxograma - Diagrama - Alinhamentos/APICentriumOAuth.yaml` (contrato exportado consumido pelas specs do Checkout) também não foi regenerado; ambos ficam como verificação pendente antes de considerar a mudança pronta para uso em produção.

---

### AD-116: `obterPlataforma()` — função plana para expor o layout atual fora de React, fechando a lacuna com a feature 008 — achado de `/speckit-tasks` da feature 007 (2026-08-31)

**Decision:** A feature 007 ganha `src/client/layout/obterPlataforma.ts`, exportando `obterPlataforma(): 'DESKTOP' | 'MOBILE'` — uma função pura e sem estado, `() => classificarLayout(window.innerWidth)` — ao lado de `useIsMobile()` (hook React já existente no design). Nenhuma store nova, nenhum estado adicional: o valor é lido sob demanda, no mesmo instante em que é consultado, sem cache nem assinatura de eventos.

**Reason:** Ao gerar `tasks.md` da feature 007, `specs/008-pagamento-geral/contracts/pagamento-domain-api.md` (§2, `PagamentoDeps.capacidades()`) já declarava que a plataforma (`Plataforma = 'DESKTOP' | 'MOBILE'`) é fornecida pela feature 007 — mas essa dependência é uma **função plana**, injetada na composição do `vendaStore` fora de qualquer componente React, e o único artefato que a 007 expunha para essa finalidade era `useIsMobile()`, um **hook**, inutilizável fora da árvore de componentes. `/speckit-analyze` não pega esse tipo de gap (compara artefatos dentro da mesma feature, não a superfície que uma feature já implementada em contrato — 008 — espera de uma feature ainda em design — 007). Como `classificarLayout` já usa o mesmo limiar (`< 768`) que `matchMedia('(max-width: 767.98px)')` usa para inteiros de `window.innerWidth`, bastou expor um segundo ponto de leitura sem duplicar a regra do limiar.

**Trade-off:** Nenhum identificado — é a opção estritamente mais simples entre as consideradas (store dedicada para "plataforma atual", ou fazer `capacidades()` aceitar um parâmetro): a leitura direta de `window.innerWidth` a cada chamada é barata e não precisa de sincronização, e evita introduzir estado observável só para espelhar o que o navegador já expõe de graça.

**Impact:** `specs/007-layout-responsivo-mobile/contracts/layout-domain-api.md` (nova função na seção 1), `data-model.md` (nenhuma entidade nova — `obterPlataforma` não tem estado) e `tasks.md` (T004/T005). Nenhum artefato de `specs/008-pagamento-geral/` muda — o contrato de `capacidades()` já estava correto, só faltava a 007 prover o meio de satisfazê-lo; quando a feature 008 gerar seu próprio `tasks.md`, a tarefa de composição do `vendaStore` chama `obterPlataforma()` diretamente, sem gap.
...
### AD-120: `repricarSku` recebe o desconto de convênio como quarto parâmetro opcional — recalcula sempre o desconto de convênio, nunca o manual (2026-09-02; corrigido em 2026-09-02 pela AD-122 — a redação original preservava `descontoLinha` inteiro quando o percentual era `0`, o que arrastava desconto de convênio obsoleto de um cliente anterior)

**Decision:** A assinatura real de `repricarSku` é `(linhas, codigoProduto, tipoPreco, descontoConvenioPercentual = 0)`. `LinhaCarrinho` separa o desconto em dois campos — `descontoConvenio` (derivado, ver AD-122) e `descontoManual` (digitado pelo operador em produto `'E'`) — e `repricarSku` **sempre** recalcula `descontoConvenio` a partir do percentual recebido, inclusive zerando-o quando o percentual é `0` (cliente sem convênio, AD-108). `descontoManual` nunca é tocado por `repricarSku`, em nenhum dos dois casos.

**Reason:** `contracts/precificacao-domain-api.md` especificava três parâmetros e `T028` exigia que o desconto de convênio fosse "recalculado junto com `repricarSku`". Fazer isso em duas passagens (reprecificar, depois aplicar convênio) deixaria a linha, entre as duas chamadas, com preço novo e desconto calculado sobre o preço velho — um estado intermediário incoerente que qualquer seletor lendo no meio observaria. Como o parâmetro é opcional, toda chamada de três argumentos escrita contra o contrato original continua válida: é extensão, não quebra.
...
### AD-122: `LinhaCarrinho.descontoLinha` separado em `descontoConvenio` + `descontoManual` — reverte parcialmente a AD-120 (2026-09-02)

**Decision:** `LinhaCarrinho` deixa de ter um único campo `descontoLinha` e passa a ter dois: `descontoConvenio` (derivado, escrito apenas por `repricarSku`, recalculado — inclusive zerado — a cada reprecificação conforme `descontoConvenioPercentual`) e `descontoManual` (escrito apenas pelo operador, em produto tipo `'E'`, via `EdicaoItemEditavel`/`EdicaoQuantidadeItem`; nunca tocado por `repricarSku`). `totalLinha` soma os dois campos antes de chamar `calcularTotalLinha`. Todo consumidor que lia/escrevia `descontoLinha` (`carrinhoSlice.ts`, `GridItens.tsx`, `useCarrinho.ts`, `EdicaoItemEditavel.tsx`) foi atualizado para o campo correto conforme a origem do valor.

**Reason:** Uma revisão de código (`code-review` nível high) na feature 003 encontrou que a redação original da AD-120 — "quando o percentual é `0`, o `descontoLinha` existente é preservado" — preservava **os dois tipos de desconto indistintamente**. Isso tinha duas consequências erradas: (1) um cliente com convênio ativo perdia o convênio (ex.: trocado pelo cliente default do AD-108, `descontoConvenioPercentual = 0`) e a linha continuava descontando pelo convênio do cliente anterior, porque nada zerava o valor; (2) simetricamente, um desconto manual digitado num produto `'E'` era silenciosamente sobrescrito no instante em que um cliente com convênio era selecionado, porque a próxima passagem com `percentual > 0` recalculava o campo único inteiro. Separar os dois campos elimina ambas as classes de bug: `descontoConvenio` agora reflete sempre o cliente ativo (inclusive `0`), e `descontoManual` sobrevive a qualquer troca de cliente, que é a garantia que a AD-120 original pretendia dar só para o caso manual.

**Trade-off:** Um campo a mais em `LinhaCarrinho` e em todo shape derivado (`InserirItemInput`, `AjustesDeItem`), e todo somatório de desconto da linha precisa somar os dois campos em vez de ler um só — custo linear, pago uma vez nesta correção. Em troca, a UI que exibe/edita desconto (`GridItens.tsx`) pode mostrar os dois separadamente no futuro, se um requisito pedir (hoje soma os dois para exibição).

**Impact:** `src/client/domain/precificacao/linha.ts`, `reprecificacao.ts` (ver AD-120, reescrita para refletir este comportamento); `src/client/stores/slices/carrinhoSlice.ts`, `src/client/features/carrinho/{EdicaoItemEditavel,GridItens,useCarrinho}.tsx`; testes em `tests/unit/domain/precificacao/reprecificacao.spec.ts` e `tests/integration/carrinhoSlice.spec.ts` cobrindo as duas transições (convênio→sem-convênio zera só `descontoConvenio`; desconto manual sobrevive a cliente ganhando convênio). Commit `cf42fad`, branch `feat/003-carrinho-produto-precificacao`.
...
1. **`Cancelar venda` fica desabilitado enquanto a venda não tem linha nenhuma.** Suspender uma venda vazia criaria um rascunho sem item no ERP, que o operador teria de limpar depois. ⚠️ **Esta AD exigia linha *ativa*, e isso foi corrigido em AD-140** (2026-09-03, pedido do usuário): linha **cancelada conta** — ela permanece no array por rastreabilidade (`CART-08`) e é prova de que a venda foi digitada, e travar o botão ali deixava sem saída justamente a venda com todos os itens cancelados. O piso permanece: nenhuma linha, nenhum cancelamento. A prop `bloqueado` de `BotaoCancelarVenda` passa a cobrir dois motivos decididos pelo call site — sem item, ou pagamento aprovado não removível (`FR-005`).
...
### AD-137: Achados da implementação da feature 006 (importação de DAV) — o elo `NumeroNota` que faltava, o cliente da venda que nunca chegava ao ERP, e os desvios deliberados do Pencil (2026-09-03)

**Contexto:** implementação da feature `006-importacao-dav` (`/speckit-implement`), 25 tarefas de `specs/006-importacao-dav/tasks.md`. Três achados que nenhum artefato de design previa.

**1 — `identidadeVenda` não estava no plano, e sem ela o DAV nunca fecharia no ERP.** `tasks.md` (T012–T018) descreve a orquestração de `importarVendaExistente` populando carrinho, cliente, vendedor, pagamento e auditoria — e **nada mais**. Mas `NumeroNota` só chega a `FaturarNFCe` através de `identidadeVenda.numeroNota`: é de lá que `montarRetratoVenda` (feature 004) o lê. Como `DavNum` saiu do contrato (AD-107), esse número é o **único** elo com o documento de origem. Sem gravar a identidade, toda venda importada seria faturada com `NumeroNota: 0`, como venda nova, e o DAV ficaria aberto no ERP — sem erro, sem aviso, detectável só na conferência. `importarVendaExistente` passa a chamar `definirIdentidadeVenda({ origem: 'DAV', numeroNota })`, recebida como porta em `ImportacaoVendaDeps`.

**Não é `abrirSessaoDeVenda`,** apesar do TSDoc daquela função (`src/client/stores/vendaStore.ts`) antecipar que as features 006/011 a chamariam com `origem`/`numeroNota` do documento. `abrirSessaoDeVenda` **zera** o histórico de auditoria, e importar um DAV no meio de uma venda apagaria o registro do que o operador já tinha feito (contra `FR-009` da feature 001). A trilha correta é o histórico existente seguido de `DAV_IMPORTADO`. A antecipação daquele TSDoc fica **superada neste ponto** para a 006; a 011 decidirá a sua por conta.

**2 — `FaturarNFCe` mandava sempre o cliente default do PDV, nunca o cliente da venda (bug pré-existente, da feature 005).** `useFinalizarOuSuspenderVenda.ts` montava o retrato com `clienteCodigo: sessao.ClienteDefaultCodigo` — literal, sem olhar `clienteAtual`. O arquivo é da feature 004, escrita antes de existir slice de cliente, e a 005 implementou a identificação sem voltar para religar o campo. Efeito: **toda** NFCe saía emitida para o consumidor padrão, mesmo com o operador tendo identificado outro cliente — silencioso, visível só na nota emitida. Corrigido para `venda.clienteAtual?.codigoCliente ?? sessao.ClienteDefaultCodigo`, mantendo o default como fallback (AD-032). O defeito era da **005**, não da 006; foi a exigência de `FR-007` da 006 (o cliente do documento precisa chegar ao faturamento) que o expôs, via E2E. Travado por dois testes de regressão em `tests/integration/finalizacaoSuspensao.spec.ts`.

**3 — Desvios deliberados do Pencil na janela de importação** (nó `Hao17`, "Modal Menu DAV"), todos registrados no TSDoc de `ModalImportacaoDav.tsx`, mesmo padrão de AD-093/AD-135:

- **Filtros "Status", "Vendedor", "Tipo" e "Origem" não implementados** — `ListaDAVs` não tem parâmetro correspondente (AD-024); só `Txtbusca` e o período de emissão existem. Controles que não filtram nada.
- **Colunas "Origem" e "Status" removidas** — `CheckoutListaDAVs.DAV_DAV` não tem esses campos. Exibi-los exigiria inventar o estado do documento.
- **Botão do rodapé é "Importar DAV", não "Faturar DAV"** — confirmar **não** fatura: popula o carrinho, e a venda segue o fluxo normal de pagamento e finalização (`FR-008`). O rótulo do desenho prometeria uma emissão que o clique não faz.
- **Subtítulo perdeu "Reimprima DAVs"** — reimpressão é proibida neste fluxo (`FR-009`, AD-035).
- **O "Modal menu importação" (nó `yg9zq`) não foi construído** — é um seletor entre "Importar DAV" (esta feature) e "Importar NFCe" (feature 011, inexistente). Hoje obrigaria o operador a escolher numa lista de uma opção só, com a outra morta. O atalho "Menu Importação" abre a janela de DAV direto; quando a 011 existir, o seletor entra entre os dois sem alterar `ModalImportacaoDav`.

**Outras decisões de implementação:** o desconto do documento entra em `LinhaCarrinho.descontoManual`, não em `descontoConvenio` — este último é campo derivado que `repricarSku` reescreve, e seria apagado na primeira reprecificação caso o operador editasse a linha e a descongelasse (invariante I6 da 003). Toda a rede (`GetDav` + `GetCliente`) acontece **antes** da primeira mutação, invertendo a ordem que `contracts/importacao-domain-api.md` §3 numera — a numeração descreve o resultado, não a janela de falha, e assim D7 (`FR-010`) vale ao pé da letra: falha nenhuma deixa meio documento no carrinho. Falha ao resolver o cliente **aborta** a importação inteira, em vez de seguir com o cliente anterior, porque importar itens sob o cliente errado produziria NFCe com preço e destinatário divergentes do documento.

**Ressalva a `contracts/importacao-domain-api.md`:** o contrato promete reuso "sem alteração" pela feature 011. Vale para `mapearVendaExistente`; **não** vale para `paraLinhaCarrinho`, que fixa `origem: 'DAV'` e precisará de um parâmetro quando a 011 usar `'RASCUNHO'`.

**Impact:** novos — `src/shared/schemas/dav.schema.ts`, `src/client/domain/importacaoVenda/mapearVendaExistente.ts`, `src/client/services/dav/davQueries.ts`, `src/client/features/dav/{useImportacaoDav.ts,ModalImportacaoDav.tsx,BotaoMenuImportacao.tsx}`. Alterados — `src/client/stores/slices/carrinhoSlice.ts` (duas actions aditivas: `importarLinhasCongeladas`, `editarSnapshotDescricao`), `src/client/features/finalizacao-suspensao/{AcoesFinaisVenda.tsx,useFinalizarOuSuspenderVenda.ts}`. Testes — `tests/unit/domain/importacaoVenda/mapearVendaExistente.spec.ts` (16), `tests/integration/importacaoDav.spec.ts` (15), `tests/e2e/importacao-dav.spec.ts` (13), `tests/support/dav.ts`, `tests/e2e/support/erp-mock.ts` (rotas `ListaDAVs`/`GetDav`), `tests/integration/finalizacaoSuspensao.spec.ts` (+2 de regressão). 436 unitários/integração e 98 E2E verdes, `tsc --noEmit` e `eslint` limpos.

**Complementa:** **AD-095** (vendedor sem nome), **AD-096** (descrição best-effort), **AD-107** (`DavNum` fora do contrato), **AD-114** (`DAV_IMPORTADO`), **AD-115** (`GetCliente` por `CodCliente`).

### AD-138: Um documento importado substitui a venda, nunca se soma a ela — a importação é recusada com notificação quando a venda já está em digitação (2026-09-03)

**Decisão direta do usuário**, durante a implementação da feature 006: *"Um DAV/NFCe não pode ser importado para dentro de uma venda que já tem cliente/item identificado, se tentar fazer isso tem que informar e dar erro com notification."* Vira `FR-011` em `specs/006-importacao-dav/spec.md`.

**Por que a regra é forçada pelo contrato, não uma preferência de UX.** Um documento importado traz itens, cliente, vendedor e formas de pagamento já registrados no ERP, e passa a ser a NFCe rascunho daquela venda. Não existe "mesclar": `FaturarNFCe` carrega **um** `NumeroNota` (`montarRetratoVenda.ts`) e **um** `clienteCodigo`. Importar sobre uma venda em digitação misturaria dois documentos num só, e o operador só descobriria na nota emitida.

**A regra nasceu de um achado de revisão (`ecc:code-reviewer`, Sonnet), não de uma suposição.** A primeira implementação permitia importar um segundo DAV sobre o primeiro. `definirIdentidadeVenda` **substitui** `identidadeVenda` incondicionalmente, enquanto `importarLinhasCongeladas` apenas **soma** as linhas: importar A e depois B deixava o carrinho com os itens de A **e** B, e `FaturarNFCe` saía com o `NumeroNota` de B. O ERP fechava o DAV B; **o DAV A ficava aberto para sempre, com os itens dele já faturados sob outro número** — sem erro, sem aviso, detectável só na conferência fiscal. A mesma revisão apontou uma segunda lacuna, latente: `definirIdentidadeVenda` não é barrada por `podeMutarCarrinho()` (diferente de `importarLinhasCongeladas` e `selecionarCliente`), então, quando a feature 008 ligar o lock real de pagamento, uma importação com pagamento aprovado gravaria o `NumeroNota` do documento enquanto carrinho e cliente recusariam a mudança — a venda passaria a apontar para o rascunho de outro documento mantendo o próprio conteúdo. **Esta AD fechou essa lacuna apenas na borda da feature 006** (a checagem dentro de `importarVendaExistente`); o slice em si continuou sem guarda até **AD-139**, que a moveu para dentro de `identidadeVendaSlice` e passou a cobrir todo call site.

**Forma da solução:** uma regra pura, `recusaDeImportacao(EstadoVendaParaImportacao) → MotivoRecusaImportacao | null` (`src/client/services/dav/davQueries.ts`), com quatro motivos — `venda-bloqueada`, `ja-importou-documento`, `carrinho-populado`, `cliente-identificado` — e um texto por motivo em `mensagemDeRecusa`. **Dois pontos de aplicação, uma regra só:**

1. **No clique do atalho** (`BotaoMenuImportacao.tsx`): a janela nem abre, e o motivo aparece em notificação de erro. O operador precisa saber antes de escolher um documento, não depois de confirmar. O botão continua **habilitado** de propósito — desabilitado ele não explicaria nada, e é justamente o motivo que diz ao operador que a saída é cancelar a venda.
2. **Dentro de `importarVendaExistente`**, antes até da chamada de rede — é o que fecha qualquer call site futuro que não passe pelo botão, e evita gastar um `GetDav` para depois não ter nada a desfazer.

**O cliente default não conta como "cliente identificado".** A distinção é `clienteSlice.houveEscolhaExplicita` **em conjunção com `clienteAtual !== null`** (a flag sozinha estava errada — corrigido em **AD-139**, porque `limparCliente()` zera o cliente sem zerar a flag): a tela de venda nasce com o default aplicado (AD-032), então considerá-lo impediria toda importação. ⚠️ **Esta AD contava "itens" só como linhas ativas (`linhasAtivas`), e isso foi corrigido em AD-141** (2026-09-03, pedido do usuário): a recusa conta **todas** as linhas, canceladas inclusive (`linhasNaVenda`) — a pergunta é "esta venda já foi digitada?", não "há o que faturar?". Desde AD-141 a regra volta a coincidir com a de `BotaoCancelarVenda` (AD-140), que também passou a contar qualquer linha, embora as duas respondam a perguntas distintas ("o documento entraria sobre conteúdo já lançado?" e "há venda digitada a suspender?").

**Consequência no teste da 006:** o cenário de faixa de quantidade (`tests/integration/importacaoDav.spec.ts`, T020) inseria um item manual e **depois** importava — ordem agora proibida. Foi reescrito na ordem que o operador de fato tem (`FR-008`): importa primeiro, insere o item manual depois. A asserção não mudou: a linha congelada continua fora do agregado por SKU (invariante I3, AD-067).

**Corrige, no ponto:** o TSDoc de `definirIdentidadeVenda` em `ImportacaoVendaDeps`, que descrevia a gravação da identidade como incondicional. **Complementa:** **AD-137** (implementação da 006), **AD-030**/**AD-043** (predicado único de "a venda pode ser mutada"), **AD-032** (cliente default).

**Impact:** `src/client/services/dav/davQueries.ts` (regra pura, erro, porta `estadoDaVenda`), `src/client/features/dav/{useImportacaoDav.ts,BotaoMenuImportacao.tsx}` (`recusaAtual`, recusa no clique), `specs/006-importacao-dav/spec.md` (`FR-011`). Testes: `tests/integration/importacaoDav.spec.ts` (+9, incluindo a segunda importação recusada) e `tests/e2e/importacao-dav.spec.ts` (+3). 445 unitários/integração e 101 E2E verdes.

**Achado de baixa severidade da mesma revisão, também corrigido:** o `useCallback` de `useImportacaoDav` nunca memoizava — `sobrescritas` é um objeto novo a cada render, e estava no array de dependências. Removido, já que nada depende da identidade referencial de `importar`.

### AD-139: A guarda pós-pagamento da identidade da venda mora no slice, e o início de sessão é uma ação separada da mudança de identidade (2026-09-03)

**Revisão da própria etapa de correção da AD-138.** Três defeitos, um deles a lacuna que a AD-138 declarou mas não fechou.

**1. A guarda `podeMutarCarrinho()` estava só na borda da 006, não no slice.** `importarVendaExistente` checava `deps.estadoDaVenda()` antes de chamar `definirIdentidadeVenda`, mas `identidadeVendaSlice` continuava sem `deps` — qualquer outro call site (a retomada de rascunho da 011, um hook futuro, um teste) gravaria a identidade da venda com pagamento aprovado. `carrinhoSlice` e `clienteSlice` já recebiam o predicado; a identidade era o único dos três estados de venda mutáveis sem ele, o que contraria AD-043 (uma fonte de verdade só para "a venda ainda pode ser mutada").

**A armadilha que a guarda ingênua criaria, e por que a solução é dividir a ação em duas.** `abrirSessaoDeVenda` (`vendaStore.ts`) chama `definirIdentidadeVenda` em dois momentos: ao abrir a tela de venda **e** logo depois de `FaturarNFCe` retornar sucesso (`useFinalizarOuSuspenderVenda.ts`, caso `'sucesso'`: `encerrarVenda()` → `descartarAuditoria()` → `resetarIdentidadeVenda()` → `abrirSessaoDeVenda('NOVA')`). Esse segundo ponto roda com o pagamento **ainda aprovado** no estado — a 008 é quem limpará isso, e hoje nada quebra só porque `podeMutarCarrinho` é o stub `() => true` de `carrinhoDepsPadrao`. Guardar a ação única transformaria a abertura da venda seguinte num no-op silencioso no dia em que a 008 ligasse o predicado real: bomba-relógio, exatamente o que não se quer deixar para outra feature desarmar.

A solução não inventa padrão novo — aplica o que a base já usa. As ações de **limpeza e de início** de venda já são deliberadamente não guardadas: `limparCarrinho` (003), `descartarAuditoria`/`resetarAuditoria` (001) e `inicializarClientePadrao` (005) não passam por `podeMutarCarrinho()`, porque a limpeza de fim de venda nunca pode ser bloqueada pelo estado que ela existe para limpar. O que é guardado é a **mutação no meio da venda** (`inserirItem`, `selecionarCliente`). A identidade da venda ganhou as duas faces separadas:

- `iniciarIdentidadeVenda(identidade)` — **não guardada**, chamada por `abrirSessaoDeVenda` e só por ela, no mesmo ponto de `resetarAuditoria` (invariante D1 da 004).
- `definirIdentidadeVenda(identidade)` — **guardada**, no-op com aviso (nunca exceção, igual a `inserirItem`/`selecionarCliente`), usada pela importação de DAV (006) e pela futura retomada de rascunho (011).
- `resetarIdentidadeVenda()` — **não guardada**, pelo mesmo motivo de `limparCarrinho`.

A validação de `NumeroNota` (`ErroIdentidadeVendaInvalida`) roda **antes** da guarda nas duas: um número fracionário ou negativo é defeito de contrato, não estado legítimo da venda, e engoli-lo como no-op esconderia o bug onde ele mais importa (Constitution IV).

**2. `houveEscolhaExplicita` sozinho não é proxy de "a venda tem cliente".** `limparCliente()` (recusa de pessoa jurídica, AD-133) zera `clienteAtual` **sem** mexer na flag — de propósito, para que a próxima identificação válida ainda conte como primeira escolha (D9 da 005). Cenário concreto: o operador identifica um cliente pessoa física, depois digita o **código** de um cliente pessoa jurídica; `GetCliente` devolve o cadastro, a guarda de `useCliente` recusa e a venda fica **sem cliente nenhum** — e o atalho de importação passava a recusar com "Esta venda já tem um cliente identificado" apontando para um campo de cliente vazio, sem saída possível a não ser cancelar a venda inteira. `estadoDaVendaAtual` agora responde `houveEscolhaExplicita && clienteAtual !== null`, que é o que a regra sempre quis perguntar.

**3. A pré-condição era lida uma vez, antes de dois `await`.** Entre `recusaDeImportacao(deps.estadoDaVenda())` e a primeira mutação há `GetDav` e `GetCliente`, com a venda viva atrás da janela. Com a guarda do item 1 no lugar, um estado que virasse nesse intervalo — o caso real é a 008 aprovando um TEF de forma assíncrona — faria cada mutação virar no-op na guarda do seu próprio slice **enquanto `DAV_IMPORTADO` era registrado e a janela fechava como sucesso**: trilha de auditoria afirmando uma importação que não aconteceu, com o carrinho vazio. `importarVendaExistente` reverifica agora depois da rede, colado nas mutações, o que torna verdadeira a promessa de atomicidade que o próprio TSDoc já fazia. `estadoDaVenda` passa a ser documentada como "chamada duas vezes, precisa ler o estado a cada chamada".

**Três suspeitas da revisão que não se confirmaram, e ficam registradas para não serem reabertas:** (a) a **ordem** dos quatro motivos de `recusaDeImportacao` está correta — em todo estado com dois motivos válidos o escolhido é o mais informativo (`venda-bloqueada` explica melhor que `carrinho-populado`; `ja-importou-documento` explica melhor que "já tem itens", já que os itens **são** do documento); (b) `BotaoMenuImportacao` e `ModalImportacaoDav` chamarem `useImportacaoDav` cada um não custa nem diverge — o hook não usa nenhum hook do React, não guarda estado e devolve duas closures que leem o store no momento da chamada; (c) o `disabled={davSelecionado === null || importando}` do botão "Importar DAV" continua necessário: ele cobre "nenhuma linha selecionada" e o duplo clique, coisas que a recusa no atalho não cobre.

**Corrige, no ponto:** **AD-138** (dois parágrafos: o que a lacuna latente deixou aberto, e a definição de "cliente identificado"). **Complementa:** **AD-043**/**AD-030** (predicado único de mutabilidade), **AD-133** (`limparCliente`), **AD-137** (implementação da 006).

**Impact:** `src/client/stores/slices/identidadeVendaSlice.ts` (`IdentidadeVendaDeps`, `criarIdentidadeVendaSlice(deps)`, `iniciarIdentidadeVenda`), `src/client/stores/vendaStore.ts` (`identidadeVendaDepsPadrao`, 3º parâmetro de `criarVendaStore`, `abrirSessaoDeVenda`), `src/client/features/dav/useImportacaoDav.ts` (`clienteIdentificado`), `src/client/services/dav/davQueries.ts` (reverificação pós-rede). Testes: `tests/integration/identidadeVendaSlice.spec.ts` (novo, 7), `tests/integration/importacaoDav.spec.ts` (+4) e `tests/e2e/importacao-dav.spec.ts` (+1). 456 unitários/integração e 102 E2E verdes; `tsc --noEmit` e `eslint` limpos.

### AD-140: Cinco correções de uso na importação de DAV e na saída da venda (2026-09-03)

**Cinco pedidos diretos do usuário sobre a feature 006 já implementada, aplicados sem reabrir o ciclo de Spec Kit** (mesmo critério de AD-124/AD-136: correção pontual sobre feature entregue vai a código, testes e AD).

**1. ESC fecha a janela de importação.** O `onKeyDown` no backdrop só disparava com o foco **dentro** do modal — um clique no fundo, ou o calendário de um filtro devolvendo o foco ao `body`, e a tecla não fazia nada. Agora é ouvinte de `window` enquanto a janela está aberta, o mesmo padrão de `DialogoDocumentoFiscal` (004). Nada foi importado nesse ponto, então sair é sempre seguro.

**2. O período de emissão vira dois campos com calendário próprio, já preenchidos.** O `<input type="date">` nativo só abre o calendário pelo ícone do navegador: clicar no meio do campo não fazia nada. `src/client/components/ui/campo-data.tsx` (novo) é um campo que abre o calendário a **qualquer** clique ou foco, aceita digitação em `DD/MM/AAAA` e devolve `YYYY-MM-DD`, que é o formato de `Datainicial`/`Datafinal`. Construído à mão, sem dependência nova — a base não tem `react-day-picker` nem biblioteca de data, e instalar uma para desenhar um mês ampliaria a superfície de supply-chain (mesma cautela das libs de UI). A aritmética usa os componentes **locais** de `Date`: `toISOString()` deslocaria o dia para todo o Brasil.

O período nasce aplicado: início em **hoje − 7 dias**, fim em **hoje**, recalculado a cada abertura da janela (o Checkout fica aberto o turno inteiro; uma janela montada ontem ofereceria o período de ontem). O teto é o **dia**, não um instante — `Datainicial`/`Datafinal` são `format: date` no contrato, então "hoje" já cobre até as 23:59, e não há horário a enviar nem a exibir.

**3. Enter importa o documento selecionado.** De qualquer ponto da janela, Enter equivale a "Importar DAV". Sobre a **linha** a tecla é resolvida pela própria linha, que interrompe a propagação: no teclado o Enter chega à linha que tem o foco, que pode não ser a selecionada — o primeiro Enter escolhe a linha, o segundo importa. Sem essa separação, navegar com Tab e teclar Enter importaria o documento **anterior**.

**4. "Menu Importação" fica desabilitado com a venda já iniciada.** A recusa da AD-138 continua valendo em todos os seus pontos de aplicação; muda a superfície: em vez de um botão clicável que responde com notificação de erro, o atalho é `disabled` e o motivo vai no `title`. Para isso `useImportacaoDav` passou a expor `recusa` **reativa** — `useVendaStore((venda) => recusaDeImportacao(estadoParaImportacao(venda)))`, seletor que devolve `string | null` (um retrato novo a cada render provocaria re-render infinito na comparação padrão do Zustand). Sem subscrição, nada re-renderizaria o botão quando o primeiro item entra no carrinho e ele continuaria clicável. `recusaAtual()` permanece para a checagem no gesto, e `importarVendaExistente` segue reaplicando a regra.

**5. Linha cancelada volta a permitir cancelar a venda.** A regra anterior (**AD-126**, 2026-09-02) exigia linha **ativa** para habilitar "Cancelar venda", e deixava sem saída exatamente a venda que mais precisa dela: todos os itens cancelados, nada a finalizar e nada a cancelar. `useVendaTemItem` passa a olhar `linhas.length > 0` — a linha cancelada permanece no array por rastreabilidade (`CART-08`) e é prova de que a venda foi digitada. O piso continua existindo: venda sem nenhuma linha não vira rascunho vazio no ERP.

**Efeito colateral nos testes:** as emissões dos DAVs sintéticos do mock E2E deixaram de ser datas fixas de 2026 e passaram a ser **relativas** a hoje (`emissaoRelativa`, ontem e −4 dias). Com o filtro padrão de 7 dias, datas fixas sairiam da lista sozinhas com a passagem do tempo e a suíte quebraria sem ninguém tocar em código.

**Corrige, no ponto:** **AD-126** (que exigia linha ativa para cancelar a venda). **Complementa:** **AD-137** (implementação da 006), **AD-139** (a recusa e seus pontos de aplicação). ⚠️ **Seguida por AD-141** no mesmo dia: a recusa de importação também passou a contar linha cancelada, então as duas regras voltaram a coincidir na contagem, ainda que respondam a perguntas distintas.

**Impact:** `src/client/components/ui/campo-data.tsx` (novo — `CampoData`, `isoLocal`, `isoRelativoAHoje`), `src/client/features/dav/ModalImportacaoDav.tsx`, `src/client/features/dav/BotaoMenuImportacao.tsx`, `src/client/features/dav/useImportacaoDav.ts` (`ApiImportacaoDav.recusa`), `src/client/features/finalizacao-suspensao/AcoesFinaisVenda.tsx` (`useVendaTemItem`). Testes: `tests/integration/finalizacaoSuspensao.spec.ts` (caso da linha cancelada invertido), `tests/e2e/importacao-dav.spec.ts` (+4 casos: período padrão, calendário, ESC, Enter; três casos de recusa passam a afirmar `disabled` + `title`) e `tests/e2e/support/erp-mock.ts` (emissões relativas). 456 unitários/integração e 106 E2E verdes; `tsc --noEmit` e `eslint` limpos.

### AD-141: A recusa de importação conta linha cancelada — a pergunta é "esta venda já foi digitada?", não "há o que faturar?" (2026-09-03)

**Pedido direto do usuário, logo depois de AD-140:** o "Menu Importação" também não pode ficar habilitado quando há item no carrinho **mesmo cancelado**. `EstadoVendaParaImportacao.itensAtivos` (linhas não canceladas) vira `linhasNaVenda` (todas), e `recusaDeImportacao` responde `carrinho-populado` para qualquer linha no array.

**Por que a contagem anterior estava errada.** A linha cancelada não sai do carrinho: ela permanece no array por rastreabilidade (`CART-08`), aparece riscada na grid, entra no `Log` de auditoria e vai no retrato de `FaturarNFCe`. Importar um documento por cima dela juntaria, dentro de uma nota só, o que o operador lançou e cancelou com o conteúdo do documento — o mesmo tipo de mistura que a recusa existe para impedir (AD-138). "Nada a faturar" não é o mesmo que "nada foi digitado", e era a primeira leitura que a regra estava fazendo.

**As duas regras da venda voltam a contar igual, sem virarem a mesma regra.** Depois de AD-140, `BotaoCancelarVenda` já contava qualquer linha; agora a recusa de importação também. A coincidência é consequência, não acoplamento: continuam sendo perguntas distintas ("o documento entraria sobre conteúdo já lançado?" e "há venda digitada a suspender?"), cada uma no seu ponto — e é justamente essa combinação que dá saída ao operador, porque a venda com todos os itens cancelados tem o atalho de importação fechado **e** o de cancelar a venda aberto.

**A mensagem mudou junto:** "Esta venda já tem itens lançados, **mesmo que cancelados**. Cancele a venda para importar um documento." Sem o "mesmo que cancelados", o operador que acabou de cancelar a última linha leria um motivo que não bate com o carrinho que ele está vendo.

**Corrige, no ponto:** **AD-138** (contagem de "itens" na recusa). **Complementa:** **AD-140**.

**Impact:** `src/client/services/dav/davQueries.ts` (`EstadoVendaParaImportacao.linhasNaVenda`, `recusaDeImportacao`, `mensagemDeRecusa`), `src/client/features/dav/useImportacaoDav.ts` (retrato da venda). Testes: `tests/integration/importacaoDav.spec.ts` (+1 caso com a store real, fixtures renomeadas) e `tests/e2e/importacao-dav.spec.ts` (+1 caso: cancelar a única linha mantém o atalho desabilitado e o "Cancelar venda" habilitado). 457 unitários/integração verdes; `tsc --noEmit` limpo.

### AD-142: O atalho bloqueado é `aria-disabled`, não `disabled` — clicar precisa explicar o motivo (2026-09-03)

**Pedido direto do usuário:** clicar no "Menu Importação" bloqueado tem de informar o motivo. O `disabled` nativo introduzido em AD-140 tornou isso impossível — o navegador não dispara evento nenhum em botão desabilitado, então `onClick` nunca rodava e a notificação com o motivo **só aparecia quando a importação era possível**, isto é, quando ela não era necessária. O `title` cobria o mouse parado sobre o botão, e nada mais: quem clica (o gesto natural) não recebia resposta alguma.

**A troca é `disabled` → `aria-disabled`.** O botão continua clicável e focável, com a mesma aparência apagada e `cursor-not-allowed`; leitores de tela seguem anunciando o controle como desabilitado, e `expect(...).toBeDisabled()` do Playwright continua valendo — ele reconhece `aria-disabled`. Quem barra a abertura é o `abrir()`, que já reaplicava a recusa e emite a notificação; o `title` permanece para o hover. O padrão vale só para este atalho: `BotaoCancelarVenda` continua com `disabled` nativo, porque lá não há motivo a explicar que o operador não veja na própria tela.

**Efeito colateral nos testes:** a checagem de *actionability* do Playwright recusa clicar em elemento com `aria-disabled`, mesmo o que o navegador clica sem problema — os dois casos que exercitam o clique bloqueado usam `click({ force: true })`, com a limitação anotada no próprio teste.

**Corrige, no ponto:** **AD-140** (item 4 — o bloqueio do atalho). **Complementa:** **AD-141** (o que a mensagem diz).

**Impact:** `src/client/features/dav/BotaoMenuImportacao.tsx`. Testes: `tests/e2e/importacao-dav.spec.ts` (dois casos passam a afirmar a notificação no clique bloqueado). 457 unitários/integração e 22 E2E de importação verdes; `tsc --noEmit` e `eslint` limpos. **Generalizada em AD-143**, que faz deste o padrão de todo botão bloqueável da base.

### AD-143: Bloqueio explicativo vira padrão da base — `lib/bloqueio.ts` (2026-09-03)

**Pedido direto do usuário:** aplicar o mesmo tratamento da AD-142 a "Cancelar venda", ao botão de inserir item e a "Identificar" cliente, e **fixar a regra para todo botão que desabilite em algum momento**.

**O padrão, em um arquivo.** `src/client/lib/bloqueio.ts` define `MotivoBloqueio = string | null` e dois helpers: `atributosDeBloqueio(motivo)` (espalha `aria-disabled` + `title`) e `acaoBloqueavel(motivo, acao)` (embrulha o handler: bloqueado, notifica o motivo e não executa). `components/ui/button.tsx` ganhou `aria-disabled:opacity-50 aria-disabled:cursor-not-allowed`, para o estado apagado valer em qualquer `Button` sem repetir classe.

**O motivo é texto, não booleano.** `BotaoCancelarVenda` deixou de receber `enviando`/`bloqueado: boolean` e passou a receber `bloqueado: MotivoBloqueio` — a frase que o operador lê. Quem decide é o call site, que é quem conhece a regra: `motivoDeBloqueioDoCancelar` em `AcoesFinaisVenda.tsx` responde "Aguarde: esta venda ainda está sendo enviada ao ERP." ou "Não há nada a cancelar: nenhum item foi lançado nesta venda.", nessa ordem — dizer "não há itens" a quem está esperando o ERP seria falso. Nos outros dois, o texto espelha **termo a termo** a condição que já bloqueava: os três casos de `podeConfirmar` na entrada rápida (consulta em andamento, código não digitado, valor inválido na prévia) e os dois da guarda de `identificar()` (consulta em andamento, documento vazio). Sem esse espelho, o texto poderia dizer uma coisa e o bloqueio responder a outra.

**Fora do escopo desta AD:** "Finalizar venda" e os botões de paginação dos modais continuam com `disabled` nativo — o usuário nomeou três botões, e a feature 008 ainda vai reescrever a condição de finalizar. O padrão vale para eles quando forem mexidos.

**Dois gotchas de teste, que valem para qualquer botão convertido daqui em diante:** (1) `toBeDisabled()` do **Playwright** reconhece `aria-disabled`, mas o do **jest-dom** não — em Vitest a asserção é `toHaveAttribute('aria-disabled', 'true')`. (2) A checagem de *actionability* do Playwright recusa clicar em `aria-disabled`: os casos usam `click({ force: true })`, e o de "Identificar" usa `dispatchEvent('click')` porque o botão vive dentro do card de cliente, que abre com transição de altura — um clique por coordenada cai no container recortado e o teste fica intermitente (passava isolado, falhava na suíte).
...
### AD-146: Cinco desvios decididos na implementação da feature 008 (2026-09-03)

A feature 008 (pagamento geral) foi implementada — 44 tarefas, 596 testes unitários/integração verdes, `tsc --noEmit` e `eslint` limpos. Cinco decisões foram tomadas **durante** a implementação, por o artefato de design divergir do que o código ou o desenho exigiam. Ficam registradas aqui porque nenhuma delas é derivável de `tasks.md`.

**(1) O desconto de capa é inline no cartão, não um modal.** `tasks.md` T035 pedia `ModalDescontoCapa.tsx`. O Pencil **não tem** modal de desconto de capa: o desenho põe o bloco dentro do cartão "Pagamento e totais" (nó `Jup0R`, "Desconto e acréscimo" — combobox de tipo + campo de valor + equivalente financeiro em `$warning`). Como o design é a fonte de verdade visual do projeto (CLAUDE.md § "Referência visual (design)"), o componente entregue é `ControleDescontoCapa.tsx`, inline. ~~O combobox do desenho oferece "Acréscimo", que nenhum `FR-xxx` implementa: a opção aparece **bloqueada com motivo**, porque suprimi-la esconderia o que o próprio rótulo do bloco anuncia.~~ **SUPERADO por AD-147 (2026-09-04):** o combobox de tipo de ajuste foi **removido** — não existe campo de acréscimo em `CheckoutFaturarNFCe`, então a opção era impossível, não apenas não implementada. O bloco é hoje só o campo de desconto.

**(2) `aplicarPagamento` é assíncrona.** `contracts/pagamento-domain-api.md` §2 a declarava `void` e afirmava que `aplicarValeDevolucao` seria "a única action assíncrona" — inconsistente com a tabela de comportamento do mesmo contrato, que exige `await validarInsercao(...)` **antes** de qualquer mutação (`FR-019`, I11). Uma action `void` não conseguiria garantir isso. `aplicarPagamento`/`aplicarForma` são `Promise<void>`; `aplicarValeDevolucao` continua sendo a única que faz **rede** por conta própria.

**(3) "Finalizar venda" passou a exigir saldo coberto.** `AcoesFinaisVenda.useVendaTemValorAFaturar` exigia só item com valor; agora exige também `saldoRestante === 0`. Sem isso o operador emitiria uma NFCe cujo `Σ FormaValor` não fecha com o total da nota — divergência fiscal detectável só na conferência. O próprio código da 004 já antecipava a extensão em comentário. **Consequência aceita:** o layout compacto não monta o cartão de pagamento (`App.tsx`: `{!compacto && <PainelPagamentoETotais />}`), logo **não há caminho de operador para pagar no mobile** e nenhuma venda pode ser finalizada lá. A superfície de pagamento do compacto é o `MobileWizard` da feature 007, ainda não implementada; o E2E do passo 7 da 004 está `test.skip` com essa justificativa, em vez de removido.

**(4) O rateio do desconto de capa entrou em `montarRetratoVenda`.** Não existe campo de desconto de capa em `CheckoutFaturarNFCe`: a única forma de expressá-lo é diluído em `produtos[].DescontoValor` (`erp-pagamento-api.md` §3). A função ganhou um 4º parâmetro **opcional** (`rateioDescontoCapa`), somado ao desconto que a linha já trazia (convênio + manual), com `ValorTotal` recebendo o abatimento. Nenhum call site existente quebra, e a 014 continua usando a mesma função — o que preserva a invariante de que venda validada e venda emitida não divergem.

**(5) `useCondicoesPagamento` lê `/api/bootstrap` por conta própria.** É uma segunda leitura da mesma rota que o `bootstrapClient` (worker + Dexie) já faz. Deliberado: `research.md` D1 rejeita ler do Dexie porque aquela cadeia não tem invalidação por tempo, e `PAY-01` exige frescor de 30 min — semântica de TanStack Query, não de armazenamento local. As **flags** `TEFAtivo`/`UtilizaCentriumPAG`, por outro lado, são lidas do `sessionStore` de forma síncrona (`vendaStore.capacidadesDoBootstrap`), porque `aplicarPagamento` não pode esperar rede para decidir se aciona TEF/PIX; bootstrap ausente ou fora do contrato ⇒ integração **desligada**, nunca habilitada por otimismo.

**Portas ainda injetadas como stub** (assinaturas definitivas, ligar é substituir o corpo em `vendaStore.pagamentoDepsPadrao`): `iniciarIntegracao` (features 009/010) e `validarInsercao`/`invalidarVeredito` (feature 014 — `ACEITA` sempre, o que é correto: o gate da 014 é filtro **adicional** sobre regras do ERP, e a validação local de `podeAplicarForma` já roda antes dele por `FR-020`).

**Impact:** `src/client/domain/pagamento/` (5 módulos), `src/client/stores/slices/pagamentoSlice.ts`, `src/client/services/pagamento/`, `src/shared/schemas/pagamento.schema.ts`, `src/client/features/pagamento/` (7 componentes), `src/client/stores/vendaStore.ts` (composição + `podeMutarCarrinho` real), `src/client/domain/venda/montarRetratoVenda.ts`, `src/client/features/finalizacao-suspensao/` (gate de saldo, bloqueio de suspensão por pagamento irreversível), `src/client/features/dav/useImportacaoDav.ts` (stub de `importarFormasDePagamento` removido) e `tests/e2e/support/` (catálogo no mock + helper `quitarVendaEmDinheiro`). **Pendente:** T044 (os 8 cenários de `quickstart.md`) e a execução do E2E `pagamento-geral.spec.ts`, que exigem a stack de pé.

### AD-147: Revisão fina do cartão de pagamento — acréscimo removido, realce da condição só no foco, scroll lateral eliminado (2026-09-04)

Revisão do cartão "Pagamento e totais" contra o `.pen` (via MCP do Pencil, nó `OzP7o`) e contra `ApiCentriumOAuth.yaml`, a pedido do usuário. Quatro correções, mais um diagnóstico que **não** é de código.

**(1) "Acréscimo" foi removido do produto, não apenas bloqueado.** AD-146(1) mantinha a opção na lista, bloqueada com motivo, por o nó `uEPUi` do Pencil a desenhar. A verificação do contrato derrubou essa leitura: `CheckoutFaturarNFCe` (`ApiCentriumOAuth.yaml`, linhas 1462–1553) **não tem nenhum campo de acréscimo** — a única saída monetária de ajuste é `DescontoPercentual`/`DescontoValor` por linha de `produtos`, exatamente onde o rateio de AD-098 grava. Uma varredura do YAML inteiro por `Acrescimo`/`Acréscimo` não encontra ocorrência alguma. Sem payload capaz de transportá-lo, o acréscimo não é "não implementado": é impossível. Um seletor com um único item viável não é um seletor, então o combobox de tipo de ajuste saiu e o cabeçalho do bloco virou "Desconto". A `ModoAjuste` (`'PERCENTUAL' | 'VALOR'`) do toggle R$/% **não** foi tocada — ela é a unidade do desconto, não o tipo de ajuste.

**(2) A borda `$cb-blue` do combobox de condição é estado de foco, não realce permanente.** O nó `IHcIy` traz `stroke: $cb-blue` de 1.5px e a implementação a reproduziu constante, lendo-a como "primeiro passo do pagamento" (TSDoc original de `SeletorCondicaoPagamento`). O autor do desenho confirmou que aquele contorno ilustra o combobox **com o foco dentro dele**. O controle passou a usar a mesma borda `$hairline` de 1px dos demais (`uEPUi`, `a2PXo`, `UwoSd`, `f9twX8`) e ganhou `focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50` — o mesmo anel de `BotaoFinalizarVenda`/`campo-data`, para o foco ter uma aparência só no produto. Efeito colateral positivo: a navegação por TAB, antes indistinguível do estado de repouso, agora é perceptível.

**(3) O scroll lateral do cartão tinha duas causas, e as duas foram tratadas.** *Causa imediata:* o `ComboboxPagamento` renderiza a raiz como `w-full`; dentro da linha horizontal do desconto ele reivindicava os 358px inteiros e, como o irmão era `flex-1 min-w-0`, o campo de valor era espremido a **0px** e o toggle R$/% (`shrink-0`, 65px) escapava para fora — medido no navegador: `scrollWidth` 422 contra `clientWidth` 358. Sumiu com (1), e o combobox não é mais usado dentro de nenhuma linha horizontal. *Causa estrutural:* a coluna rolável declarava só `overflow-y-auto`; pelo CSS, `overflow-x: visible` combinado com um `overflow-y` não-visível **computa como `auto`**, então qualquer estouro de 1px em qualquer bloco futuro reintroduziria a barra. A coluna passou a declarar `overflow-x-hidden` explicitamente. Medição pós-correção: `scrollWidth === clientWidth === 358`.

**(4) Props mortas do `ComboboxPagamento` removidas.** `rotuloVisivel`, `destacado`, `classeTrigger` e `classeTexto` existiam só para o combobox de tipo de ajuste. Mantê-las deixaria disponível justamente o caminho (`classeTrigger="w-[158px] shrink-0"` numa raiz `w-full`) que produziu o estouro de (3).

**(5) Diagnóstico sem correção de código: "Nenhuma condição de pagamento cadastrada" em dev é processo velho, não bug.** O sintoma relatado (combobox de condição vazio e, por consequência, combobox de forma bloqueado por "Escolha primeiro a condição de pagamento") vem do processo do ERP mockado que estava de pé na porta 4010: `GET /ApiCentriumOAuth/GetSessao` nele devolve `SessaoUsuario` **sem** `CondicoesDePagamento`, `ConfiguracoesTEF` e `ConfiguracoesPIX` — é uma instância iniciada antes do commit que acrescentou o catálogo a `tests/e2e/support/erp-mock.ts` (`stack.ts` roda sem `--watch`). O arquivo atual está correto: subindo o mesmo mock numa porta livre, o payload atravessa `bootstrapPagamentoSchema` e `paraCondicoesPagamento` e chega ao domínio com 1 condição ("A VISTA") e 3 formas (`DINHEIRO`, `CARTAO CREDITO`, `PIX`). **Reiniciar a stack de dev resolve.** O bloqueio da forma pela ausência de condição é comportamento correto e permanece: o catálogo do ERP é hierárquico (`CondicoesDePagamento[].CondicaoFormasDePagamento[]`), e não existe conjunto de formas fora de uma condição.

**(6) Setas navegam a seleção com a lista fechada.** Pedido do usuário na segunda rodada: com o foco no combobox **fechado**, `ArrowDown`/`ArrowUp` mudam a escolha **sem abrir** o painel, para o caixa percorrer condição e forma no ritmo do teclado — abrir a lista a cada tecla exigiria um Enter ou um clique por escolha e mataria o ganho. Regras: sem nada escolhido só desce (não existe "anterior à primeira", e subir do vazio para o fim escolheria a última condição sem o operador ter pedido); com algo escolhido anda nos dois sentidos, um passo por tecla; as pontas **não circulam** (voltar ao topo em silêncio trocaria a forma sem o operador perceber); opção bloqueada é **pulada**, não é parada, senão o teclado ficaria preso num item que o clique recusa (`formaDisponivel`, `FR-002`/`FR-003`); combobox bloqueado e lista aberta ignoram a tecla — o bloqueado **em silêncio**, sem o toast de `acaoBloqueavel`, porque uma seta segurada repete `keydown` e encheria a tela de avisos idênticos. Vale para os dois seletores por serem o mesmo `ComboboxPagamento`.

**(7) Espaçamento da coluna e recorte do anel de foco.** O `gap-base` (16px) entre os blocos do cartão não vinha do desenho: no Pencil eles distam ~8px (`oGiPa` y=104 → `Jup0R` y=178 → `uZUQX` y=274 → `J3Y1L` y=350, descontadas as alturas). Passou a `gap-xs`, e o rodapé total→ações a `gap-sm` (12px, `LcGwh` y=576 → `UaFF2` y=725). O `overflow-x-hidden` de (3) recortava o anel de `focus-visible`, que se desenha 3px **para fora** da borda: a coluna ganhou `-mx-1 px-1`, avançando 4px de cada lado sob o `p-base` do `aside` e recuperando-os como padding — o conteúdo mantém 358px e o anel cabe inteiro. `overflow-clip-margin` (o recurso de AD-134) **não** serve aqui: pelo CSS, `overflow-x: clip` ao lado de um `overflow-y` que rola tem valor usado `hidden`, e a margem de clipe só vale para `clip` de verdade — confirmado no navegador, o computado volta `hidden`.

**Impact:** `src/client/features/pagamento/SeletorCondicaoForma.tsx`, `src/client/features/pagamento/ControleDescontoCapa.tsx`, `src/client/features/pagamento/PainelPagamentoETotais.tsx` e `tests/unit/client/pagamento/SeletorCondicaoForma.spec.tsx` (novo, 9 casos cobrindo as regras de (6) no componente compartilhado e a fiação de cada seletor — store na condição, props na forma). Nenhum teste existente alterado: `tests/e2e/pagamento-geral.spec.ts` só usa `campo-valor-ajuste`, que permanece, e nenhum teste referenciava `combobox-tipo-ajuste`. `tsc --noEmit`, `eslint` e os 606 testes unitários/integração verdes.

### AD-148: Forma sem troco não recebe acima do saldo (FR-024) e o vermelho é só do faltante (2026-09-04)

Duas correções do usuário na terceira rodada de revisão do cartão de pagamento, mais uma pergunta em aberto sobre o vale devolução.

**(1) `FR-024` — a regra existia no código, errada, e não existia na spec.** `derivarValores` fazia `valorAplicado = min(valorInformado, saldoRestante)` para **toda** forma. Para `Dinheiro` isso está certo: o excedente vira troco. Para as demais era **truncamento silencioso** — o operador digitava 150,00 no PIX de uma venda de 100,00 e o ERP recebia `FormaValor: 100.00`, sem nada avisando que o valor mudou. Nem a spec nem `data-model.md` diziam o que deveria acontecer; `FR-012` só proibia troco fora do dinheiro. Agora: `podeAplicarForma` ganhou o motivo `VALOR_ACIMA_DO_SALDO` e um 4º parâmetro opcional `valorInformado`, e recusa com aviso qualquer forma que não gere troco quando o valor excede o saldo em aberto. A recusa é **local** (`FR-020`): não consulta o ERP nem abre cobrança PIX/TEF. O `min` de `derivarValores` permanece como rede de segurança para um chamador futuro que esqueça de passar o valor — o pior desfecho passa a ser um truncamento, nunca um `FormaValor` acima do total da nota. `FR-024` e dois cenários de aceitação foram acrescentados a `specs/008-pagamento-geral/spec.md` (US4).

**(2) O realce `$danger` do bloco de totais é só do "Faltante".** A implementação pintava de vermelho as duas faces do segundo cartão de métrica, argumentando que ambas pedem ação do operador. Errado: nesta tela vermelho significa **venda que ainda não fecha** — o mesmo sinal do "Faltante R$ …" da lista de pagamentos aplicados. Troco é o desfecho normal de um pagamento em dinheiro, com a venda já coberta; pintá-lo de vermelho gastava o único sinal de alarme da tela no caso em que está tudo certo. "Troco" passou ao mesmo tratamento de "Recebido".

**(3) Em aberto — o modelo do vale devolução precisa de confirmação do ERP.** Verificado na KB (`PCheckout_GetSessao`): `FormaFpgUtiCar` é o atributo `FpgUtiCar` de `TTPAGAM_WEB`, o cadastro de formas de pagamento, copiado sem transformação para o catálogo da sessão. `PCheckout_ValidaTicketDevolucao` apenas chama `PValidaTicketNfCe(..., 'validar', ...)` e devolve valor + validade — a ação literal `'validar'` indica que existem outras ações na mesma procedure, ou seja, o ticket é **conferido** aqui e **consumido** depois, o que sustenta `FR-009`. O que **não** foi possível confirmar (o worker do MCP GeneXus ficou travado numa busca longa e o health-check passou a bloquear novas chamadas) é o **domínio de valores de `FpgUtiCar`**, e portanto se `'VDV'` significa *"esta forma **é** a de vale devolução"* ou *"esta forma **aceita** um vale por cima"*. A implementação atual segue a segunda leitura, herdada de AD-048/`research.md` D10 — decisão direta do usuário, tomada contra a recomendação da época e com o risco explicitamente aceito. A primeira leitura é a mais provável pelo formato do contrato (`TicketDevolucao` é campo **por forma** em `CheckoutFaturarNFCe.FormasDePagamento[]`, e não campo de capa) e explicaria o incômodo relatado — hoje o vale só pode ser informado **depois** de inserir uma forma de pagamento, somando o valor do ticket ao `valorAplicado` daquela forma, o que pode levar `Σ FormaValor` acima do total da nota. **Nada foi alterado no vale nesta rodada**: as duas leituras produzem fluxos de operador diferentes e a escolha é do usuário. Reabrir exige confirmar o domínio de `FpgUtiCar` na KB.

**Impact:** `src/client/domain/pagamento/saldoPagamento.ts`, `src/client/stores/slices/pagamentoSlice.ts`, `src/client/features/pagamento/TotalDaVenda.tsx`, `specs/008-pagamento-geral/spec.md` (FR-024 + cenários 4 e 5 de US4), `tests/unit/domain/pagamento/saldoPagamento.spec.ts` (+5 casos) e `tests/integration/pagamentoSlice.spec.ts` (+3 casos; o caso "Pix acima do saldo limita o valorAplicado" afirmava o comportamento antigo e passou a afirmar a recusa).

### AD-149: Vale devolução é a forma `FpgUtiCar = 'VDV'`, não um crédito somado sobre outra forma — revoga AD-048 (2026-09-04)

**Decisão do usuário**, com o ciclo de vida do ticket confirmado na KB do GeneXus na mesma sessão.

**O que a KB mostra.** `PCheckout_GetSessao` copia `FpgUtiCar` (atributo de `TTPAGAM_WEB`, o cadastro de formas de pagamento) para `FormaFpgUtiCar` sem transformar. `PCheckout_ValidaTicketDevolucao` delega a `PValidaTicketNFCe` com a ação `'validar'`, que tem quatro desfechos de recusa — `DevTicSit = 1` "Ticket ainda não emitido", `DevTicSit = 3` "Ticket de devolução já foi utilizado no documento : N/S", `DevTicSit = 4`/`Today() > DevTicven` "vencido", e não encontrado "inválido" — e um de aceite, `DevTicSit = 2`, que devolve `DevValTot` inteiro. A mesma procedure tem as ações `'emitir'` (marca `DevTicSit = 3` e carimba empresa/número/série da NF — **é aqui que o ticket é consumido**, no faturamento), `'cancelar'` (3 → 2, ao cancelar a NF) e `'inutilizar'` (5).

**Três consequências diretas:**

1. **`'VDV'` marca a forma que *é* o vale devolução.** AD-048/`research.md` D10 lia `FpgUtiCar` como sinalizador de elegibilidade e tratava vazio como elegível, o que tornava *toda* forma capaz de receber um ticket. `ehElegivelParaVale` foi substituída por `ehFormaDeValeDevolucao` — `fpgUtiCar.trim().toUpperCase() === 'VDV'`, e nada mais.
2. **O vale é inserido como pagamento, não somado sobre um.** Escolher a forma de vale abre a janela do código; validado, o valor do ticket entra como um `PagamentoAplicado` próprio, com o seu `TicketDevolucao` — que é exatamente o formato de `CheckoutFaturarNFCe.FormasDePagamento[]`, onde `TicketDevolucao` é campo **por forma**. O caminho antigo somava o valor ao `valorAplicado` de um pagamento existente e conseguia levar `Σ FormaValor` acima do total da nota. O botão de ticket saiu da lista de pagamentos aplicados; o código aparece ali como a anotação "Vale <código>". Passando por `aplicarNucleo`, o vale herda todas as guardas — inclusive `FR-024`, que recusa um ticket de 25,00 numa venda de 10,00: o ERP baixa `DevValTot` inteiro, não existe uso parcial, e aplicar perderia a diferença.
3. **A guarda contra repetição é obrigatória e é local (`FR-025`).** A hipótese de que "o consumo ocorre no faturamento, então daria erro de qualquer jeito" **não se sustenta**: `'validar'` aprovaria o mesmo código duas vezes (o ticket só vira `DevTicSit = 3` em `'emitir'`), e `'emitir'` sobre um ticket já marcado não devolve erro — apenas regrava. Sem a guarda, o mesmo crédito entraria duas vezes na venda, o ERP baixaria um só, e a nota fecharia com um valor que o cliente nunca pagou. `valeDevolucao` (escalar) virou `valesDevolucao` (lista): dois tickets **diferentes** na mesma venda são legítimos, o mesmo código não.

O vale emite **dois** eventos de auditoria — `VALE_DEVOLUCAO_USADO` e `FORMA_PAGAMENTO_APLICADA` — porque são dois fatos, e sem o segundo o valor apareceria na trilha sem forma de origem.

**Não confirmado:** o domínio completo de valores de `FpgUtiCar` (a busca por fonte na KB percorre 37.946 objetos e não terminou dentro do orçamento do worker). `'VDV'` vem da instrução direta do usuário; os demais valores possíveis do campo permanecem desconhecidos e são todos tratados como "forma comum".

**Impact:** `src/client/domain/pagamento/valeDevolucao.ts`, `src/client/stores/slices/pagamentoSlice.ts`, `src/client/features/pagamento/` (`ModalValeDevolucao`, `ListaPagamentosAplicados`, `PainelPagamentoETotais`, `EntradaPagamento`), `specs/008-pagamento-geral/spec.md` (FR-008/FR-009/FR-010 reescritos, FR-025 novo, glossário), `tests/e2e/support/erp-mock.ts` (forma `VALE DEVOLUCAO`, rota `ValidaTicketDevolucao` com os cinco desfechos), `tests/unit/domain/pagamento/valeDevolucao.spec.ts` e `tests/integration/pagamentoSlice.spec.ts`. 619 testes verdes, `tsc --noEmit` e `eslint` limpos.

### AD-150: Excedente de vale confirmado, verde no troco, e a limpeza de pagamento que faltava na finalização (2026-09-04)

Três correções do usuário na quarta rodada.

**(1) `FR-026` — vale maior que o saldo passa a ser possível, com confirmação.** AD-149 fazia `FR-024` recusar o ticket que não coubesse no saldo. Está severo demais: o valor do vale **não é digitado pelo operador**, é o do ticket, e recusar deixava o cliente sem nenhuma forma de usar o crédito. Agora `podeAplicarForma` isenta a forma `'VDV'` da recusa, e `aplicarValeDevolucao` recebe um callback `confirmarExcedente` — consultado **só** quando o ticket vale mais do que falta pagar — que a UI resolve com um painel mostrando os três números (vale, faltante, perda) antes de "Aplicar mesmo assim". Sem callback, o excedente é tratado como não confirmado: o padrão é seguro para qualquer chamador que ainda não saiba perguntar. Recusar não consome nada — o ticket só muda de situação na ação `'emitir'`, no faturamento. Confirmado, o pagamento entra **limitado ao saldo** (`derivarValores`), de modo que `Σ FormaValor` nunca ultrapassa o total da nota enquanto o ERP baixa `DevValTot` inteiro: é exatamente a perda que a confirmação anunciou. A espera usa "confirm como promessa" (`Promise` resolvido por clique, guardado num `ref`) para o slice decidir tudo numa passagem, sem revalidar o ticket depois da resposta.

**(2) Verde no troco.** O bloco de totais tinha dois tons; passou a ter três, um por significado: `alerta` (`$danger`) só no "Faltante", `sucesso` (`$up`) no troco **maior que zero** — dinheiro a devolver, desfecho que deu certo e ainda pede um gesto — e `neutro` no resto, inclusive no troco zerado. A prop booleana `destacado` virou `tom: 'neutro' | 'alerta' | 'sucesso'`.

**(3) `limparPagamentos()` nunca era chamado na finalização.** `useFinalizarOuSuspenderVenda` limpava carrinho, auditoria e identidade da venda — e deixava o slice de pagamento intacto. O sintoma relatado: com o carrinho zerado e o desconto de capa ainda de pé, `calcularSaldo` passava a computar `0 − desconto` e o "Total a pagar" aparecia **negativo** logo depois de finalizar. Pior que o sintoma era o silencioso: condição de pagamento, formas aplicadas e vales de devolução sobreviviam para a venda seguinte. A chamada entrou na mesma transação de limpeza. Em defesa adicional, `calcularSaldo` passou a pisar `totalLiquido` em zero: I8 garante `desconto <= subtotal` no instante da aplicação, mas o subtotal cai depois, e nenhum estado intermediário deve produzir um total negativo em tela.

**Impact:** `src/client/domain/pagamento/saldoPagamento.ts`, `src/client/stores/slices/pagamentoSlice.ts`, `src/client/features/pagamento/` (`ModalValeDevolucao`, `TotalDaVenda`), `src/client/features/finalizacao-suspensao/useFinalizarOuSuspenderVenda.ts`, `specs/008-pagamento-geral/spec.md` (FR-026 novo, exceção anotada em FR-024), `tests/unit/domain/pagamento/saldoPagamento.spec.ts` e `tests/integration/pagamentoSlice.spec.ts`. 624 testes verdes, `tsc --noEmit` e `eslint` limpos.

### AD-151: Nenhum desconto pode zerar a venda nem um item — aperta AD-098, e a condição de pagamento exige venda com valor (2026-09-04)

Sete correções do usuário na quinta rodada; as três primeiras são de regra, as demais de tela.

**(1) A condição de pagamento fica bloqueada com subtotal 0,00.** Sem mercadoria não há venda a cobrar, e escolher a condição só levava o operador um passo adiante para descobrir, no combobox de forma, que não existe saldo a cobrir. O bloqueio é o explicativo de `lib/bloqueio.ts` (AD-143), com o motivo dito no clique. A **forma** de pagamento já dependia da condição desde a implementação original de `SeletorFormaPagamento` (o catálogo é hierárquico — as formas são as da condição escolhida), então o segundo pedido do usuário já estava atendido e passa a valer por transitividade também para o carrinho vazio.

**(2) O desconto de capa não pode zerar o total da venda.** O limite anterior era `desconto > subtotal` (I8): aceitava o desconto **igual** ao subtotal e produzia uma nota de R$ 0,00, que nenhuma forma de pagamento consegue fechar. Passou a `desconto >= subtotal`.

**(3) O desconto de capa não pode deixar item algum abaixo de R$ 0,01 depois do rateio — isto aperta AD-098, sem revogá-la.** O rateio continua sendo divisão igual com clamp e redistribuição, e `ratearDescontoCapa` continua fixando no próprio `totalLiquido` a linha cuja parcela estouraria. O que muda é que **esse desfecho deixa de ser aceitável na entrada**: uma linha fixada no teto fica valendo zero — um produto entregue de graça, decidido por um algoritmo que o operador não vê. Na prática, todo desconto aceito daqui em diante produz um rateio **sem clamp**; o clamp permanece no algoritmo como rede de segurança do caminho de montagem do payload, que não repete a checagem. A guarda é a função pura `recusaDoDescontoCapa(desconto, subtotal, linhas)`, que devolve `'ZERA_A_VENDA' | 'ZERA_UM_ITEM' | null` e nunca lança — ela verifica a pré-condição de `ratearDescontoCapa` antes de chamá-la. O slice traduz a recusa em toast ao sair do campo (`onBlur`) ou no Enter, e `AVISO_DESCONTO_ACIMA_DO_SUBTOTAL` foi substituído por `AVISO_DESCONTO_ZERA_A_VENDA` e `AVISO_DESCONTO_ZERA_ITEM` — a frase antiga descrevia um limite que não é mais o vigente.

**Consequência no caso de borda autoritativo.** O exemplo de `specs/008-pagamento-geral/data-model.md` §5 e do quickstart (70,00 / 29,00 / 1,00 com 10% de desconto) passa a ser **recusado**: os 10,00 divididos igualmente zeram a linha de 1,00. Ele continua correto como descrição do *algoritmo* de rateio, e é por isso que os testes de `ratearDescontoCapa` não mudaram; deixou de ser um cenário *aplicável* pela tela. O fluxo dourado E2E foi ajustado de 10% para 2% (2,00 → 0,67 / 0,67 / 0,66, menor linha em 0,34), preservando tudo que ele prova: split de formas, troco, saldo zerado e soma dos descontos no payload.

**(4) O desconto de um item não pode zerar o total daquele item.** Mesmo piso, `TOTAL_MINIMO_DA_LINHA = 1` centavo, agora exportado de `domain/precificacao/linha.ts` e compartilhado pelos dois caminhos que conseguem zerar uma linha. Não se confunde com o piso de `calcularTotalLinha`, que impede o total **negativo** (I8) e aceita zero: aquele é aritmético, este é de negócio. Só alcança produto `'E'`, o único em que o campo de desconto aceita digitação. Avisa no `blur`, nunca a cada tecla — digitar "10,00" num item de 10,00 passa por "1", "1,0"… e cada passagem dispararia um toast sobre um valor ainda em construção. O texto permanece no campo para correção; quem impede a inserção é `bloqueioDeInsercao`, com a frase específica antes do genérico "há um valor inválido" (o valor **é** bem formado; o que o recusa é a regra).

**(5) Espaçamento do equivalente financeiro.** A linha "= R$ … sobre o subtotal" é rodapé do campo de desconto, não bloco próprio: os 8px de `gap-xs` da coluna do cartão a afastavam do "Forma de pagamento". Margem inferior de −4px, só no modo percentual — o único em que a linha existe.

**(6) A pílula "Seguro" sai do cabeçalho de pagamento** e **(7) a etiqueta "BRL" sai do bloco de total.** As duas estão no Pencil (`y3cr1` e `X227y`) e nenhuma descreve estado do sistema: não há modo inseguro do qual distinguir a tela, e o valor logo abaixo já vem com "R$" de `formatarCentavos`. São desvios deliberados do desenho, anotados nos dois componentes.

**Impact:** `src/client/domain/precificacao/linha.ts` (`TOTAL_MINIMO_DA_LINHA`), `src/client/domain/pagamento/descontoCapa.ts` (`recusaDoDescontoCapa`, `RecusaDescontoCapa`), `src/client/stores/slices/pagamentoSlice.ts`, `src/client/features/pagamento/` (`SeletorCondicaoForma`, `ControleDescontoCapa`, `PainelPagamentoETotais`, `TotalDaVenda`), `src/client/features/carrinho/EntradaRapidaProduto.tsx`, `tests/unit/domain/pagamento/descontoCapa.spec.ts`, `tests/integration/pagamentoSlice.spec.ts`, `tests/unit/client/pagamento/SeletorCondicaoForma.spec.tsx` e `tests/e2e/pagamento-geral.spec.ts`. 634 testes verdes, `tsc --noEmit` e `eslint` limpos; os E2E de pagamento não puderam ser executados nesta máquina — já falhavam antes destas mudanças, na bipagem do produto contra o mock local.

### AD-152: A condição de pagamento congela a venda, e o pagamento ganha uma saída explícita — "Limpar" (2026-09-04)

Cinco correções do usuário na sexta rodada, todas no cartão de pagamento.

**(1) `podeMutarCarrinho()` passa a exigir também condição não selecionada — e por isso nasce `descartarPagamento()`.** O predicado era "nenhum pagamento aprovado" (I7). Agora é "nenhuma condição escolhida **e** nenhum pagamento aprovado": a condição é o ponto em que o operador declara que terminou de montar a compra e passou a cobrá-la, e deixar a grid editável a partir dali permitiria mudar o total por baixo de um pagamento em curso — a condição é justamente o que o gate da 014 e o rateio do desconto de capa tomam como fixo. Como o predicado é compartilhado com cliente e identidade da venda por decisão explícita (AD-043), o congelamento vale para os três; uma segunda regra de "a venda ainda pode mudar" divergiria em silêncio.

Isso criava um beco sem saída: o combobox só **troca** de condição, nunca a desmarca, e `limparPagamentos()` é a limpeza pós-entrega da feature 004, sem guarda nenhuma. Daí a action nova `descartarPagamento()` — gesto de tela que zera condição, formas, desconto de capa e vales, recusa quando há TEF/PIX aprovado (I6 — o estorno é do ERP), audita cada forma removida e chama `invalidarVeredito()` (`FR-021`). Na tela ela é o botão "Limpar", no cabeçalho do cartão, na faixa que a pílula "Seguro" desocupou em AD-151. `AVISO_CARRINHO_BLOQUEADO` foi reescrito para nomear essa saída: o texto anterior falava só de "pagamento aprovado", que deixou de ser o gatilho mais comum.

**(2) Venda sem valor líquido não recebe pagamento.** Guarda em `aplicarNucleo`, **antes** de `podeAplicarForma`: com `totalLiquido` zero o `saldoRestante` também é zero, e a função pura responderia `SALDO_JA_COBERTO` — a frase do desfecho oposto, o da venda integralmente paga. `AVISO_VENDA_SEM_VALOR` nomeia a causa real. A guarda ficou no slice, e não no domínio, para não trocar a assinatura de `podeAplicarForma` (que decide sobre a **forma**, não sobre a venda) e os oito call sites de teste que dependem dela. `recusaDoDescontoCapa` (AD-151) já impede o desconto de zerar a venda; esta guarda cobre o que vem **depois** — itens cancelados derrubam o subtotal sem tocar no desconto aplicado, e `calcularSaldo` pisa o líquido em zero (AD-150). O motivo também aparece antes do clique, no combobox de forma e no botão "Adicionar pagamento".

**(3) O percentual de desconto passa a aceitar uma casa decimal, não duas.** `99,9` é o formato do produto. A segunda casa não sobrevive ao arredondamento em nenhum carrinho pequeno — `aplicarPercentual` fecha em centavo inteiro, de modo que `10,25%` e `10,3%` de R$ 40,00 dão o mesmo valor — e prometia uma precisão que o resultado nunca teve.

**(4) O equivalente financeiro acompanha o campo, não o desconto aceito.** Sintoma relatado: digitar 100% deixava a linha "= R$ …" parada no valor anterior, porque ela lia `descontoCapa.valorResolvido` do store — e um desconto recusado nunca chega ao store. O operador via o número velho ao lado do aviso de recusa, sem como relacionar os dois. Quem converte percentual em reais continua sendo o domínio (`resolverDescontoCapa`, o mesmo que o slice chama); o componente só escolhe **qual entrada** mostrar resolvida: o texto, enquanto ele for um percentual legível; o valor aplicado, quando não for.

**(5) Cursor do combobox bloqueado.** O trigger de condição/forma é um `<button>` cru, fora de `components/ui/button.tsx` — que traz `aria-disabled:cursor-not-allowed` no seu `cva`, e é de onde vem o comportamento do "Cancelar venda". Só o cursor foi copiado: a opacidade que o `Button` também aplica apagaria o texto da condição já escolhida, que o operador precisa continuar lendo.

**Impact:** `src/client/stores/slices/pagamentoSlice.ts` (`descartarPagamento`, `AVISO_VENDA_SEM_VALOR`, `podeMutarCarrinho`), `src/client/stores/slices/carrinhoSlice.ts` (`AVISO_CARRINHO_BLOQUEADO`), `src/client/features/pagamento/` (`PainelPagamentoETotais` com `BotaoLimparPagamento`, `SeletorCondicaoForma`, `ControleDescontoCapa`, `EntradaPagamento`), `tests/integration/pagamentoSlice.spec.ts` e `tests/unit/client/pagamento/ControleDescontoCapa.spec.tsx` (novo). 641 testes verdes, `tsc --noEmit` e `eslint` limpos; os E2E seguem inexecutáveis nesta máquina pelo mesmo motivo registrado em AD-151.

### AD-153: O desconto de capa congela o carrinho — a guarda do rateio deixa de ser coincidência e vira invariante (2026-09-04)

Correções de uma revisão independente (subagent com contexto reduzido, pedida pelo usuário) sobre AD-151/AD-152, mais a regra que o usuário fixou ao ler o relatório: *"se tiver condição, desconto, forma de pagamento, etc, não pode voltar e mudar o carrinho, tem que limpar tudo pra poder voltar pro carrinho."*

**(1) A regra 2 de AD-151 estava sendo violada no cupom, e a causa era estrutural.** `recusaDoDescontoCapa` valida o rateio contra as linhas **do instante da aplicação**; `montarPagamentosParaPayload` rateia **de novo**, na finalização, com as linhas de então — sem revalidar. Como só a condição e o pagamento congelavam o carrinho, o desconto sobrevivia a qualquer edição de item no meio. Cenário verificado: A = R$ 100,00 e B = R$ 100,00, desconto de R$ 100,00 aceito (50/50); o operador reduz B para R$ 10,00; na finalização o clamp fixa B em R$ 10,00 e o payload sai com `ValorTotal: 0.00`. A soma dos totais bate com o líquido, então nada mais na cadeia percebe. Pior: com o desconto acima da soma restante, `ratearDescontoCapa` **lançava** dentro da finalização, depois de `VENDA_FINALIZADA`/`VENDA_SUSPENSA` já ter sido registrado e antes do estado `enviando` — botão inerte, promise rejeitada e trilha de auditoria mentindo. Em `SUSPENDER` não havia sequer a guarda de subtotal.

**A correção é a regra do usuário, não uma revalidação:** `podeMutarCarrinho()` passa a exigir também `descontoCapa === null`. Com o carrinho congelado enquanto há desconto, as duas transições deixam de existir e a pré-condição de `ratearDescontoCapa` passa a ser invariante. Revalidar na montagem teria sido a alternativa, e é pior: mudaria o total no instante de faturar, sem o operador ver. Como defesa em profundidade, `useFinalizarOuSuspenderVenda` passou a envolver a montagem em `try/catch`, transformando um bug de composição em falha de negócio visível em vez de promise rejeitada.

**(2) Regressão de AD-151: linha que já valia zero travava todo desconto de capa.** `linha.totalLiquido - parcela < TOTAL_MINIMO_DA_LINHA` era avaliado para todas as linhas, inclusive as que já valiam zero — um brinde de `PrecoVenda = 0`, ou uma linha de DAV cujo desconto igualava o bruto. A parcela dela é zero, `0 − 0` é menor que o mínimo, e **qualquer** desconto era recusado, inclusive R$ 0,01, com a mensagem "zeraria um item" sobre um item que o operador não zerou e sem nenhum valor menor capaz de liberar. A regra é sobre a **transição**: recusa-se o desconto que faz uma linha viável deixar de ser, e linha que já estava abaixo do mínimo é ignorada.

**(3) A guarda do desconto de item sobe da UI para o slice.** Em AD-151 ela vivia em `EntradaRapidaProduto.tsx` e exigia produto `'E'`. Mas `descontoManual` é absoluto sobre o total e **não** escala com a quantidade (só `descontoConvenio` é recalculado por `repricarSku`): uma linha de DAV com 10 un × R$ 10,00 e R$ 50,00 de desconto vale R$ 50,00 e, reduzida para 5 un pelo lápis, vale zero — sem o campo de desconto ser tocado, num produto que a barra nem deixa editar. `inserirItem` e `editarItem` passaram a recusar com aviso, e a checagem é `desconto > 0 && totalLinha < TOTAL_MINIMO_DA_LINHA`: o `desconto > 0` é o que preserva o brinde de preço zero, que o cadastro do ERP permite vender.

**(4) `selecionarCondicao` deixava vales órfãos.** Ela esvaziava `pagamentos` (I9) mas não `valesDevolucao`, enquanto `removerPagamento` filtrava os dois. O ticket ficava no estado apontando para um `idPagamento` inexistente e — como a guarda de código repetido consulta essa lista — o operador não conseguia reinformar o mesmo vale na condição nova.

**(5) O vale devolução pedia confirmação de perda antes de descobrir que a venda não tinha valor.** Numa venda de líquido zero, `aplicarValeDevolucao` validava o ticket na rede, calculava um excedente igual ao ticket inteiro e abria o diálogo pedindo ao operador que confirmasse perder o valor — para só então `aplicarNucleo` recusar. A guarda de venda sem valor virou a primeira instrução do método.

**(6) Equivalente financeiro.** Correção da própria correção de AD-152: texto ilegível como percentual (`"10,25"`, digitado no caminho para `"10,2"`) voltava a exibir o desconto anterior — reproduzindo o defeito que AD-152 queria eliminar. Agora vale zero. Texto que corresponde exatamente ao desconto vigente lê o `valorResolvido` gravado, para a linha descrever a venda e não uma conta refeita.
...
### AD-158: Feature 009 (pagamento PIX) implementada — a janela nasce do estado, não de um callback (2026-09-04)

A feature 009 foi implementada por completo (`/speckit-implement`, 27 tarefas de `specs/009-pagamento-pix/tasks.md`). Sete decisões foram tomadas **durante** a implementação e divergem, ou estendem, o que os artefatos de design fixaram. Cada uma está registrada no TSDoc do arquivo correspondente; esta AD é o índice delas.

**1. `iniciarIntegracao` continua no-op — e isso não é pendência.** O contrato da 008 entrega a integração por uma porta imperativa (`PagamentoDeps.iniciarIntegracao`, `vendaStore.ts`). Montar o modal a partir dela exigiria guardar o contexto da cobrança em algum lugar, criando um estado paralelo ao `PagamentoAplicado` e capaz de discordar dele. Em vez disso a janela é **função do estado**: `usePixPendente` (`ListaPagamentosAplicados.tsx`) monta `ModalPix` enquanto existir um pagamento `PENDENTE_INTEGRACAO` com `integracao === 'PIX_DINAMICO'`, e o desmonta no instante em que ele é confirmado ou removido. Ligar também a porta imperativa criaria um segundo gatilho para o mesmo desfecho — exatamente o que a nota final do `tasks.md` proíbe. A porta permanece no contrato porque o veredito `TEF` (feature 010) ainda não tem dono.

**2. `CobrancaPix` não tem `idPagamento`, contra `data-model.md` §1.** Nenhum ponto do desenho consegue preenchê-lo: nem `ModalPixProps` nem `useGerarPix().gerar` recebem esse dado, e ele não trafega na resposta do ERP. A correlação existe e é mais forte do que um campo copiado — quem monta a janela fecha `idPagamento` dentro dos callbacks `onAprovado`/`onAbandonado`. Uma segunda cópia do mesmo vínculo poderia divergir da primeira em silêncio.

**3. A prop é `valor`, não `saldoRestante`, contra `contracts/pix-domain-api.md` §3.** O que chega é `PagamentoAplicado.valorAplicado`, já limitado ao saldo por `derivarValores` (008). Nos cenários do quickstart os dois números coincidem (é o mesmo `60,00` do Cenário 6), mas se separam num split em que o operador cobra **parte** do saldo por PIX — e aí o correto é o valor da forma inserida. Manter o nome antigo prometeria um número que a prop não carrega. `FR-010`/J6 continuam satisfeitos.

**4. `ConfiguracoesPIX.MinimoPix` passou a ser convertido a `Centavos` na fronteira Zod.** Antes atravessava o `looseObject` da 008 sem uso, como `double` em reais. Agora `pagamento.schema.ts` o converte, `paraMinimoPix` (`pagamentoMapper.ts`) o expõe e ele viaja em `CatalogoPagamento.minimoPix` — junto do catálogo, e não numa query própria, porque é o mesmo payload de `/api/bootstrap` que já é lido ali. Fica **fora** de `CapacidadesPagamento`: aquele tipo alimenta `resolverIntegracao`, que decide para onde uma forma roteia, e o piso não participa dessa decisão.

**5. Valor abaixo do mínimo **remove** o pagamento pendente.** `FR-009` diz "toast e a janela não abre". Só isso deixaria a venda com um `PENDENTE_INTEGRACAO` órfão que nada mais resolveria — a forma já foi inserida pela 008 quando o modal monta. O modal avisa, chama `onAbandonado('VALOR_ABAIXO_DO_MINIMO')` e renderiza `null`, sem tocar a rede.

**6. Token novo `--cc-color-up-ink` (#04794A).** É o `$success-ink` do Pencil, o **texto** sobre `$success-soft` no componente "Badge" (nó `qrD4E`) usado pelo modal como "Aguardando confirmação". Não é `--cc-color-up` (#05B169), que é a cor do marcador: usá-lo como texto sobre o fundo claro derruba o contraste. O desenho separa os dois de propósito.

**7. O MCP do Pencil não conectou nesta sessão** (`CONNECTION_CLOSED`, junto do `genexus`). O design foi lido direto de `design/CentriumCheckout.pen` (frame "PDV Online Web - Modal PIX", nó `j3pJA`), que é JSON legível e é a mesma fonte que o MCP serve — alternativa já registrada. Três estados do modal **não têm nó** no desenho e estão documentados no TSDoc de `ModalPix.tsx`: "gerando" (shimmer `cc-shimmer` na caixa do QR Code), "erro de geração" (painel de alerta + "Tentar novamente") e o bloqueio por valor mínimo (a janela não chega a aparecer).

Duas notas menores: `PixQueriesDeps.intervaloMs` é injetável **só para teste** — o padrão de produção continua os 10s fixos de AD-026 —, e `useGerarPix` guarda a chamada em voo num `ref` porque o `StrictMode` do React 19 executa o efeito duas vezes em desenvolvimento; sem a trava, uma segunda cobrança real ficaria órfã no adquirente, sem caminho de cancelamento (`research.md` D11).

**Impact:** novos `src/client/domain/pix/` (`cobrancaPix.ts`, `interpretarStatusPix.ts`, `validarValorMinimoPix.ts`, `montarDadosPagador.ts`), `src/client/services/pix/` (`pixMapper.ts`, `pixQueries.ts`), `src/shared/schemas/pix.schema.ts` e `src/client/features/pagamento/pix/ModalPix.tsx`; alterados `pagamento.schema.ts`, `pagamentoMapper.ts`, `pagamentoQueries.ts`, `ListaPagamentosAplicados.tsx`, `vendaStore.ts`, `global.css` e `tests/e2e/support/erp-mock.ts` (endpoints `GerarPIX`/`StatusPIX` e as configurações `pixAtivo`/`minimoPix`/`statusPixTransicoes`). 702 testes de unidade/integração verdes (44 novos), 2 cenários E2E novos verdes, `tsc --noEmit` e `eslint` limpos.

**Pré-existente, fora do escopo desta feature:** 5 cenários E2E falhavam também sem estas mudanças (verificado com `git stash`). Corrigidos logo em seguida — ver AD-159.

### AD-159: O ERP mockado volta a refletir o cadastro real — catálogo, ticket de vale e o DAV sem pagamento (2026-09-04)

**Pedido do usuário:** *"corrija as falhas E2E. Crie um vale ticket que posso validar, crie mais uma condicao de pagamento e mais formas, corrija a forma de pagamento cartao para nao ser VDV, e sim ter uma forma especifica do vale."*

`tests/e2e/support/erp-mock.ts` não é só fixture de CI: é a stack que o operador do projeto sobe localmente para testar à mão. Cinco cenários E2E estavam vermelhos e as duas causas eram **colisão de merge** entre as PRs #50 (008) e #51 (006), não regressão da 009:

1. **Os SKUs `070000`/`029000`/`001000` sumiram do `CATALOGO`.** As duas branches editaram o mesmo arquivo em paralelo; o merge preservou o catálogo de pagamento da 008 mas não os produtos que os cenários dela bipam. Os três testes de `pagamento-geral.spec.ts` falhavam em `biparProduto`, antes de qualquer asserção. Repostos, com o código de cada um codificando o próprio preço (`070000` → 70,00).

2. **O DAV sintético trazia `FormaMeioPagtoNFe: '01'`.** O domínio `Nfce_FormaPagto` usa **nomes** (AD-023), então `importarFormasDePagamento` descartava a forma como meio desconhecido: ela nunca chegou à venda em nenhum momento da história da suíte. Enquanto finalizar não exigia saldo zero isso passava despercebido; com a 008, "Finalizar venda" nunca habilitava e dois cenários de `importacao-dav.spec.ts` quebravam.

   A forma foi **removida**, não corrigida para `'Dinheiro'`. Um DAV é documento pendente de cobrança, e um pagamento importado entra `APROVADO` — que **congela a venda** (I7). Corrigir o literal teria trocado uma falha por outras duas: "item novo é precificado normalmente" (carrinho congelado) e "segundo documento é recusado por já ter documento" (passaria a ser recusado por já ter pagamento). Os dois cenários que finalizam a venda agora quitam pela UI com `quitarVendaEmDinheiro`, como todo E2E desde a 008.

**O catálogo de pagamento cresceu**, a pedido do usuário: a condição "A VISTA" ganhou cartão de débito (5), PIX estático (6) e vale alimentação (7); entrou uma segunda condição, "30 DIAS" (prazo 30, entrada mínima R$ 20,00), com boleto (8), crediário (9) e duplicata (10) — formas **distintas**, para tornar visível que a forma pertence à condição e que trocar de condição esvazia as aplicadas (I9). Os códigos 1–4 são estáveis: os cenários os endereçam por `opcao-forma-<codigo>`.

**Sobre "cartão não pode ser VDV":** o cadastro sintético já estava correto — `FormaCodigo: 2` (cartão) sempre teve `FpgUtiCar` vazio, e `FormaCodigo: 4` é a única forma de vale. `ehFormaDeValeDevolucao` compara só contra `'VDV'` desde 2026-09-04 (a leitura "vazio = aceita vale" de AD-048 foi revogada). Se o cartão abrir a janela do vale numa stack local, **o build servido está defasado**: o `webServer` do Playwright tem `reuseExistingServer` e reaproveita um processo antigo na porta 3100 sem reconstruir — foi o que aconteceu nesta sessão e produziu diagnósticos enganosos até o processo ser derrubado. O comentário no mock agora diz isso explicitamente. Ticket novo `VALE10` (R$ 10,00) cobre exatamente o produto `001234`, para fechar uma venda de um item pela forma de vale sem excedente.

**Achado colateral, virou item 40 de `PENDENCIES.md`:** o cenário "não retransmite o payload no F5" contava **duas** leituras de `/api/bootstrap` — a revalidação condicional do cache da 002 (`304`) e a query de catálogo da 008 (`useCondicoesPagamento`), que refaz a rota inteira sem `If-None-Match` e recebe `200`. O veredito dependia de qual chegasse primeiro, e crescer o catálogo bastou para inverter a corrida. O cenário passou a contar só as requisições condicionais, que é o que ele afirma; que a segunda leitura baixe o payload completo é custo real e ficou registrado como pendência da 008.

**Impact:** `tests/e2e/support/erp-mock.ts`, `tests/e2e/importacao-dav.spec.ts`, `tests/e2e/auth-bootstrap.spec.ts`, item 40 em `.specs/project/PENDENCIES.md`. Suíte E2E inteira verde (116 passados, 1 pulado) e 702 testes de unidade/integração verdes.

### AD-160: Uma condição de pagamento por venda — a troca é recusada, não silenciosa (2026-09-04)

**Pedido do usuário:** *"3 - Ao alterar uma condicao de pagamento, a forma de pagamento selecionada no combobox deve ser limpa. 4 - Não deve ser possível alterar a condição de pagamento se uma forma de pagamento daquela condicao já foi inserida, cada venda tem uma condicao de pagamento só. 4.1 - (…) se eu troco a condicao de pagamento, as formas de pagamento já inseridas somem (Nao sei se sao apagadas, ou se só nao sao exibidas e seriam enviadas para o ERP)."*

**Resposta à dúvida de 4.1: eram apagadas.** `selecionarCondicao` fazia `set({ condicaoSelecionada, pagamentos: [], valesDevolucao: [] })` — as formas saíam do estado, então nunca chegariam ao ERP. Não era exibição: era perda de dado. O que estava errado não é o payload, é o gesto: um esvaziamento silencioso do que o caixa acabou de digitar, sem aviso e sem caminho de volta.

**A regra nova substitui I9.** Antes: "trocar a condição esvazia os pagamentos". Agora: **cada venda usa uma condição só** — com qualquer forma aplicada, `selecionarCondicao` recusa a troca com aviso e **nada muta** (no-op com aviso, mesmo padrão de `carrinhoBloqueado()`). Sem pagamento a troca segue livre, e aí não há o que esvaziar: `valesDevolucao` só existe vinculado a um `idPagamento`, então lista de pagamentos vazia implica a de vales vazia. A saída continua sendo explícita e já existia — **"Limpar"** (`descartarPagamento`, AD-152), que zera condição, formas, desconto e vales *a pedido do operador*. A frase do aviso nomeia a regra e aponta esse botão.

O combobox de condição passa a bloquear com **a mesma frase** que o slice usa ao recusar (`AVISO_CONDICAO_COM_PAGAMENTO`, importada, não redigida de novo): a regra chega ao operador antes de ele abrir a lista, e uma segunda redação divergiria da primeira com o tempo. Continua `aria-disabled` + `acaoBloqueavel`, nunca `disabled` nativo (AD-143) — clicar ensina o motivo.

**Trocar a condição limpa a forma escolhida.** `formaSelecionada` é rascunho de UI e vive em `PainelPagamentoETotais`; mantida através da troca, o combobox exibiria o nome de uma forma que a condição nova não oferece e `EntradaPagamento` aceitaria um valor para ela, com a recusa chegando só no clique. O reset compara o código da condição anterior **durante o render** (padrão de `abertoAnterior` em `ModalValeDevolucao`), não num `useEffect`: assim não existe um quadro intermediário mostrando a forma antiga sob a condição nova. O modal de vale fecha junto, pelo mesmo motivo.

**Ícone do vale devolução.** A forma de vale tem `FpgUtiCar = 'VDV'`, mas o seu `FormaMeioPagtoNFe` é livre no cadastro e costuma ser `'Outros'` — pelo mapa `ICONE_POR_MEIO` ela herdava a carteira genérica, o mesmo ícone de "pagamento não informado". Agora `iconeDaForma`/`iconeDoPagamento` (em `iconePorMeio.ts`, uma regra para os dois call sites) devolvem `Ticket`, o ícone do cabeçalho de `ModalValeDevolucao`: o combobox e a lista mostram o mesmo desenho da janela do código. **Não** é `TicketCheck` (o do campo e do botão "Aplicar vale"), que significa "validado" e diria algo falso numa forma ainda não inserida. O marcador difere por call site e isso é deliberado: a forma do catálogo usa `ehFormaDeValeDevolucao(forma)`; o pagamento aplicado usa `ticketDevolucao !== null`, porque `PagamentoAplicado` congela o meio e o ticket, nunca o `FpgUtiCar` (`data-model.md` §2, "Regra de fronteira").

**PIX ligado por padrão no ERP mockado** (`pixAtivo: true`, `minimoPix: 5`): o `GetSessao` sintético precisa expor PIX para o fluxo da 009 ser testável à mão na stack local, sem um `POST /__mock/config` antes de cada sessão. Não afeta as demais suítes — só a forma `Pix` roteia para `PIX_DINAMICO`, e os cenários que quitam uma venda usam dinheiro.

**Impact:** `src/client/stores/slices/pagamentoSlice.ts` (`AVISO_CONDICAO_COM_PAGAMENTO`, `selecionarCondicao`), `src/client/features/pagamento/SeletorCondicaoForma.tsx`, `PainelPagamentoETotais.tsx`, `iconePorMeio.ts`, `ListaPagamentosAplicados.tsx`, `tests/e2e/support/erp-mock.ts`, `tests/integration/pagamentoSlice.spec.ts` e `tests/e2e/pagamento-geral.spec.ts`. 704 testes de unidade/integração verdes e 117 cenários E2E verdes (1 pulado).

### AD-161: PIX e TEF deixam de ser o mesmo caso — PIX é removível com confirmação, TEF bloqueia; e a janela do PIX trava até aprovar (2026-09-04)

**Pedido do usuário**, oito correções numa mesma passagem: *"1 - Mesmo com o PIX já pago, o usuario consegue 'Cancelar venda', mandando do rascunho pro ERP. 1.1 - Se tiver PIX gerado, tem que informar que o PIX precisa ser cancelado e pedir confirmação (…). 1.2 - Se tiver TEF, não pode cancelar, por que o cancelamento do TEF precisa ser feito antes de cancelar. 2 - Pode SIM remover uma forma de pagamento PIX, mas isso não cancela e faz o estorno automaticamente, o operador precisa fazer isso diretamente no banco (…). 3 - Adicionar modal de confirmação da remoção de forma de pagamento do PIX (Tem que preservar log). 4 - Ao inserir o pix, nao deve constar como pago automaticamente, deve constar como pago no endpoint mock uns 20s depois de gerar. 5 - A imagem que recebemos do ERP/Mock esta em base64, tem que decodificar e exibir corretamente. 6 - O texto recebido do ERP/Mock (TrnBase64text) também está em base64, tem que decodificar (Aqui, faca primeiro a validacao se é base64, se nao estiver, so transmite o dado). 7 - O modal do PIX, não pode ser fechado pelo usuário enquanto o pagamento ainda não está aprovado, nem com ESC, ele é fechado automaticamente 10s depois quando o PIX for identificado como pago (…). 8 - (…) Se já tiver informado cliente (Que nao seja o default), ou algum produto no carrinho, ao usuario tentar atualizar a página, deve pedir confirmacao (…)."* Correção enviada durante a execução: *"Cancelar operação deve ser algo como Desistir da operacao, pois de fato o PIX não é cancelado pelo checkout."*

**A premissa corrigida: "integração externa" não é uma categoria só.** AD-030 e AD-042 fixaram que pagamento aprovado por TEF **ou** PIX é irreversível, e a implementação da 008 traduziu isso literalmente — `pagamento.integracao !== 'NENHUMA' && status === 'APROVADO'` bloqueava remoção, "Limpar" e suspensão. O usuário separou os dois casos, e a distinção é material:

- **TEF continua irreversível.** A transação vive num terminal físico. Remover a forma da venda sem cancelar lá deixa o Checkout e a operadora discordando sobre dinheiro já debitado, e o cancelamento tem uma ordem obrigatória: **primeiro o terminal, depois a venda**.
- **PIX passa a ser removível, com confirmação.** O Checkout **nunca** soube cancelar cobrança PIX — não existe endpoint para isso no contrato (invariante J5 da 009, AD-040). Bloquear a forma na tela não estornava nada e não protegia o dinheiro de ninguém; só prendia o operador numa venda que ele precisava reorganizar. A remoção agora existe e passa por confirmação explícita, e é a confirmação que anuncia o que o sistema não faz.

O predicado de bloqueio passou de `integracao !== 'NENHUMA'` para `integracao === 'TEF'` em três lugares que precisavam concordar: `removerPagamento`, `descartarPagamento` ("Limpar") e o gate de suspensão. `AVISO_PAGAMENTO_IRREVERSIVEL` ("Pagamento aprovado por TEF/PIX…") foi substituído por `AVISO_TEF_IRREVERSIVEL`, que nomeia a operação certa — cancelar no terminal — em vez de "o estorno é operação do ERP".

**Cancelar venda com PIX pergunta; com TEF, recusa.** "Cancelar venda" envia `SUSPENDER` (`FR-002` da 004). Antes desta mudança, uma venda com PIX **aprovado** caía no bloqueio de suspensão e o operador ficava sem saída nenhuma; com PIX **pendente**, ela era suspensa em silêncio e a cobrança seguia viva no banco sem ninguém ser avisado. Agora a máquina de estados de `useFinalizarOuSuspenderVenda` ganhou o estado `confirmar-suspensao-pix` e a porta `confirmarSuspensao()`. A checagem fica **antes** do evento terminal de auditoria: registrar `VENDA_SUSPENSA` e o operador voltar atrás deixaria o histórico afirmando uma suspensão que nunca houve. Cobrança **pendente conta tanto quanto aprovada** — o gatilho não é "o dinheiro entrou", é "existe cobrança registrada que o Checkout não sabe cancelar", e o pior desfecho é justamente o cliente pagar depois de a venda virar rascunho.

**Um diálogo, quatro call sites.** `DialogoConfirmacaoPix` é parametrizado por texto, não por um `variante: 'remocao' | 'suspensao' | …` (Open/Closed): desistir da cobrança na janela do PIX, remover a forma da lista, "Limpar" e "Cancelar venda" descrevem o mesmo fato do mundo real. Os textos moram em `avisosPix.ts`, num módulo próprio — quatro redações do mesmo fato divergem com o tempo até uma delas prometer um cancelamento que não existe. `AVISO_DESASSOCIACAO_MANUAL` foi reescrito: falava só em "desassociar na Central de Transações PIX do ERP", e o usuário corrigiu que a cobrança que já recebeu dinheiro se resolve **no banco** — a Central só desfaz a associação com o documento.

**A janela do PIX trava até aprovar, e sobrevive dez segundos à aprovação.** ESC ficou inerte e o `X` do cabeçalho nasce bloqueado (`aria-disabled` + motivo, nunca `disabled` mudo — AD-143); os dois liberam quando o pagamento confirma. `onAprovado` continua sendo chamado **na hora** — o pagamento vira `APROVADO` no `vendaStore` imediatamente, e adiar isso junto com o fechamento deixaria o total da venda mentindo por dez segundos —, mas a janela só chama `onFechar` depois do intervalo, mostrando disco verde, subtítulo "Pagamento aprovado" e badge "Pagamento confirmado". Isso exigiu que `usePixPendente` deixasse de procurar "existe um pagamento pendente de PIX?" e passasse a seguir o `idPagamento` **exibido**: a condição antiga desmontaria a janela no exato quadro em que ela precisa mostrar que deu certo. A decisão de AD-158 ("a janela nasce do estado") fica preservada — a janela continua sendo função do estado, só que do pagamento que ela está exibindo.

**"Cancelar operação" virou "Desistir da operação"**, por correção direta do usuário no meio da implementação. O rótulo anterior prometia um cancelamento que o Checkout não executa; um botão que promete cancelar e apenas remove a forma da tela é a pior espécie de falso positivo num caixa. **O botão permanece** — ele é a única saída de uma cobrança que nunca vai ser paga, e removê-lo transformaria o travamento do item 7 em deadlock: sem status terminal do ERP, o operador só sairia com F5, perdendo a venda inteira. A escolha foi apresentada ao usuário, que confirmou manter o botão com confirmação.

**Base64: o nome do campo é promessa, não fato.** `domain/pix/base64.ts` (novo, puro) resolve os itens 5 e 6. `decodificarSeBase64` **valida primeiro** — alfabeto exato, preenchimento múltiplo de 4 e legibilidade do resultado — e só então decodifica; entrada que não é base64 sai como entrou. Um `atob` incondicional sobre um "copia e cola" já em texto puro devolveria bytes sem sentido para o operador colar no app do banco, e a versão anterior ainda por cima **esvaziava** o campo em caso de erro. `fonteDeImagemBase64` monta a `data:` URL detectando o tipo MIME pelos primeiros bytes: a UI declarava `image/jpeg` para todo conteúdo, inclusive para os **PNG** que o `PGetBarCodeImage` do ERP gera. `CobrancaPix.qrCodeImagemBase64` virou `qrCodeFonte` (URL pronta) — guardar o base64 cru obrigava a UI a escolher um formato que só o conteúdo conhece.

**ERP mockado: PIX pago pelo relógio, e um PNG de verdade.** `statusPixTransicoes` passou a nascer **vazio**, e nesse modo `StatusPIX` devolve `'G'` até `atrasoPagamentoPixMs` (20s) depois da geração, e `'P'` a partir dali — rastreado por `TrnGUID`, não por um relógio global, para a segunda cobrança de uma venda não nascer herdando o tempo da primeira. O roteiro explícito continua tendo precedência e é o que todo cenário E2E usa, porque teste não pode depender de relógio. `Trnbase64image` deixou de ser um `/9j/…` de 32 caracteres que não era imagem nenhuma e passou a ser um PNG 116×116 real com padrão de QR sintético: o valor antigo bastava para afirmar o formato da `data:` URL, mas na stack local o operador via um ícone quebrado e não conseguia distinguir "o mock é falso" de "a decodificação está errada" — que era exatamente o defeito sob investigação.

**`beforeunload` finalmente saiu do papel.** A constitution do projeto exige desde 2026-08-20 que "F5 durante a venda MUST exigir confirmação explícita do operador (`beforeunload`)" (AD-006), e nenhuma linha de código implementava isso: um F5 acidental apagava carrinho, cliente e pagamento sem uma única pergunta. `useAvisoAoSair(ativo)` é um hook genérico que não conhece o `vendaStore` (Dependency Inversion) e apenas decide **se** o diálogo nativo aparece — o texto é do navegador e não há API que o mude desde 2016. O gatilho reusa o mesmo recorte de `vendaAtiva` (AD-138): cliente default **não** conta, porque a pré-seleção automática não é ação do operador e perguntar ali transformaria o aviso em ruído; linha cancelada **conta**, porque permanece no array por rastreabilidade (`CART-08`) e é prova de que houve digitação. O ouvinte é removido quando não há venda, para não desligar o bfcache do navegador.

**Impact:** novos — `src/client/domain/pix/base64.ts`, `src/client/features/pagamento/pix/avisosPix.ts`, `src/client/features/pagamento/pix/DialogoConfirmacaoPix.tsx`, `src/client/lib/useAvisoAoSair.ts`, `tests/unit/domain/pix/base64.spec.ts`. Alterados — `src/client/domain/pix/cobrancaPix.ts`, `src/client/services/pix/pixMapper.ts`, `src/client/features/pagamento/pix/ModalPix.tsx`, `ListaPagamentosAplicados.tsx`, `PainelPagamentoETotais.tsx`, `src/client/stores/slices/pagamentoSlice.ts`, `src/client/features/finalizacao-suspensao/useFinalizarOuSuspenderVenda.ts`, `AcoesFinaisVenda.tsx`, `src/client/App.tsx`, `tests/e2e/support/erp-mock.ts`, `tests/integration/{ModalPix,pagamentoSlice}.spec.*`, `tests/e2e/pagamento-pix.spec.ts`. Supera o recorte de AD-030/AD-042 (só quanto ao PIX) e a Story P1 "Fechamento do modal com PIX pendente" de AD-040. 723 testes de unidade/integração verdes, `tsc --noEmit` e `eslint` limpos, 118 cenários E2E verdes (1 pulado).

### AD-162: TEF ganha cancelamento de transação — endpoint no ERP + polling, como o PIX (2026-09-04)

**Pedido do usuário:** *"9 - TEF pode cancelar a transacao, mas o cancelamento o TEF chamará um endpoint no ERP para tal operação. (Alterar na SPEC provisória do TEF) 9.1 - No cancelamento do TEF, deve solicitar o cancelamento chamando o endpoint, e ficar consultando outro Endpoint do ERP para ver se foi de fato cancelado, com pooling, semelhante ao PIX. (Alterar na SPEC provisória do TEF)"*

**Corrige AD-023 na parte que afirmava não existir fluxo de reversão.** Aquela decisão concluía que, como a forma não pode ser removida da UI, "qualquer reversão de um TEF já aprovado é tratada fora do Checkout, diretamente no terminal físico pelo operador". O usuário informa que **existe** um caminho pelo ERP: o Checkout solicita o cancelamento chamando um endpoint e sonda um segundo endpoint até o ERP confirmar que a transação foi de fato cancelada — a mesma mecânica de `GerarPIX`/`StatusPIX` (AD-026: intervalo fixo, sem backoff, sem SSE).

**O que continua valendo de AD-023/AD-042, sem mudança:** a forma TEF **não sai da venda por remoção direta**, e a venda com TEF aprovado **não é suspensa** enquanto a transação existir. O cancelamento é uma operação própria, com pedido e confirmação assíncrona, e só depois de confirmada a venda volta a poder ser cancelada — é exatamente o que o item 1.2 do usuário fixa ("o cancelamento do TEF precisa ser feito antes de cancelar" a venda).

**Fica sem implementação nesta passagem, e é deliberado.** A feature 010 (TEF) não existe no código, e os **nomes e contratos dos dois endpoints não estão no `ApiCentriumOAuth.yaml`** — abre o item 41 de `.specs/project/PENDENCIES.md`. O bloqueio de AD-037 (protocolo de comunicação com o terminal, parceiro a ser trocado) permanece separado deste ponto: aquele é sobre falar com o terminal físico, este é sobre falar com o ERP. A spec registra a regra agora para que a fase Design da 010 não a redescubra nem invente um mecanismo próprio.

**Impact:** `.specs/features/pagamento-tef/spec.md` (Edge Cases reescritos, nova Story P2, `PAY-12`), `.specs/features/finalizacao-suspensao-venda/spec.md`, `.specs/project/PENDENCIES.md` (item 41), `specs/010-pagamento-tef/spec.md` (Spec Kit — User Story 3, `FR-007`–`FR-009`, correção do cenário 2 da US1, Edge Cases e Assumptions) e `specs/010-pagamento-tef/checklists/requirements.md` (nota de emenda). Sem mudança de código.

### AD-163: A origem da escolha decide o modal do vale; sem forma não digita valor; excluir passa a riscar, não apagar (2026-09-04)

**Pedido do usuário**, três correções: *"1 - Se passar o Vale devolucao pela seta, nao deve chamar o modal automaticamente, nesse caso só mudando o foco com tab. Mas se selecionar com o mouse, ai pode chamar diretamente o modal pra informar o codigo, pois é nesse caso que temos certeza que o usuario quis de fato colocar essa forma de pagamento. 2 - Não pode colocar o valor recebido antes de informar a forma de pagamento. 3 - Ao operador excluir a forma de pagamento da lista, ela deve ficar riscada, mas permanecer lá, assim como o produto, a diferença é que o que foi excluido, não é enviado para o ERP como um pagamento efetuado, é só exibicao na UI (Log da insercao/exclusao da forma de pagamento deve ser preservado normalmente)"*

**Item 1 — a seta deixou de abrir o modal do vale devolução.** Desde AD-123/AD-152, `navegarPorSeta` percorre o combobox de forma **sem abrir a lista**, e `escolherForma` (`PainelPagamentoETotais.tsx`) abria o modal do vale sempre que a forma virava `Vale Devolução`, sem olhar como ela chegou lá. Isso fazia a seta abrir uma janela de confirmação a cada opção sobrevoada — o operador que só queria ver a próxima forma ficava preso, tendo de apertar Escape para continuar. `OpcaoCombobox.aoEscolher` (`SeletorCondicaoForma.tsx`) ganhou um parâmetro `OrigemSelecao` (`'teclado' | 'mouse'`), propagado por `navegarPorSeta` (sempre `'teclado'`) e pelo `onClick` da opção (sempre `'mouse'`), até `SeletorFormaPagamento.onSelecionarForma` e `escolherForma`. Só `origem === 'mouse'` abre o modal — o clique é gesto único e deliberado; a seta continua escolhendo a forma (o rascunho `formaSelecionada` muda normalmente), só não dispara mais nada sozinha. `SeletorCondicaoPagamento` ignora o parâmetro (não muda de comportamento) porque TypeScript aceita um callback com menos parâmetros do que a assinatura declarada.

**Item 2 — revoga a decisão anterior sobre o campo de valor.** Até aqui `bloqueioDoCampo` em `EntradaPagamento.tsx` travava a digitação **só** por venda sem valor, de propósito documentado: "'Escolha a forma antes' impede *adicionar*, não digitar". O usuário decidiu o oposto — sem forma escolhida não há para onde os centavos digitados irem, e o campo passou a ficar `readOnly` (com `aria-disabled` + motivo, nunca `disabled` mudo, AD-143) também quando `forma === null`. `bloqueioDeInsercao` (o botão "Adicionar pagamento") não mudou — já recusava sem forma.

**Item 3 — exclusão vira estado, não desaparecimento.** `StatusPagamento` ganhou um quarto valor, `'EXCLUIDO'`, ao lado de `PENDENTE_INTEGRACAO`/`APROVADO`/`RECUSADO`. `removerPagamento` deixou de filtrar o array (`pagamentos.filter(id !== alvo)`) e passou a mapear o pagamento alvo para `{ ...pagamento, status: 'EXCLUIDO' }` — mesmo tratamento do item cancelado do carrinho (`linha.cancelada`, `GridItens.tsx`): é rastreabilidade, não apagamento. `calcularSaldo` e `montarPagamentosParaPayload` já filtravam por `status === 'APROVADO'`, então uma forma excluída sai do saldo e do envelope ao ERP **sem nenhum filtro novo** nos dois — só `podeAplicarForma` precisou de um ajuste (a checagem de dinheiro duplicado passou a ignorar `EXCLUIDO` do mesmo jeito que já ignorava `RECUSADO`). O vale devolução vinculado a uma forma excluída sai de `valesDevolucao` como antes — o ticket não foi de fato consumido (o ERP só marca `DevTicSit = 3` no faturamento), então o mesmo código pode ser informado de novo na venda. `ItemPagamentoAplicado` (`ListaPagamentosAplicados.tsx`) risca a faixa (`text-muted-foreground line-through`), some com o botão de remover e anuncia o estado por `sr-only`, todos os três já usados por `GridItens.tsx` para o produto cancelado — nenhum vocabulário visual novo. `removerPagamento` chamado sobre um pagamento já `EXCLUIDO` é no-op (idempotente, sem segundo evento de auditoria); a guarda de TEF aprovado (AD-161) continua intacta, avaliada antes da mutação.

**Efeito colateral aceito, não corrigido:** `temPixNaVenda()` (`useFinalizarOuSuspenderVenda.ts`) conta qualquer pagamento PIX "em qualquer status" — antes desta AD, remover um PIX o tirava do array e a função naturalmente parava de contá-lo; agora ele permanece `EXCLUIDO` e continua contando. Isso é uma melhoria silenciosa, não uma regressão: a cobrança PIX segue viva no banco mesmo depois de "excluída" da lista (AD-161), e a função existe exatamente para capturar esse risco.

**Impact:** alterados — `src/client/domain/pagamento/saldoPagamento.ts` (`StatusPagamento`, `podeAplicarForma`), `src/client/stores/slices/pagamentoSlice.ts` (`removerPagamento`), `src/client/features/pagamento/SeletorCondicaoForma.tsx` (`OrigemSelecao`, `OpcaoCombobox.aoEscolher`, `SeletorFormaPagamentoProps.onSelecionarForma`), `src/client/features/pagamento/PainelPagamentoETotais.tsx` (`escolherForma`), `src/client/features/pagamento/EntradaPagamento.tsx` (`bloqueioDoCampo`), `src/client/features/pagamento/ListaPagamentosAplicados.tsx` (`ItemPagamentoAplicado`, `ANOTACAO_POR_STATUS`), `tests/integration/pagamentoSlice.spec.ts`, `tests/unit/domain/pagamento/saldoPagamento.spec.ts`, `tests/unit/client/pagamento/SeletorCondicaoForma.spec.tsx`, `tests/e2e/pagamento-pix.spec.ts`. Novo — `tests/unit/client/pagamento/EntradaPagamento.spec.tsx`. 728 testes de unidade/integração verdes, `tsc --noEmit` limpo. E2E não reexecutado nesta passagem (stack do mock não estava de pé) — ver `.specs/project/PENDENCIES.md` se um item de acompanhamento for necessário.

### AD-164: A rolagem do cartão de pagamento desce para a lista de formas aplicadas, e cada inserção traz a lista até o fim (2026-09-04)

**Pedido do usuário:** *"Uma venda pode ter varias formas de pagamento inseridas, isso já está ok, o problema é que quando há muitas formas de pagamento, o scroll vertical aparece na box inteira de pagamentos. 1 - O Scroll vertical tem que aparecer apenas no local ali onde ficam as formas de pagamento inseridas, nao na box toda. 2 - Ao inserir uma nova ofrma de pagamento, se tiver scroll, ele tem que ir para o ultimo item inserido."*

**Item 1 — quem rola passa a ser a lista, não a coluna.** A coluna central de `PainelPagamentoETotais.tsx` já era `flex-1 min-h-0 overflow-y-auto`, e ela era o **único** bloco capaz de encolher: seletor de condição, desconto, seletor de forma, campo de valor e a lista de aplicados são todos itens flex com `min-height: auto`, então nenhum cedia altura e a sobra virava rolagem da coluna inteira. O efeito prático era o operador rolar um bloco de controles (condição, desconto, forma) para alcançar uma lista que está logo abaixo deles — e perder de vista justamente os controles que usaria em seguida. A correção não move o `overflow` de lugar, **acrescenta um destino melhor para ele**: `ListaPagamentosAplicados` ganhou `min-h-0` na `<section>` e a `<ul>` virou o contêiner rolável (`min-h-0 overflow-y-auto`). Como ela passa a ser o único descendente que pode encolher abaixo do próprio conteúdo, o algoritmo do flexbox congela os blocos de controle no tamanho do conteúdo e joga toda a falta de espaço nela. Cada `<li>` recebeu `shrink-0` — sem isso o flex tiraria a altura das próprias faixas (as 34px do Pencil virariam ~20) e nada rolaria, porque o conteúdo "caberia" espremido. A rolagem declarada na coluna **permanece**, agora como rede de segurança: só aparece quando nem a lista zerada basta (tela baixa demais para os blocos fixos), e é o que impede o bloco de total e o botão de finalizar de saírem do cartão.

Duas armadilhas de `overflow` já conhecidas se repetem no novo contêiner e estão anotadas nele: `overflow-x-hidden` é obrigatório junto do `overflow-y-auto` (pelo CSS, `overflow-x: visible` ao lado de um `overflow-y` não-visível é computado como `auto`, e a primeira faixa a estourar 1px criaria barra horizontal), e o corte resultante comeria o anel de `focus-visible` do botão de remover — devolvido por dentro com `-mx-1 px-1`, mesmo truque da coluna. `overflow-clip-margin` continua sem serventia aqui pelo motivo já registrado em AD-134/AD-147.

**Item 2 — a lista salta para a forma recém-inserida.** `useEffect` em `ListaPagamentosAplicados` com `lista.scrollTop = lista.scrollHeight`. A dependência é o **id da última forma** (`pagamentos.at(-1)?.idPagamento`), não o comprimento do array nem o array: desde AD-163 remover não tira o item da lista, só o marca `EXCLUIDO`, então o comprimento não distingue mais inserção de exclusão; e `pagamentos` troca de referência a cada mutação do slice (confirmação de PIX, recusa de TEF), o que arrancaria a lista de onde o operador a deixou ao reler uma forma anterior. `scrollTop`/`scrollHeight` em vez de `scrollIntoView` no último `<li>` porque `scrollIntoView` rola **todo** ancestral rolável até o elemento — incluindo a coluna do cartão, arrastando os controles de cima para fora da vista exatamente no caso de tela baixa em que a rede de segurança está ativa.

**Impact:** alterados — `src/client/features/pagamento/ListaPagamentosAplicados.tsx` (`ListaPagamentosAplicados`, `ItemPagamentoAplicado`), `src/client/features/pagamento/PainelPagamentoETotais.tsx` (só o comentário da coluna, que descrevia a rolagem antiga). Novo — `tests/unit/client/pagamento/ListaPagamentosAplicados.spec.tsx` (3 casos: o contêiner rolável é a `<ul>`; cada forma inserida leva a lista ao fim; excluir não arrasta a lista de volta). 731 testes de unidade/integração verdes, `tsc --noEmit` limpo, ESLint e Prettier limpos nos três arquivos. E2E não reexecutado (mudança é de layout/rolagem, sem asserção equivalente na suíte Playwright); a conferência visual em navegador fica para a próxima subida da stack.
