...
**Impact:** `src/client/features/pagamento/SeletorCondicaoForma.tsx`, `src/client/features/pagamento/ControleDescontoCapa.tsx`, `src/client/features/pagamento/PainelPagamentoETotais.tsx`. Nenhum teste alterado — `tests/e2e/pagamento-geral.spec.ts` só usa `campo-valor-ajuste`, que permanece; nenhum teste referenciava `combobox-tipo-ajuste`. `tsc --noEmit`, `eslint` e os 597 testes unitários/integração seguem verdes.
