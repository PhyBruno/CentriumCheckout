...
**Impact:** `src/client/features/dav/BotaoMenuImportacao.tsx`. Testes: `tests/e2e/importacao-dav.spec.ts` (dois casos passam a afirmar a notificação no clique bloqueado). 457 unitários/integração e 22 E2E de importação verdes; `tsc --noEmit` e `eslint` limpos.
