...
### AD-094: `GetCliente` não tem como buscar por código — bloqueia dados completos do cliente default (`TipoPreco=9`) sem interação do operador (2026-08-26)

**Decision:** Registrado como **pendência bloqueante** (item 31 de `.specs/project/PENDENCIES.md`), não resolvido nesta rodada — decisão direta do usuário foi sobre **como tratar** o achado, não sobre a lacuna em si. `GetCliente` aceita somente `Empresa` e `CPFCNPJ` como parâmetros (confirmado em `ApiCentriumOAuth.yaml`) — não existe `CodCliente`/`CodigoCliente`. `GetSessao.SessaoUsuario` devolve `ClienteDefaultCodigo`/`ClienteDefaultNome` (código + nome), sem CPF. Resultado: quando uma NFCe nasce com o cliente default pré-selecionado (AD-032), sem nenhuma interação do operador com o modal de busca, o Checkout **não tem como** chamar `GetCliente`/`GetListaClientes` para obter `ListaPreco`/`DescontoConvenio` desse cliente — os dois campos que a feature 003 (carrinho, AD-092) exige para resolver `TipoPreco = 9` (preço por lista do cliente) via `GetProduto`.
...
**Trade-off:** Até a resolução, o plano da feature 005 documenta o cliente default como parcialmente resolvido — código e nome disponíveis desde `GetSessao`, `ListaPreco`/`DescontoConvenio` indisponíveis sem o operador abrir o modal e reselecionar (ainda que seja o mesmo cliente). Isso não bloqueia a venda (o campo cliente nunca fica vazio, AD-032 continua válido), mas pode produzir preço incorreto em `TipoPreco = 9` até o operador interagir com o modal ou até o ERP resolver a pendência.

**Impact:** `.specs/features/identificacao-cadastro-cliente/spec.md` (Edge Cases), `.specs/project/PENDENCIES.md` (item 31, seção 1). `specs/005-identificacao-cadastro-cliente/` (`research.md`, `plan.md` — Technical Context/Constraints) documentam o limite. Referenciado também pela feature 003 (`.specs/features/carrinho-produto-precificacao/spec.md`, D9/D10 de `specs/003-carrinho-produto-precificacao/research.md`), que consome `ListaPreco`/`DescontoConvenio` do cliente selecionado — o limite descrito aqui se propaga para lá.
...
**Trade-off:** Um cadastro com `;` no nome perde o atalho até ser corrigido no ERP. Aceito deliberadamente: numa feature que lança pagamento e pode finalizar a venda sozinha, um parse "provavelmente certo" é pior que nenhum atalho — descartar produz a ausência visível de um botão, adivinhar produz um pagamento na condição errada. A alternativa de ancorar pelos campos estruturados (primeiro e dois últimos) foi rejeitada por deixar `CPgPraCod` ambíguo exatamente no caso em que `CPgFpgDes` contém o separador.

**Impact:** `specs/013-venda-rapida-cenario-pagamento/` (`FR-004`; `research.md` D3; `data-model.md` E2/I3; `contracts/erp-cenario-pagamento-api.md`). **Item 34 de `.specs/project/PENDENCIES.md`** — pedido à equipe do ERP de serialização estruturada (array JSON de objetos, como já é feito em `CondicoesDePagamento[]`) ou de proibição/escape de `;` nos campos de texto do cadastro. Não bloqueia a implementação: se atendido, apenas permite simplificar o parser.
...
**Impact:** `specs/013-venda-rapida-cenario-pagamento/` (`FR-018`; `research.md` D4; `data-model.md` I11; `contracts/erp-cenario-pagamento-api.md`). **Item 35 de `.specs/project/PENDENCIES.md`** — confirmar o literal exato contra resposta real de homologação e, confirmado, estreitar o conjunto aceito. Não bloqueia a implementação.
