...
**O cliente default não conta como "cliente identificado".** A distinção é `clienteSlice.houveEscolhaExplicita`: a tela de venda nasce com o default aplicado (AD-032), então considerá-lo impediria toda importação. Mesma razão pela qual "itens" conta só linhas **ativas** (`linhasAtivas`), a mesma noção de "a venda tem item" já usada por `BotaoCancelarVenda`.
