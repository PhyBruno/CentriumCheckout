...
### AD-128: O indicador da barra superior mostra o modo de emissão da NFCe (online × contingência), lido do polling de `GetStatusSistema` — e o polling passa a fazer uma leitura imediata ao entrar em "entre vendas" (2026-09-02)

**Decision:** A pílula de status da barra superior (nó `swUNN` do Pencil) **não** é indicador de conectividade do navegador. Ela mostra o modo de emissão da NFCe informado pelo ERP: o valor lido pelo polling de `GET /ApiCentriumOAuth/GetStatusSistema` (`FR-013`, AD-088) é traduzido em `0` → "Online" (ponto verde) e qualquer valor `>= 1` → "Contingência" (ponto amarelo). Enquanto não houve nenhuma leitura bem-sucedida, a pílula mostra "Verificando…" com ponto cinza — nunca "Online" por presunção. Uma leitura que falha (rede, sessão encerrada, corpo fora do contrato) não altera o indicador: o último valor conhecido permanece.

Como consequência, `usePollingStatusSistema` passa a **consultar imediatamente** ao entrar em "entre vendas", além de manter o intervalo de 60s. Sem isso o operador ficaria até um minuto sem saber em que modo o PDV está emitindo, que é justamente a pergunta que o indicador existe para responder.

**Rationale:** Correção direta do usuário (2026-09-02) sobre a primeira implementação da barra superior, que havia derivado o indicador de `navigator.onLine`. A spec da feature 004 já dizia isto de forma explícita — "WHEN o sistema precisa detectar **contingência do ERP** THEN o sistema SHALL chamar `GET /ApiCentriumOAuth/GetStatusSistema`" (`.specs/features/finalizacao-suspensao-venda/spec.md`) — e o mesmo valor responde às duas perguntas do Checkout: em que modo a máquina emite (o que a barra mostra) e se a configuração publicada em `GetSessao` mudou (o que dispara o rebootstrap de AD-088). Conectividade de navegador não é a informação que o operador de caixa precisa: uma rede local íntegra com o ERP em contingência é exatamente o caso que o indicador tem de denunciar.

**Trade-off:** O indicador congela durante uma venda em digitação, porque o polling continua suspenso nesse período (`FR-013`, inalterado) — o operador vê o último modo conhecido, não um valor obtido naquele instante. Aceito: retomar o polling durante a venda contraria uma decisão já tomada, e o modo de emissão só muda por evento do ERP, não por ação do operador. Segue valendo a ressalva de AD-088 sobre o significado exato de valores `> 1` (`CadStatus` não tem documentação na KB): a decisão é binária, "zero ou não-zero".
