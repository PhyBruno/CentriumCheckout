...
**O cliente default não conta como "cliente identificado".** A distinção é `clienteSlice.houveEscolhaExplicita` **em conjunção com `clienteAtual !== null`** (a flag sozinha estava errada — corrigido em **AD-139**, porque `limparCliente()` zera o cliente sem zerar a flag): a tela de venda nasce com o default aplicado (AD-032), então considerá-lo impediria toda importação. Mesma razão pela qual "itens" conta só linhas **ativas** (`linhasAtivas`): uma linha cancelada não é conteúdo que a importação sobrescreveria. Esta noção **deixou de ser compartilhada** com `BotaoCancelarVenda` em **AD-140** — lá, cancelar a venda passou a ser permitido com qualquer linha, ativa ou cancelada; são duas perguntas diferentes ("o documento entraria sobre conteúdo?" e "há venda digitada a suspender?"), e a partir daquela AD cada uma tem a sua regra.
...
**Corrige, no ponto:** **AD-138** (a noção de "a venda tem item" deixa de ser compartilhada entre a recusa de importação e o botão de cancelar). **Complementa:** **AD-137** (implementação da 006), **AD-139** (a recusa e seus pontos de aplicação).
