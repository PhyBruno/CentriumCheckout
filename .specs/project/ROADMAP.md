...
| 9 | Menu gerencial (redirect para telas legadas do ERP) | **Implementado** (2026-09-10, AD-203): atalho do meio da faixa "Atalhos da venda" abre o seletor, e cada opção redireciona por `GET /gerencial/:destino` no BFF. Segue sem spec de feature, por AD-020 — é nota arquitetural, é só link/navegação. As URLs de AD-020/AD-026 estavam erradas e foram substituídas: `wwtecfmovnaofisc.aspx` e `WWPResumoCaixa.aspx`, uma por opção | `.specs/codebase/ARCHITECTURE.md` (seção Responsividade) |
...
| 13 | Display do cliente (segunda tela — espelho do QR Code PIX) | **Implementada (2026-09-10, AD-210 a AD-213)** — rota `/display` montada **fora** de `App`/`AppShell`, aberta pelo botão do monitor da barra superior; a cobrança PIX do operador é espelhada por `BroadcastChannel`, o primeiro canal entre abas do projeto. Fecha o item 28 de `PENDENCIES.md` (AD-066). A **propaganda** do repouso ficou fora de escopo por decisão do usuário; falta a validação manual com dois monitores (T048) | `specs/015-display-cliente-pix/`, `src/client/features/display/` |
