...
- **`ProdutoPesavel`/`DavMatProdPes` — RESOLVIDO PARCIALMENTE (2026-08-25, AD-063), para o contexto de `GetProduto`/`GetListaProdutos`:** o nome real do campo no contrato é `ProdutoPesavelEditavel` (`SDTCheckout_GetProduto`, string, sem enum documentado no yaml) — não localizado nas buscas anteriores por não conter os termos `ProdutoPesavel`/`MatProdPes`/`MatBloq*`/`MatEdit*`/`MatPermite*` isoladamente. Valores confirmados pelo usuário: `'S'` = pesável, leitura na etiqueta; `'B'` = pesável, leitura na balança; `''` = não pesável e não editável; `'E'` = não pesável, mas editável. Esse mesmo campo também resolve o item 4 de `.specs/project/PENDENCIES.md` (flag de editabilidade ao TAB, AD-027) — ver `.specs/project/STATE.md`, AD-063, para a interpretação assumida (pesável e editável são mutuamente exclusivos) e a ressalva. **Continua sem confirmação:** `DavMatProdPes` (contexto de item de DAV, nome visto só em KB) não aparece em `ApiCentriumOAuth.yaml` sob nenhum nome — não foi coberto pela resposta do usuário, que tratou só do contexto de produto avulso.
