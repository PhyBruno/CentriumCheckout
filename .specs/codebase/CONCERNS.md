...
- **`DescontoConvenio` (percentual, não valor fixo):** confirmado no KB — `PGeraPedidoVenda` calcula `&ConvDsc = (1 - CliConvDsc / 100)`, fator de desconto percentual clássico. **Precisão (2026-08-31, AD-108):** o **cliente default** não tem convênio — `descontoConvenio = 0` sempre que a venda corre com ele, sem consulta a `GetCliente`. Impacta o motor de precificação (`.specs/features/carrinho-produto-precificacao/spec.md`) e o cadastro de cliente (`.specs/features/identificacao-cadastro-cliente/spec.md`).
...
- **Elegibilidade em `ValidaTicketDevolucao`:** ⚠️ **Superado em 2026-08-27 por AD-101 (`.specs/project/STATE.md`), leia lá antes de implementar.** Achado original (2026-08-21): acreditava-se que não existia campo booleano dedicado, com base em `PCheckout_ValidaTicketDevolucao` chamando `PValidaTicketNfCe.Call(..., 'validar', ..., &ValorTicket, &retorno, &msgPadrao)` — a leitura de então só olhava a atribuição de `Mensagem` (`&retorno = 0` → texto de erro do ERP; senão → literal fixo `'Ticket Válido'`) e não notou que o próprio `PCheckout_ValidaTicketDevolucao` também atribui `&Valido = true`/`false` nos mesmos dois ramos. **A elegibilidade é decidida só por `Valido`**, sempre preenchido pelo procedure — não é mais necessário comparar `Mensagem`. Ver `.specs/features/pagamento-geral/spec.md`.
- **Origem do `NumeroRascunho` em `FaturarNFCe`** (o campo se chamava `NumeroNota` até o contrato `20260914191012`; renomeado em AD-235): totalmente confirmado — `PCheckout_FaturarNFCe`: `NumeroRascunho = 0` grava um rascunho novo via `PNFeSerializaRascunhoNota.Call(..., 'SERIALIZA', ...)` (`NewCapa`, numerado pelo ERP, sem controle de sequência no cliente); `NumeroRascunho <> 0` usa o valor recebido e executa `AtualizarCapa` (rascunho pré-existente: importado de DAV/NFCe ou adotado após recusa). A procedure devolve `NumeroRascunho` na resposta **antes** das validações de saldo/rascunho, então uma recusa já deixou o rascunho gravado — por isso o Checkout adota o número devolvido (AD-235). Ver `.specs/features/finalizacao-suspensao-venda/spec.md`.
...
- **`usaPrecoPorQuantidade` — RESOLVIDO (2026-08-24, AD-025):** não existe esse campo, nem equivalente, em `SessaoUsuario` nem em `SDTCheckout_GetProduto` — busca por nome não encontrou nada. **O sinal real é `SessaoUsuario.TipoPreco = 8`** (confirmado por regra de negócio direta do usuário, ver seção "Pendências de campos/semântica do contrato — atualização 2026-08-24" abaixo), não uma inferência local. Histórico: antes de AD-025, a única alternativa cogitada (nunca implementada) era inferir esse modo localmente checando `QtdMinimaPreco2 > 0` — hipótese descartada, substituída pela resposta direta acima.
- **`ProdutoPesavel`/`DavMatProdPes` — RESOLVIDO POR COMPLETO (2026-08-25, AD-063; correções em 2026-08-26, AD-069/AD-070):** o nome real do campo no contrato é `ProdutoPesavelEditavel` (`SDTCheckout_GetProduto`, string, sem enum documentado no yaml) — não localizado nas buscas anteriores por não conter os termos `ProdutoPesavel`/`MatProdPes`/`MatBloq*`/`MatEdit*`/`MatPermite*` isoladamente. Valores confirmados pelo usuário: `'S'` = pesável, leitura na etiqueta; `'B'` = pesável, leitura na balança; `''` = não pesável e não editável; `'E'` = não pesável, mas editável. Esse mesmo campo também resolve o item 4 de `.specs/project/PENDENCIES.md` (flag de editabilidade ao TAB, AD-027). **A exclusividade pesável/editável deixou de ser interpretação assumida (AD-070): é garantida pela própria estrutura do campo (um único valor entre 4 discretos, não dois booleanos combináveis).** **`DavMatProdPes` (contexto de item de DAV) fechou sem pendência nova (AD-069):** um item de DAV carregado já vem com o produto inserido e o preço congelado (mesma regra de linha congelada de AD-067); a caracterização pesável/editável só entra em jogo se a linha for reinserida/editada, quando passa pelo fluxo normal já coberto acima — não há cenário em que o Checkout precise ler essa flag diretamente do payload de `GetDAV`. Ver `.specs/project/STATE.md`, AD-063/AD-069/AD-070.
- **`GetStatusSistema` — RESOLVIDO POR COMPLETO (2026-08-26, AD-088):** decisão direta do usuário confirma que o endpoint já implementa a semântica de limiar (`0` = nada mudou, `>=1` = mudou, exige rechamar `GetSessao`) decidida em AD-075/AD-080, e que o parâmetro `Cadmaqcod` deve ser enviado com o valor de `SessaoUsuario.CadMaqCod` (recebido em `GetSessao`). **Histórico:** achados de KB anteriores (AD-024, e nova checagem em 2026-08-26) mostravam a procedure repassando `CadStatus` bruto sem transformação, e o atributo não tem `Documentation`/`Help` na KB — lacuna de documentação do próprio ERP, não recuperável por inspeção de KB; superado pela confirmação direta do usuário nesta rodada. Ver `.specs/features/finalizacao-suspensao-venda/spec.md` e `.specs/project/PENDENCIES.md` (item 7 removido).
- **`FaturarNFCe.produtos` — trilha de tier de preço — RESOLVIDO SEM EXPANSÃO DE CONTRATO (2026-08-26, AD-069):** confirmado, campo a campo, que o array não tem nenhum campo para registrar a faixa de preço aplicada. Satisfeito pelo evento `PRODUTO_ALTERADO` do log de auditoria geral da venda (AD-061) — decisão direta do usuário de manter o log só no Checkout, sem expandir o contrato de `FaturarNFCe`. Ver `.specs/project/STATE.md`, AD-069.

**RESOLVIDO (2026-08-25, AD-058) — vínculo DAV ↔ NFCe faturada:** o item "Vínculo `CheckoutFaturarNFCe` ↔ DAV importado", antes listado acima como reforçado/pendente, saiu da lista — o vínculo não passa por uma escrita do Checkout em `CheckoutFaturarNFCe`/`DavDocFNum` (o achado de KB desta rodada, de que nenhuma procedure do Checkout escreve em `DavDocFNum`, segue tecnicamente correto). O vínculo é interno ao ERP, criado a partir do rascunho de NFCe que `GetDAV` já gera automaticamente (AD-057); o próprio ERP fecha a DAV ao identificar esse vínculo no faturamento. Ver `.specs/project/STATE.md` e `.specs/features/importacao-dav/spec.md`.
...
- **`TipoPreco` (`SessaoUsuario.TipoPreco`, via `PTrazEmpDefP.Call`):** confirmado que o valor vai de `1` a `11` e indica **diretamente qual regra de preço de venda vale para o item** — não é um espelho 0-based de `ListaPreco` como as hipóteses anteriores (AD-023) chegaram a cogitar. **Correção (2026-08-25, AD-059):** de `1` a `5` e em `9` (preço por lista), o valor a aplicar no item é sempre o campo único `PrecoVenda`, retornado por `GetProduto` — e **somente** por `GetProduto`: o retorno de `GetListaProdutos` não possui esse campo (ver AD-091). O ERP já resolve internamente qual regra vale (índice `1`-`5` ou lista do cliente) e devolve o valor final pronto nesse campo. `PrecoVenda1`...`PrecoVenda5` e `PrecoVendaLista` **não** são usados para ler o valor a aplicar nesses casos — essa leitura corrige a hipótese anterior (AD-025) de indexação direta em `PrecoVenda1`...`PrecoVenda5` e de leitura de `PrecoVendaLista` para `TipoPreco = 9`. De `6` a `11` são casos especiais:
  - `TipoPreco = 9` — **preço por lista:** aplicar **sempre** a lista de preço configurada no cadastro do cliente (`ClienteCheckout.ListaPreco`/`CliListCod`, via `PCheckout_GetCliente`) — **exceto** quando a venda corre com o **cliente default**, caso em que essa mesma lista já chega pronta em `SessaoUsuario.ListaPrecoDefault` (`GetSessao`) e o Checkout **não** chama `GetCliente` (2026-08-31, AD-108; o cliente default também não tem desconto de convênio). **Não existe lista de preço padrão da empresa**, e portanto não há fallback: `TipoPreco = 9` significa exatamente "o preço vem da lista do cliente" (confirmado por decisão direta do usuário em 2026-08-26, AD-092 — o campo `SessaoUsuario.listaPrecoPadrao` citado em redações anteriores nunca existiu no contrato; e `ListaPrecoDefault`, acrescentado em `20260827192357`, é a lista *do cliente default*, não da empresa). O valor a aplicar no item vem do campo `PrecoVenda` (ver correção AD-059 acima), já resolvido pelo ERP a partir da lista informada.
  - `TipoPreco = 8` — **preço por faixa de quantidade:** único caso em que o Checkout usa `PrecoVenda1`...`PrecoVenda5` (não `PrecoVenda`) para decidir a faixa no cliente — resolve a pendência do item abaixo (`usaPrecoPorQuantidade`).
  - `TipoPreco = 6, 7, 10, 11` — **resolvido (2026-08-25, AD-031, corrigido no mesmo dia por AD-060, decisão direta do usuário):** `6` = Preço de Custo, `7` = Preço da última venda, `10` = Preço Cliente x Produto (`PRM0241`), `11` = Preço por Índice. Os quatro valores **estão no escopo do Checkout**, tratados pela mesma regra geral de `PrecoVenda` (AD-059) — o motor de precificação (`CART-04`/`CART-05`) cobre `1`-`11` uniformemente, sem tratamento especial para nenhum deles; `8` continua sendo a única exceção real (usa `PrecoVenda1`...`PrecoVenda5`). Não há, e nunca houve, pendência de requisito aqui. Ver `.specs/project/STATE.md`, AD-060.
...
## Formato de código de barras pesável — detecção RESOLVIDA, parse fino BLOQUEANTE (2026-08-24, AD-028; reclassificado em 2026-08-26, AD-068)

**Resolvido (detecção):** um código de barras bipado com **13 dígitos**, começando em `2`, indica produto gerado por balança (pesável). Confirma o padrão EAN-13 de balança já levantado como hipótese em AD-023, descartando a sintaxe alternativa `código*quantidade`. `ProdutoPesavel`/`MatProdPes`/`DavMatProdPes` (contrato) seguem servindo só para o cadastro indicar que o produto *pode* ser pesado — não fazem parte do parse do código bipado em si.

**Pendência bloqueante (extração fina):** a extração dos demais 12 dígitos do código (faixa do código reduzido do produto vs. faixa de peso/valor vs. dígito verificador) não foi detalhada nesta rodada — nenhuma lógica de parse completa foi localizada na KB (AD-023). **Corrigido em 2026-08-26 (AD-068 em `.specs/project/STATE.md`):** sem essa máscara o Checkout não sabe qual substring do código enviar a `GetProduto` — deixa de ser "detalhe de implementação não bloqueante" e passa a ser pendência bloqueante de requisito (item 29 de `.specs/project/PENDENCIES.md`), pois a story de inserção direta de produto pesável não é implementável sem ela. Ver `.specs/features/carrinho-produto-precificacao/spec.md`.
...
**Atualização (2026-08-26, AD-084):** essa revisão manual foi adiantada (via GitHub API, sem instalar nada) para o estado atual dos dois repositórios — `anl331/goey-toast` e `0xGF/boneyard`. Nenhum conteúdo malicioso encontrado: os `SKILL.md`/`CLAUDE.md` embutidos são só documentação de uso/arquitetura para agentes de IA, sem instruções de exfiltração, comando destrutivo ou tentativa de "ignorar instruções anteriores"; nenhum dos dois `package.json` reais (`goey-toast`, `packages/boneyard/package.json` publicado como `boneyard-js`) tem script `postinstall`/`preinstall` que dispare algo automaticamente num `npm install`. Achado lateral não relacionado a supply-chain: `boneyard-js` depende de `playwright`, que baixa binários de browser (~300MB) via seu próprio `postinstall` — considerar isso na imagem Docker quando a instalação real acontecer. Esta pendência **continua aberta** (item 20 de `.specs/project/PENDENCIES.md`) porque o lembrete de reler o conteúdo bruto no momento da instalação real segue valendo — os pacotes podem publicar versões novas até lá. Ver `.specs/project/STATE.md`, AD-084.

## `GetDavOutput` desatualizado em `ApiCentriumOAuth.yaml` — retorna `SDTDav`, mas o ERP na prática devolve o mesmo shape de `CarregarNFCe` — RESOLVIDO (2026-08-25)

**Risco identificado:** o contrato (`ApiCentriumOAuth.yaml`, linhas 675-679, versão de 2026-08-21) documentava `GetDavOutput` como `{ Dav: SDTDav }` — cabeçalho do DAV + `DavItemStruct`/`DavForPagamento`. Esclarecimento direto do usuário (2026-08-25, AD-057 em `.specs/project/STATE.md`) confirmou que isso estava desatualizado: ao processar `GetDAV`, o ERP gera automaticamente um rascunho de NFCe, e é esse rascunho — no mesmo shape de `CarregarNFCeOutput` — que é de fato devolvido.

**Resolvido:** conferido no `ApiCentriumOAuth.yaml` mais recente (`info.version: 20260825172440`, 2026-08-25) — `GetDavOutput` já referencia `OutCheckoutFaturarNFCe: $ref CheckoutFaturarNFCe`, igual a `CarregarNFCeOutput`/`FaturarNFCeOutput`. Usuário atualizou o contrato conforme prometido. Não é mais pendência (removido do mapa consolidado, `.specs/project/PENDENCIES.md`, item 26).
