...
- **Mecanismo de "marcar DAV como importado/em faturamento":** resposta direta do usuário — não existe endpoint separado; ao importar e faturar a DAV, o próprio `FaturarNFCe` já trata isso, mas exige um campo preenchido no SDT `CheckoutFaturarNFCe` cujo nome exato ainda **não foi definido** (marcado explicitamente pelo usuário como "PENDÊNCIA DEV"). **RESOLVIDO (2026-08-25, AD-058):** não exige campo nenhum — o rascunho de NFCe gerado automaticamente por `GetDAV` (AD-057) já fica vinculado internamente à DAV de origem; o próprio ERP identifica esse vínculo ao faturar via `FaturarNFCe` e fecha a DAV automaticamente. Ver `.specs/features/importacao-dav/spec.md`.
...
- **Vínculo `CheckoutFaturarNFCe` ↔ DAV importado:** não é só falta de nome de campo — `genexus_analyze(mode=impact)` em `DavDocFNum` não encontrou nenhuma procedure do Checkout escrevendo nesse campo. Não existe hoje nenhum caminho de código que marque a DAV como faturada a partir do Checkout; é mudança de KB do ERP a priorizar, não resposta simples de nomenclatura. **RESOLVIDO (2026-08-25, AD-058):** o achado de KB continua tecnicamente correto, só não era o caminho relevante — o vínculo não passa por uma escrita do Checkout em `CheckoutFaturarNFCe`/`DavDocFNum`; é interno ao ERP, criado a partir do rascunho de NFCe que `GetDAV` já gera automaticamente (AD-057). Ver `.specs/features/importacao-dav/spec.md`.
