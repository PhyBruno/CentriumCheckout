...
describe('exigeDocumentoImpresso — nunca exige impressão (FR-018/AD-064)', () => {
  ...
  it('teste negativo (I10, D12): aplicar DuplicataMercantil nunca aciona o serviço de impressão', () => {
    ...
    const forma = formaDe({ meioPagtoNFe: MEIO_PAGTO.DuplicataMercantil });
...
describe('predicados sobre a forma (sanidade — comportamento já fixado em formaPagamento.ts)', () => {
  it('ehDinheiro reconhece só Dinheiro', () => {
    expect(ehDinheiro(formaDe({ meioPagtoNFe: MEIO_PAGTO.Dinheiro }))).toBe(true);
    expect(ehDinheiro(formaDe({ meioPagtoNFe: MEIO_PAGTO.Pix }))).toBe(false);
  });

  it('ehCartao reconhece crédito e débito', () => {
    expect(ehCartao(formaDe({ meioPagtoNFe: MEIO_PAGTO.CartaoCredito }))).toBe(true);
    expect(ehCartao(formaDe({ meioPagtoNFe: MEIO_PAGTO.CartaoDebito }))).toBe(true);
    expect(ehCartao(formaDe({ meioPagtoNFe: MEIO_PAGTO.Dinheiro }))).toBe(false);
  });

  it('ehPixDinamico não reconhece PixEstatico', () => {
    expect(ehPixDinamico(formaDe({ meioPagtoNFe: MEIO_PAGTO.Pix }))).toBe(true);
    expect(ehPixDinamico(formaDe({ meioPagtoNFe: MEIO_PAGTO.PixEstatico }))).toBe(false);
  });
});
