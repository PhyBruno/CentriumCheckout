...
export interface SnapshotVenda {
   ...
   * `UsuarioCodigo` com o operador que o próprio ERP associa ao `Login` do
   * cookie cifrado (`src/server/session/usuarioDaSessao.ts`), como já faz com
   * `Cliente.Empresa` — o corpo sai do navegador e seria editável no DevTools,
   * e o ERP não confere o campo contra o token. O cliente continua preenchendo
