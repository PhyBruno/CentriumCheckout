...
/**
 * Consulta de mídia equivalente ao limiar de `classificarLayout` (T006,
 * `research.md` D1).
 *
 * Derivada do limiar, e não escrita à mão: `767.98px` é `768 - 0.02`, a folga
 * que fecha o buraco em telas de largura fracionária (mesmo padrão que
 * frameworks CSS usam para "abaixo de 768"). Escrever o número solto aqui
 * criaria a segunda cópia do breakpoint que `classificarLayout` existe para
 * evitar.
 */
export const CONSULTA_LAYOUT_COMPACTO = `(max-width: ${LARGURA_MINIMA_DESKTOP_PX - 0.02}px)`;
 ...
 * direto no teste, sem sintetizar eventos de redimensionamento.
 *
 * **Quem chama isto**: `AppShell` (`research.md` D3), o único ponto do projeto
 * que decide *quais* componentes existem na tela. Um componente de domínio pode
 * consultá-lo para variar sua própria apresentação interna (densidade de grid,
 * por exemplo), mas nunca para decidir composição de tela — essa decisão é
 * exclusiva do `AppShell`, e é o que mantém a checagem de breakpoint fora de
 * dezenas de componentes (Open/Closed).
 */
