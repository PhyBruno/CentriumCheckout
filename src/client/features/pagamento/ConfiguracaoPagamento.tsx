...
export interface ConfiguracaoPagamentoProps {
  /**
   * A coluna interna disputa a altura do pai (desktop) ou cresce com o conteúdo
   * (mobile)?
   *
   * É a **única** diferença entre os dois layouts, e é de apresentação pura: no
   * cartão do desktop a lista de pagamentos aplicados é o bloco que absorve a
   * falta de espaço e ganha barra por dentro; na etapa 2 do wizard quem rola é a
   * página inteira, e prender a lista numa altura fixa criaria uma segunda barra
   * dentro de uma tela que já rola.
   *
   * Nenhum comportamento de pagamento muda com isto (`FR-009`): as mesmas
   * formas, as mesmas integrações, o mesmo rascunho de forma.
   */
  readonly alturaFixa?: boolean;
}

/**
 * O miolo do pagamento (008) — cabeçalho, condição, desconto de capa, forma,
 * valor recebido e a lista de formas aplicadas.
 *
 * **Extraído de `PainelPagamentoETotais` pela feature 007** para que a etapa 2
 * do wizard mobile monte exatamente esta composição, e não uma cópia dela. O
 * que sobrou no painel é a moldura do desktop (a coluna de 392px, a faixa de
 * atalhos F6–F9 e o rodapé com total + finalizar); tudo o que decide pagamento
 * está aqui, num arquivo só, para os dois layouts (`SC-001`).
 *
 * O rascunho de forma mora aqui pelo mesmo motivo de sempre: quem compõe o
 * seletor de forma e o campo de valor é quem segura o valor que os dois leem.
 */
export function ConfiguracaoPagamento({
  alturaFixa = false,
}: ConfiguracaoPagamentoProps = {}): ReactElement {
  /**
   ...
   * pelas opções no ritmo do teclado, sem confirmar nenhuma.
   */
  ...
  return (
    <>
      ...
      {/* No desktop esta coluna **não rola**: quem rola é a lista de pagamentos
          ...
          desconto e campo de valor param no tamanho do próprio conteúdo.
          ...
          comboboxes, que se desenha 3px **para fora** da borda. `-mx-1 px-1`
          devolve essa folga por dentro.
          ...
          descontadas as alturas).

          No mobile (`alturaFixa: false`) nada disso vale: a etapa 2 do wizard
          rola inteira, então a coluna cresce com o conteúdo e não abre uma
          segunda barra dentro da primeira. */}
      <div
        className={cn(
          '-mx-1 flex flex-col gap-xs px-1',
          alturaFixa && 'min-h-0 flex-1 overflow-x-hidden overflow-y-auto',
        )}
        data-testid="configuracao-pagamento"
      >
        <SeletorCondicaoPagamento />
