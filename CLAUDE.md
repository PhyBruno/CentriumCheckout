...
- Ícones: `data-icon-name="..."` (no export) / propriedade `icon` de nós `type: "icon"` (no MCP) com `library: "lucide"`. **O nome do Pencil não é mais o nome do import.** Desde AD-201 (2026-09-09) a biblioteca do código é o **reicon** (`reicon-react`), e `lucide-react` foi removido do projeto: o `.pen` continua dizendo qual *conceito* de ícone a tela usa, mas o nome precisa ser traduzido para o catálogo do reicon antes de virar `import`. Boa parte traduz por identidade (`trash-2` → `Trash2`, `search` → `Search`), o resto não (`pencil` → `Pen`, `circle-check` → `CheckCircle`, `qr-code` → `Qr`, `user-round` → `User`, `scan-line` → `Scan`). A tabela completa das 25 traduções não óbvias, os dois desvios conscientes e as três pegadinhas de API (inclusive o whitespace que o SVG injeta no `textContent`) estão em AD-201, `.specs/project/STATE.md`. Para um nome que ainda não esteja na tabela, procure o equivalente no catálogo real (`node_modules/reicon-react/index.d.ts`) ou em https://reicon.dev/icons — nunca chute o identificador.
