...
| 004 — finalização/suspensão | lê `vendedorAtual.codigo` para montar `vendedorCodigo` em `CheckoutFaturarNFCe`; chama `trocarVendedor({ codigo, nome: null }, 'RASCUNHO')` ao retomar rascunho via `CarregarNFCe` (`research.md` D4); MUST bloquear o botão "Finalizar" quando `vendedorAtual === null` (`FR-006`, `SC-003`) — este contrato só garante que `vendedorAtual` reflete corretamente a ausência de default, não implementa o bloqueio |
...
| 011 — recuperação de NFCe | mesmo consumo de `trocarVendedor` que a feature 004 (`origem: 'RASCUNHO'`), reaproveitando o mecanismo de retomada de rascunho |
