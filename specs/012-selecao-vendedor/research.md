...
**Rationale**: O padrão "lista capta, singular resolve" existe para cliente/produto porque a lista paginada daqueles domínios **não** traz todos os campos que a venda precisa (ex.: `DescontoConvenio` só existe em `ClienteCheckout`, retorno de `GetCliente`). Para vendedor, `VendedoresItem` (`CheckoutListaVendedores.Vendedores_Vendedores` no schema real) já traz tudo que a venda consome (`VendedorCodigo`, `VendedorNome`) — inventar uma segunda chamada sem endpoint real seria impossível, e desnecessário: não há campo adicional a completar.
...
**Decision**: `CheckoutFaturarNFCe` — o schema retornado tanto por `CarregarNFCe` quanto por `GetDAV` (`AD-057`) — só tem `vendedorCodigo`, nunca um campo de nome (confirmado por inspeção direta do schema, linhas 1414-1451 do yaml). `vendedorSlice.trocarVendedor({ codigo, nome: null }, 'RASCUNHO')` é a mesma action pública que a feature 006 já reaproveita (com `origem: 'DAV'`, o default); a UI exibe `"Vendedor #<código>"` até o operador reabrir o modal de seleção e resolver o nome manualmente pela busca — mesma UX já decidida para cliente/vendedor sem nome em `AD-095`. Este plano só define a action; o call site que a invoca a partir de `CarregarNFCe` é responsabilidade exclusiva da fase Design de `011-recuperacao-nfce` — já executado (`specs/011-recuperacao-nfce/tasks.md` T022, correção 2026-09-01 da auditoria de lacunas).
...
**Correção pós-`/speckit-tasks` (`/speckit-analyze`, 2026-08-31)**: D4 deixava a assinatura de `trocarVendedor` sem resolver a distinção `'RASCUNHO'`/`'DAV'` internamente (nota final de `contracts/vendedor-domain-api.md` original). Resolvido com um segundo parâmetro opcional `origem: 'RASCUNHO' | 'DAV' = 'DAV'` — preserva a chamada de 2 argumentos já reservada pela feature 006, sem exigir mudança no que ela já especificou. Sem impacto de AD (decisão interna de assinatura, não achado de contrato do ERP).
