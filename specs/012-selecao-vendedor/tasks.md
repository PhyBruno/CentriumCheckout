...
- [ ] T001 Criar estrutura de diretórios desta feature: `src/client/services/vendedor/`, `src/client/features/vendedor/` (`src/client/stores/slices/` e `src/shared/schemas/` já existem, criados pelas features 001/002)
...
- [ ] T014 [P] [US1] Implementar `src/client/features/vendedor/CampoVendedorVenda.tsx`: exibe `vendedorAtual.nome`, ou `"Vendedor #<codigo>"` quando `nome === null` (`AD-095`/`research.md` D4); sem indicador de `origem` (`AD-053`) — depende de T003 — **consultar o Pencil MCP antes de implementar** (CLAUDE.md § "Referência visual (design)")
