...
**Nota de escopo — `FR-009`**: a pré-seleção efetiva do vendedor no estado de vendedor é responsabilidade da feature 012 (`.specs/features/selecao-vendedor/spec.md`, AD-024), ainda sem `data-model.md`/slice definido (`research.md` D7) — mesmo padrão de "dependência declarada, não implementada" já usado por `specs/004-finalizacao-suspensao-venda/data-model.md` para o mesmo campo `vendedorCodigo`. A única tarefa desta feature para `FR-009` é T022, que expõe `vendedorCodigo` como parte do retorno de `retomarRascunho`/`RascunhoCarregado` — a pré-seleção em si (o acceptance scenario 5 da spec) só fica satisfeita quando a feature 012 consumir esse valor.
