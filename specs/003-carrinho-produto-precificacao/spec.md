...
- O sistema valida saldo/estoque disponível ao inserir um produto? Sim, desde 2026-09-16 (AD-236): antes de inserir ou aumentar um item, o sistema compara a soma do produto na venda com o saldo reconsultado no ERP e, conforme a política da empresa, avisa ou bloqueia; reduzir a quantidade nunca é bloqueado. O ERP revalida ao faturar. (A resposta anterior, "não, é responsabilidade do ERP", foi superada.)
...
- **FR-011**: Antes de inserir um produto ou alterar a quantidade de um item, o sistema MUST reconsultar o saldo do produto no ERP e comparar com ele a soma da quantidade proposta com a das demais linhas ativas do mesmo produto (inclusive as de preço congelado); quando a soma for maior que o saldo, MUST avisar e seguir se a política da empresa (`FaturaProdutoSemSaldo`) for `'A'`, MUST NOT inserir nem alterar se for `'B'` — mantendo o produto na barra como prévia bloqueada, com o motivo, em todo caminho de inserção automática — e MUST NOT validar se for vazia. Soma igual ao saldo passa, reduzir a quantidade nunca é bloqueado e, se a reconsulta falhar, vale o último saldo conhecido. **Corrigido em 2026-09-16 (AD-236):** substitui a redação anterior, que proibia o Checkout de validar saldo.
...
- O Checkout valida o saldo de estoque primeiro (FR-011, AD-236) e o ERP revalida ao suspender ou faturar; o desconto de pedidos em aberto (`PRM0401`) e o fator de conversão de unidade continuam só no ERP. (A premissa anterior, de que o Checkout não validaria saldo, foi superada em 2026-09-16.)
