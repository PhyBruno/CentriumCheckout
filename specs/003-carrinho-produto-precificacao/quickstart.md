...
| `dinheiro.spec.ts` (total de linha) | `calcularTotalLinha` com quantidade inteira e com quantidade fracionária (pesável); desconto subtraído **do total, depois** da multiplicação; desconto maior que o total bruto resulta em piso `0`, nunca negativo; desconto `0` devolve `preço × quantidade` | §1 de `data-model.md`, I8 |
