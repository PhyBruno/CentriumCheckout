...
- [ ] T012 [US2] Implementar `importarVendaExistente(numeroDav: string, origemLista: {clienteCodigo, clienteNome}): Promise<void>` em `davQueries.ts` (`contracts/importacao-domain-api.md` §3): `fetchDav` (T011) → `mapearVendaExistente` (T003) → `carrinhoSlice.importarLinhasCongeladas` (T004) — depende de T003, T004, T011
...
- [ ] T014 [US2] Wire dependências injetadas com stub em `importarVendaExistente` (T012): `vendedorSlice.trocarVendedor({codigo: vendaImportada.vendedorCodigo, nome: null})` — stub `() => {}` até a feature 012 fornecer a implementação real (assinatura já desenhada em `specs/012-selecao-vendedor/data-model.md`, sem gap a resolver) — e `pagamentoSlice.importarFormasDePagamento(vendaImportada.formasDePagamento)` — stub `() => {}` até a feature 008 fornecer (ação nova, ainda não existe no contrato atual de `specs/008-pagamento-geral/`, precisa ser acrescentada lá quando tasqueada, mesmo padrão de extensão usado nesta própria fase para `DAV_IMPORTADO`/`CodCliente`) — dependência por injeção, não por import (mesmo padrão de `specs/004-.../tasks.md` T029); não bloqueia esta feature — depende de T012
...
- [ ] T017 [US2] Wire tratamento de erro de importação (D7, AD-052): falha de `fetchDav` (T011) ou de `importarVendaExistente` (T012) exibe Goey Toast e mantém a janela de importação aberta, **sem** popular o carrinho com dado parcial — depende de T012
- [ ] T018 [US2] Wire seleção/confirmação em `ModalImportacaoDav.tsx` (T008): selecionar um DAV captura `clienteCodigo`/`clienteNome` da própria linha da lista (D4, `research.md`); botão de confirmar importação chama `importarVendaExistente` (T012-T017); sucesso fecha o modal, erro mantém aberto com toast (T017) — depende de T008, T012, T017
...
- [ ] T023 [US2] E2E `tests/e2e/importacao-dav.spec.ts` (quickstart, Cenários 2-5, mesmo arquivo de T010): itens/preço/cliente/vendedor/pagamento do carrinho refletem exatamente o DAV (SC-001/SC-002); inserir produto novo depois da importação segue precificação normal enquanto as linhas importadas permanecem congeladas, `FaturarNFCe` chamado uma única vez com `NumeroNota` idêntico ao de `GetDav` e **sem** nenhum campo de DAV (Cenário 3, FR-008); fallback de descrição/vendedor por código (Cenário 4, AD-095/AD-096); tentar importar DAV já faturado exibe toast e não popula carrinho (Cenário 5, D7) — depende de T018, T022
...
- `vendedorSlice.trocarVendedor`/`pagamentoSlice.importarFormasDePagamento` (T014) chegam por injeção de dependência das features 012 e 008 respectivamente — nenhuma delas precisa estar implementada para esta feature ser completada e testada com stubs
