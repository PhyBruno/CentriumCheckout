...
- [X] T001 Criar estrutura de diretórios desta feature: `src/client/domain/importacaoVenda/`, `src/client/services/dav/`, `src/client/features/dav/` (`src/client/stores/slices/` e `src/shared/schemas/` já existem, criados pelas features 001/002)
...
- [X] T015 [US2] Wire `auditoriaSlice.registrarEventoAuditoria` em `importarVendaExistente` (T012): dispara `DAV_IMPORTADO` (factory de `specs/001-.../eventos.ts`, T020, AD-114) com `{numeroDav, numeroNota: vendaImportada.numeroNota, quantidadeLinhas, quantidadeFormasDePagamento}` — depende de T012
...
- [X] T019 [P] [US2] Integration test `tests/integration/importacaoDav.spec.ts`: `importarVendaExistente` sobrescreve cliente e vendedor mesmo com um cliente/vendedor default já selecionado antes da importação (FR-007) — depende de T013, T014
- [X] T020 [P] [US2] Integration test `tests/integration/importacaoDav.spec.ts`: nenhum evento de reprecificação (`repricarSku`) nem `PRODUTO_INSERIDO` é emitido pelas linhas importadas via `importarLinhasCongeladas` (`data-model.md` §6) — depende de T004
- [X] T021 [P] [US2] Integration test `tests/integration/importacaoDav.spec.ts`: `DAV_IMPORTADO` é emitido exatamente uma vez, com `quantidadeLinhas`/`quantidadeFormasDePagamento` corretos — depende de T015
- [X] T022 [P] [US2] Integration test `tests/integration/importacaoDav.spec.ts`: falha isolada de uma chamada de `GetProduto` (descrição) não bloqueia a importação nem as demais linhas — linha afetada mantém `codigoProduto` como fallback de descrição (AD-096) — depende de T016
- [X] T023 [US2] E2E `tests/e2e/importacao-dav.spec.ts` (quickstart, Cenários 2-5, mesmo arquivo de T010): itens/preço/cliente/vendedor do carrinho refletem exatamente o DAV (SC-001/SC-002; a parte de **pagamento** do Cenário 2 passo 6 fica limitada a confirmar que `pagamentoSlice.importarFormasDePagamento` foi chamado com os dados corretos via stub — a tela real de pagamento só existe quando a feature 008 for implementada, ver Notes); inserir produto novo depois da importação segue precificação normal enquanto as linhas importadas permanecem congeladas, `FaturarNFCe` chamado uma única vez com `NumeroNota` idêntico ao de `GetDav` e **sem** nenhum campo de DAV (Cenário 3, FR-008); fallback de descrição/vendedor por código (Cenário 4, AD-095/AD-096); tentar importar DAV já faturado exibe toast e não popula carrinho (Cenário 5, D7) — depende de T018, T022
...
## Resultado da implementação (2026-09-03)

As 25 tarefas concluídas. Gates finais: `npx tsc --noEmit` sem erro (T024), `eslint` limpo, **436** testes unitários/integração e **98** E2E verdes.

**T025 — cenários do `quickstart.md`:** os cinco viraram teste automatizado em `tests/e2e/importacao-dav.spec.ts`, incluindo as verificações que a tarefa pedia manualmente — `NumeroNota` como único elo com o DAV no payload de `FaturarNFCe` (o número do DAV aparece **só** dentro de `Log`, que é a trilha de auditoria local, AD-114); fallback de vendedor por código e de descrição por `GetProduto` sem bloquear a importação (AD-095/AD-096); e ausência da janela no layout compacto (AD-046). Os stubs de `trocarVendedor`/`importarFormasDePagamento` ficaram registrados como **item 39** de `.specs/project/PENDENCIES.md`.

**Três achados não previstos por nenhum artefato de design** (detalhe completo em AD-137, `.specs/project/STATE.md`):

1. **`identidadeVenda` faltava na orquestração.** T012–T018 não mencionam gravá-la, mas `NumeroNota` só chega a `FaturarNFCe` por `identidadeVenda.numeroNota`. Sem isso a venda importada seria faturada com `NumeroNota: 0` e o DAV nunca fecharia no ERP. `importarVendaExistente` ganhou a porta `definirIdentidadeVenda`. Não usa `abrirSessaoDeVenda`, que zeraria o histórico de auditoria da venda em andamento.
2. **Bug pré-existente da feature 005:** `useFinalizarOuSuspenderVenda` enviava `clienteCodigo: sessao.ClienteDefaultCodigo` literal, então **toda** NFCe saía para o consumidor padrão mesmo com outro cliente identificado. Corrigido e travado com dois testes de regressão.
