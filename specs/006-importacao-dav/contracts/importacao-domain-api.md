...
1. Copia `NumeroRascunho` (era `NumeroNota` até AD-235) sem transformação para `numeroRascunho` — preservado para reenvio **intacto** em `FaturarNFCe`. O DAV de origem **não** é modelado nem reenviado: o ERP identifica sozinho que a NFCe veio de um DAV (AD-107). Com `DavNum` fora do contrato, `NumeroRascunho` passou a ser o **único** elo com o DAV, por isso o item 5 abaixo trata sua ausência como erro de contrato, não como dado opcional.
...
3. Mapeia `clienteCodigo` diretamente e `ClienteNome` do documento para `clienteNome` (`null` quando vazio, AD-237). O cadastro completo continua vindo de `GetCliente` (AD-115); o nome do documento é o recuo quando essa chamada falha (ver §2, passo 4). `vendedorCodigo` vem do documento, **exceto** quando ele vem `0` e a linha da listagem traz um código — aí vale o da linha (`VendedorDaLista.codigo`, AD-235, divergência do ERP no item 56 de `PENDENCIES.md`). `vendedorNome` vem do documento e, na falta, do nome da linha (AD-172).
4. Mapeia `FormasDePagamento[]` 1:1, sem reclassificação (D6, `research.md`).
5. Nunca lança para dado ausente/opcional (ex.: documento sem forma de pagamento) — devolve arrays vazios; lança **só** se `clienteCodigo`/`vendedorCodigo`/`NumeroRascunho` vierem ausentes do schema Zod já validado (erro de contrato, não de dado de negócio).
...
7. `registrarEventoAuditoria(criarEventoDavImportado({ numeroDav, numeroRascunho: vendaImportada.numeroRascunho, quantidadeLinhas, quantidadeFormasDePagamento }))` — **não** existe um método `auditoriaSlice.registrar(tipo, detalhes)`; o dispatcher real (`specs/001-.../contracts/auditoria-events.md`) é `registrarEventoAuditoria(evento)`, e o evento é montado por uma factory function tipada (`criarEventoDavImportado`, `src/client/domain/auditoria/eventos.ts`, tipo #20 do catálogo, AD-114) — mesmo padrão usado por toda outra feature consumidora.
