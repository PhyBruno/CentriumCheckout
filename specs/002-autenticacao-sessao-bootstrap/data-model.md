...
| `SessaoUsuario.QtdMinCharParaConsulta` | integer (int64) | `GetSessao` | Piso de caracteres para disparar `GetListaProdutos` (AD-024), já com o mínimo de 3 aplicado pelo ERP em `PCheckout_GetSessao` — o Checkout **nunca** hardcoda 3. Consumido pela feature `carrinho-produto-precificacao` — só armazenado aqui |
| `SessaoUsuario.UsuarioTipoCodigoProduto` | string | `GetSessao` | Enviado **sempre** como `Tipocodproduto` em `GetProduto`, nunca inferido por chamada (AD-033). Consumido pela feature `carrinho-produto-precificacao` — só armazenado aqui |
| `SessaoUsuario.ClienteDefaultCodigo` | integer (int64) | `GetSessao` | Cliente que abre toda venda antes de qualquer identificação (AD-032); enviado como `Codcliente` em `GetProduto` desde a primeira inserção. Consumido pelas features `carrinho-produto-precificacao` e `identificacao-cadastro-cliente` — só armazenado aqui |

> **Nota (2026-09-02, AD-119):** as três linhas acima foram acrescentadas na implementação da feature 003. `sessaoUsuarioSchema` é `z.looseObject`, então esses campos já **chegavam** ao Dexie antes — o que faltava era declará-los, e portanto tipá-los. Toda feature que passe a consumir um campo do `GetSessao` deve acrescentá-lo aqui e ao schema na mesma tarefa: *repassar sem interpretar* (Constitution III) e *consumir sem tipar* (violação da Constitution IV) são coisas diferentes.
