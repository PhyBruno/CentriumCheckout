...
- **FR-023**: O sistema MUST recusar o acionamento, informando o operador e sem alterar a venda, quando a venda já tiver **forma de pagamento aplicada**, ou quando já tiver uma condição de pagamento selecionada **diferente** da condição do cenário acionado. **Decisão direta do usuário (2026-09-05):** o atalho carrega um par (condição, forma) e cada venda aceita uma condição só; sobre uma venda já iniciada por outro par, a única alternativa à recusa seria reconciliar par a par, e errar essa reconciliação lança o pagamento na condição errada, em silêncio. A recusa MUST nomear a saída (o "Limpar" do cartão de pagamento), porque insistir na tecla nunca resolve. A condição **igual** à do cenário não recusa: é o estado que um atalho recusado depois da seleção da condição deixa para trás, e travá-lo faria o atalho envenenar a própria retentativa.
