...
**Revisão `/speckit-analyze` (2026-09-01)**: 6 achados corrigidos nesta versão — **I1 (CRÍTICO para integração)**: `validarInsercao` precisa de um segundo argumento, `origem: 'MANUAL' | 'ATALHO_CENARIO'`, para o evento de auditoria; resolvido sem mudar nenhuma assinatura pública consumida por 013 — `aplicarPagamento` (008) embute `'MANUAL'` e `aplicarForma` (008, consumida pelo atalho) embute `'ATALHO_CENARIO'` internamente (`contracts/validacao-domain-api.md` §3, `specs/008-pagamento-geral/tasks.md` T010 já atualizado). **C1**: nova T013, cobrindo o quickstart Cenário 4 (falha de comunicação), antes sem tarefa dedicada. **C2**: T012 estendida para fechar I5 pelo call site real de `validarInsercao`, não só pela função pura (já testada em `specs/004-.../tasks.md` T009). **C3**: T008 estendida com a composição explícita de `ValidacaoDeps`. **M2**: T020 (E2E) estendida para confirmar paridade mobile/desktop (`FR-019`). **U1**: a medição de `SC-006` (<2s) foi movida de uma menção genérica em Polish para uma asserção explícita em T015 (E2E US1).
...
- `src/client/domain/venda/montarRetratoVenda.ts` **já está especificado** em `specs/004-finalizacao-suspensao-venda/tasks.md` T004, parametrizado para `'FATURAR' | 'SUSPENDER' | 'VALIDAR'`, com o teste de equivalência I5 (`VALIDAR` ≡ `FATURAR` exceto `SuspenderOuFaturar`) já coberto por T009 da 004. Esta feature **reutiliza** o módulo — nenhuma tarefa aqui o reimplementa (T012 fecha o mesmo I5 pelo call site real, ver C2 acima).
- `src/client/stores/slices/pagamentoSlice.ts` (008) **já chama** `validarInsercao(candidata, origem)` e `invalidarVeredito()` como dependências injetadas dentro de `aplicarPagamento`/`aplicarForma`/`removerPagamento` (T010), hoje presas a um stub sempre-aceita (T041). O payload do catálogo já ecoa `FormaEntrada` (T007/T027, AD-111).
...
- [X] T001 Criar estrutura de diretórios desta feature: `src/client/domain/validacaoVenda/`, `src/client/services/validacao/`, `src/client/features/validacao/`, `tests/unit/domain/validacaoVenda/` (`src/client/domain/venda/`, `src/client/stores/slices/` e `src/shared/schemas/` já existem, criados pelas features 001/004/008)
...
- [X] T002 [P] Implementar `src/client/domain/validacaoVenda/interpretarVeredito.ts`: tipos `Veredito` (união discriminada `ACEITA`/`RECUSADA`/`INDISPONIVEL`, `data-model.md` §1), `MensagemValidacao`, `SeveridadeERP`, `CausaIndisponibilidade`; funções `interpretarRespostaValidacao(resposta: ValidarNFCeOutput): Veredito` (ramifica **exclusivamente** em `Valido` — `Type` nunca decide bloqueio, AD-110; `Valido=false` sem mensagens ⇒ `RECUSADA` com mensagem genérica, `FR-008`), `vereditoDeFalha(causa): Veredito`, `autorizaFinalizacao(veredito: Veredito | null): boolean` (`null` ⇒ `false`; só `ACEITA` autoriza, `FR-015`)
- [X] T003 [P] Implementar `src/client/domain/validacaoVenda/projetarPagamentos.ts`: tipo `FormaCandidata` (carrega `fpgUtiCar` e `entrada` obrigatórios, `contracts/validacao-domain-api.md` §1); função pura `projetarPagamentos(aplicados, candidata): readonly PagamentoParaPayload[]` — lista "como ficaria" (`FR-002`), não muta `aplicados`
- [X] T004 [P] Implementar `src/shared/schemas/validarNFCe.schema.ts` (Zod): `mensagemValidacaoSchema` (`Id` com default `''`, `Type: number().int()`, `Description: string()`) e `validarNFCeOutputSchema` (`Valido: boolean()` **sem default** — ausência é `RESPOSTA_INVALIDA`, nunca aceite presumido; `messages` default `[]`) — `contracts/erp-validacao-api.md`
- [X] T005 Implementar `src/client/services/validacao/validarNFCeMutation.ts`: `useValidarNFCe()` como TanStack **mutation** (não query — sem cache, sem refetch, `research.md` D10), `POST /api/erp/ValidarNFCe` via proxy do BFF (002), `retry: 0` e timeout explícito; valida a resposta com `validarNFCeOutputSchema` (T004) antes de interpretar (T002); **nunca rejeita a promise** — toda falha (timeout, rede, HTTP de erro, corpo inválido) vira `vereditoDeFalha(causa)` (T002), para que nenhum call site precise de `try/catch` (I4) — depende de T002, T004
- [X] T006 Implementar `src/client/features/validacao/notificarVeredito.ts`: `notificarVeredito(veredito: Veredito, toast: GoeyToast): void` — `ACEITA` sem avisos: nenhuma notificação; `ACEITA` com avisos: uma notificação de **aviso** por mensagem, texto íntegro, auto-dismiss (`FR-005`/`FR-007`); `RECUSADA`: uma notificação de **erro** por mensagem, ou a genérica de `FR-008`; `INDISPONIVEL`: notificação de erro com texto próprio do Checkout, **distinta** de recusa de negócio, convidando a nova tentativa (`FR-009`) — depende de T002
- [X] T007 Implementar `src/client/stores/slices/validacaoVendaSlice.ts`: estado `{ vereditoVigente: Veredito | null, emValidacao: boolean }`; `validarInsercao(candidata: FormaCandidata, origem: 'MANUAL' | 'ATALHO_CENARIO'): Promise<Veredito>` — guarda de entrada `emValidacao === false` (senão devolve imediatamente o veredito de "ocupado" sem consultar, `FR-011`/I8); projeta a candidata (T003) sobre os pagamentos aplicados (dependência injetada `pagamentosAplicados()`, feature 008); monta o retrato via `montarRetratoVenda(snapshot, 'VALIDAR', pagamentosProjetados)` (dependência injetada `snapshotVenda()` + módulo reutilizado de 004, **não reimplementado**); chama `validar()` (T005); interpreta (T002); `ACEITA` ⇒ grava `vereditoVigente`, chama `notificarVeredito` (T006); `RECUSADA`/`INDISPONIVEL` ⇒ **não** toca `vereditoVigente`, chama `notificarVeredito` (T006) e `registrarEventoAuditoria` (dependência injetada, factory `VALIDACAO_VENDA_RECUSADA` já existente em 001 T018, com `origem` ecoado do parâmetro, condição/forma/motivo); `invalidarVeredito(): void` ⇒ `vereditoVigente = null`; seletor `podeFinalizar(): boolean` ⇒ `autorizaFinalizacao(vereditoVigente)` (T002). **`origem` é fornecido pelo chamador conforme o ponto de entrada** — `aplicarPagamento` (008) sempre passa `'MANUAL'`, `aplicarForma` (008, consumida pelo atalho da 013) sempre passa `'ATALHO_CENARIO'`; contrato atualizado em `contracts/validacao-domain-api.md` §3 (achado I1 do `/speckit-analyze`, 2026-09-01, já resolvido — nenhuma assinatura pública de 008/013 muda) — depende de T002, T003, T005, T006
- [X] T008 Combinar `validacaoVendaSlice.ts` (T007) em `src/client/stores/vendaStore.ts` (já existe, feature 001), compondo explicitamente `ValidacaoDeps`: `snapshotVenda` (features 001/002/003/005/012, já reais nesta altura do projeto), `pagamentosAplicados` (008, já real), `registrarEvento` (001, já real) — nenhuma delas é stub, ao contrário do padrão que 008/004 usaram para consumir esta feature (008 T041, 004 T029), porque a 014 é construída depois que todas as suas próprias dependências já existem (achado C3 do `/speckit-analyze`, 2026-09-01) — depende de T007
- [X] T009 Substituir os dois stubs injetados deixados pelas features 004 e 008 pela implementação real do slice (T008), na composição de `vendaStore.ts`: `validarInsercao`/`invalidarVeredito` (stub sempre-aceita de `specs/008-pagamento-geral/tasks.md` T041) e `podeFinalizar()` (stub sempre-`true` de `specs/004-finalizacao-suspensao-venda/tasks.md` T029) passam a apontar para as actions reais de `validacaoVendaSlice`; inclui ajustar `pagamentoSlice.ts` (008 T010) para que `aplicarPagamento` chame `validarInsercao(candidata, 'MANUAL')` e `aplicarForma` chame `validarInsercao(candidata, 'ATALHO_CENARIO')` — depende de T008
- [X] T010 [P] Unit test `tests/unit/domain/validacaoVenda/interpretarVeredito.spec.ts`: **teste negativo obrigatório** (AD-110) — `Valido=false` + `Type=Warning` ⇒ `RECUSADA` (não `ACEITA`); `Valido=true` + `Type=Warning` ⇒ `ACEITA` com avisos; `Valido=false` sem `messages` ⇒ `RECUSADA` com mensagem genérica (`FR-008`); `autorizaFinalizacao(null)===false`, `autorizaFinalizacao({resultado:'RECUSADA',...})===false` — I3, `FR-006` — depende de T002
- [X] T011 [P] Unit test `tests/unit/domain/validacaoVenda/projetarPagamentos.spec.ts`: candidata presente na lista projetada; lista `aplicados` original não é mutada (referência distinta); ordem preservada — I2 — depende de T003
- [X] T012 Integration test `tests/integration/validacaoVendaSlice.spec.ts`: `RECUSADA` não muta `vereditoVigente` nem estado externo (I1); dois `validarInsercao` disparados sem aguardar o primeiro ⇒ o segundo devolve imediatamente sem nova requisição (I8/`FR-011`); `retrato` enviado contém as formas aplicadas **mais** a candidata (I2, espiando o argumento de `montarRetratoVenda`); `invalidarVeredito()` zera `vereditoVigente` (I7); **e (achado C2 do `/speckit-analyze`, fecha I5 pelo call site real desta feature, complementando o teste da função pura em `specs/004-.../tasks.md` T009)**: para o mesmo snapshot e a mesma lista de pagamentos projetados, o retrato que `validarInsercao` de fato envia (via `montarRetratoVenda(..., 'VALIDAR', ...)`) é idêntico ao que `montarRetratoVenda(..., 'FATURAR', ...)` produziria, exceto `SuspenderOuFaturar` — depende de T008
- [X] T013 [P] Integration test mesmo arquivo `tests/integration/validacaoVendaSlice.spec.ts` (**achado C1 do `/speckit-analyze`, cobre o quickstart Cenário 4, antes sem tarefa dedicada**): timeout, erro de rede, resposta HTTP de erro e corpo que falha no schema (T004) produzem `INDISPONIVEL` (I4) com notificação **distinta** da recusa de negócio (`FR-009`); restabelecida a rota (mock voltando a responder normalmente), uma nova tentativa consulta de novo e aceita — nenhuma retentativa automática ocorre no meio (`research.md` D5, `retry: 0` de T005); resposta `200` sem o campo `Valido` produz o mesmo `RESPOSTA_INVALIDA` — depende de T012
...
- [X] T014 [P] [US1] Integration test mesmo arquivo `tests/integration/validacaoVendaSlice.spec.ts`: cliente com crédito bloqueado + condição a prazo ⇒ `RECUSADA`, nenhum pagamento (venda intacta), notificação de erro com o texto do ERP íntegro, evento `VALIDACAO_VENDA_RECUSADA` na auditoria com `origem: 'MANUAL'`, `podeFinalizar()===false` (I6) — cliente default + condição a prazo produz o mesmo desfecho — Acceptance Scenarios 1, 3, 4; `FR-001`, `FR-004`, `FR-007`, `FR-015`, `FR-018` — depende de T012
- [X] T015 [P] [US1] E2E `tests/e2e/validacao-previa.spec.ts` (quickstart Cenário 1): fluxo completo desktop, cliente bloqueado, notificação de erro com texto do ERP, auditoria, finalização bloqueada; corrigir a causa e repetir a inserção ⇒ nova consulta é feita (não reaproveita a anterior) e a inserção passa a ser aceita (Acceptance Scenario 2). **Medir o tempo entre a confirmação da inserção e o desfecho visível (pagamento aplicado ou recusa exibida) e asserir `< 2s`** — `SC-006`, achado U1 do `/speckit-analyze` (2026-09-01, mesmo padrão de medição de tempo já usado por `specs/013-.../tasks.md` T023) — depende de T009
...
- [X] T017 [P] [US2] Integration test mesmo arquivo (**teste negativo de severidade a nível de slice**, complementa T010): `EmpLimCre='B'` com a mesma forma do T016, mas `Valido=false` e mensagem `Type=Warning` ⇒ tratado como `RECUSADA` (nenhum pagamento aplicado, notificação de erro) — quickstart Cenário 3, Acceptance Scenario 4; `FR-006`, I3 — depende de T012
...
- [X] T018 [P] [US3] Integration test `tests/integration/validacaoVendaSlice.spec.ts`: espionar `validarInsercao` do `vendaStore` combinado (T008) e confirmar que `aplicarPagamento` (008, botão) chama com `origem='MANUAL'` e que `aplicarForma` (008, consumida pelo atalho F6–F9 da 013) chama com `origem='ATALHO_CENARIO'` — ambos invocam a **mesma** instância do gate, nenhum caminho implementa validação própria ou o contorna (achado I1 do `/speckit-analyze`, já resolvido em T007/T009) — Acceptance Scenario 2 de US3; `FR-001`, `SC-001` — depende de T009
- [X] T019 [P] [US3] Integration test mesmo arquivo: `RECUSADA` não aciona `iniciarIntegracao` (PIX/TEF) em nenhum caso — nem antes nem depois da consulta; `ACEITA` aciona a integração normalmente, só após o veredito favorável — quickstart Cenário 6, I9; `FR-010`, `SC-002` — depende de T012
- [X] T020 [P] [US3] E2E `tests/e2e/validacao-previa.spec.ts` (quickstart Cenários 5 e 6): acionar o atalho F6 numa venda que o ERP recusa ⇒ nenhum pagamento lançado, **nenhuma** finalização automática iniciada (mesmo com "encerra a operação" ligado), notificação idêntica à do caminho manual; F6 pressionado duas vezes em sequência rápida ⇒ uma única consulta (`FR-011`); forma PIX/TEF numa venda recusada ⇒ nenhuma cobrança/transação iniciada. **Repetir o mesmo cenário com `capacidades().plataforma === 'MOBILE'`** (mesma fixture mobile de `specs/007-.../tasks.md`/`specs/013-.../tasks.md` T024), confirmando que `validarInsercao` é chamado identicamente independente da plataforma — `FR-019`, achado M2 do `/speckit-analyze` (2026-09-01) — depende de T009

**Checkpoint**: User Story 3 coberta — o mesmo mecanismo do Foundational já satisfaz os dois caminhos de acionamento e as duas plataformas sem código adicional, exatamente como `research.md` D1/D8 previu.
...
- [X] T021 [P] [US4] Integration test mesmo arquivo: venda com último lançamento `ACEITA` e total coberto ⇒ `podeFinalizar()===true`, e a mutation de validação (T005, mockada/espiada) **não** é chamada de novo na finalização — Acceptance Scenario 1; `FR-013` — depende de T009
- [X] T022 [P] [US4] Integration test mesmo arquivo: remover um pagamento aplicado (008, `removerPagamento`) chama `invalidarVeredito()` (T007) ⇒ `vereditoVigente` volta a `null`; a próxima inserção, mesmo com candidata idêntica, gera **nova** consulta — quickstart Cenário 7; Acceptance Scenario 2; `FR-014` — depende de T009
- [X] T023 [P] [US4] Integration test mesmo arquivo: sem nenhum veredito favorável vigente (`vereditoVigente === null` ou `RECUSADA`/`INDISPONIVEL`), a finalização (004, `podeFinalizar()`) bloqueia a emissão mesmo com o total coberto — Acceptance Scenario 3; `FR-015`, `SC-005` — depende de T009
- [X] T024 [P] [US4] Integration test mesmo arquivo: pagamento dividido — inserir a primeira forma (ex.: dinheiro, crediário `0`, aceita) e depois a segunda (crediário, retrato muda) ⇒ **duas** consultas distintas, nunca reaproveitadas, agrupadas ou cacheadas; desfecho da segunda pode divergir do da primeira; remover e reinserir a segunda gera uma **terceira** consulta — quickstart Cenário 8; `FR-001a`, I2a — depende de T012
...
- T012, T013 (integration tests Foundational) em paralelo entre si
- T014, T015 (testes US1) em paralelo entre si
- T016, T017 (testes US2) em paralelo entre si
- T018, T019, T020 (testes US3) em paralelo entre si
- T021–T024 (testes US4) em paralelo entre si
...
Task: "E2E cenário 1 com medição de tempo <2s em tests/e2e/validacao-previa.spec.ts"
```
...
4. US3 → validar isoladamente (mesmo gate por qualquer caminho e plataforma)
...
- `montarRetratoVenda.ts` **não é reimplementado aqui** — é o módulo de `specs/004-finalizacao-suspensao-venda/tasks.md` T004, já parametrizado para `'VALIDAR'` e já testado (I5) por T009 da 004; T012 fecha o mesmo I5 pelo call site real desta feature
...
- **`origem` de `validarInsercao` (T007)** — achado I1 do `/speckit-analyze` (2026-09-01), já resolvido: `contracts/validacao-domain-api.md` §3 e `specs/008-pagamento-geral/tasks.md` T010 foram atualizados na mesma revisão; nenhuma assinatura pública de 008 ou 013 mudou
