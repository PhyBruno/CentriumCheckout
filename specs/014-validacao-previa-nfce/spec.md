...
- **FR-004**: Quando o ERP recusar a venda, o sistema MUST NOT efetivar a inserção, MUST preservar a venda exatamente no estado anterior ao acionamento, e MUST exibir ao operador **todos** os motivos devolvidos pelo ERP numa **janela de recusa**, que ele fecha antes de seguir. **Atualizado (2026-09-16, AD-239):** a recusa era uma notificação de erro por motivo — o pagamento não entrava, a lista não mudava e a explicação passava voando, indistinguível de um clique que não pegou. A janela é a mesma do desfecho recusado da finalização, com a cópia de pagamento. Aviso de aceite (`FR-005`) e indisponibilidade (`FR-009`) continuam em notificação.
